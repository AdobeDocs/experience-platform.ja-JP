...
| 匿名化 | 匿名デバイス。 場合によっては、 `useragents` 匿名化ソフトウェアによって変更された |
...
| TV | セットトップボックスに似ていますが、TV に組み込まれているデバイス。 |
| ホームアプライアンス | 冷蔵庫のような（通常は大きな）家電。 |
...
| ロボットイミテータ | Web サイトを訪問するロボットで、のようなロボットのふりをする [!DNL Google]ですが、実際にはそうではありません。 **注意**:ほとんどの場合、ロボットイミテータは確かにロボットです。 |
