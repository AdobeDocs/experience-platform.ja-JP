...
このガイドを読む前に、カタログ開発者ガイドの「[はじめに](../../catalog/api/getting-started.md)」で説明されている手順に従って、[!DNL Experience Platform] API を呼び出すために必要な資格情報を収集します。
...
>[!NOTE]
>
>現在、該当するデータセットにラベルが存在する場合、新しいラベルは PUT リクエストを通じてのみ追加でき、これには `If-Match` ヘッダーが必要です。ラベルがデータセットに追加されると、後でラベルを更新または削除するために最新の`etag`値が必要になります。<br>PUT メソッドを実行する前に、データセットラベルに対してGET リクエストを実行する必要があります。 リクエストの変更を目的とした特定のフィールドのみを更新し、残りのフィールドは変更しないでください。 さらに、PUT呼び出しがGET呼び出しと同じ親エンティティを保持していることを確認します。 こうした相違は、顧客のエラーにつながります。
...
>[!NOTE]
>
>`labels` パラメーターに空のリストを指定することで、データセットのラベルを完全に削除できます。 データセットがラベルを保持することは必須ではありません。
