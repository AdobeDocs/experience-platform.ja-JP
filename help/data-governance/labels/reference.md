...
| ラベル | 定義 |
| --- | --- |
| **サードパーティ** | サードパーティデータは、サードパーティのデータベンダーによって提供されるデータです。サードパーティのデータベンダーとは、組織との間で、Platform と連携してサードパーティのデータにアクセス、使用、表示、送信することを承認する契約を締結したエンティティです。 |
| **サードパーティエンリッチメント** | サードパーティ組織が収集したデータで、データ主体と直接関係がないもの。 このラベルは、ファーストパーティプロファイルのエンリッチメントに使用されるサードパーティデータに適用する必要があります。 |
| **サードパーティの予測** | サードパーティ組織が収集したデータで、データ主体と直接関係がないもの。 ラベルは、新規顧客に見込まれるファネルのトップに使用されるサードパーティデータに適用する必要があります。 |
...
Adobe Experience Platform Segment Match を使用すると、Platform で生成したオーディエンスをプライバシーおよび同意の環境設定に一致させることができ、充実したプロファイリングやダウンストリームの洞察を促進できます。C11 ラベルは、[!DNL Segment Match] プロセスで使用すべきでないデータを示します。Segment Match から除外するデータセットやフィールドを決定し、それに応じて C11 ラベルを追加したら、そのラベルが Segment Match ワークフローによって自動的に適用されます。
