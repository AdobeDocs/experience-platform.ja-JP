...
パートナーエコシステムラベルは、組織の外部のソースから取得したデータの分類に使用されます。

このラベルは、見込み客データの使用を管理するために使用されます。

| ラベル | 定義 |
| --- | --- |
| **サードパーティ** | サードパーティデータは、サードパーティのデータベンダーによって提供されるデータです。サードパーティデータベンダーとは、Experience Platformと併せてサードパーティのデータにアクセス、使用、表示、送信することをユーザーに許可する契約をユーザーの組織と締結した事業体です。 |
| **第三者の利益** | データ主体と直接関係のないサードパーティの組織が収集したデータ。 このラベルは、ファーストパーティプロファイルのエンリッチメントに使用されるサードパーティデータに適用する必要があります。 |
| **第三者試掘等** | データ主体と直接関係のないサードパーティの組織が収集したデータ。 このラベルは、新規のお客様へのfunnel見込み客に加えて、使用されるサードパーティデータに適用する必要があります。 |
...
一部のデータガバナンスポリシーでは、ステッチ ID データをパーソナライゼーションに使用することを制限しています。C10 ラベルは、オーディエンスの結合ポリシーで「プライベートグラフ」オプションが使用されている場合に、自動的にオーディエンスに適用されます。

#### C11 {#c11}

Adobe Experience Platform Segment Match を使用すると、Experience Platformで生成されたオーディエンスをプライバシーおよび同意の環境設定と照合して、エンリッチメントされたプロファイルやダウンストリームのインサイトを促進できます。 C11 ラベルは、[!DNL Segment Match] プロセスで使用すべきでないデータを示します。Segment Match から除外するデータセットやフィールドを決定し、それに応じて C11 ラベルを追加したら、そのラベルが Segment Match ワークフローによって自動的に適用されます。

#### C12 {#c12}

このラベルのデータは、Experience Platformから書き出すことはできません。 C12 のラベル付きフィールドは、CSV ダウンロード、API 使用、アクティベーションワークフローから除外されます。
