...
この動作を使用すると、正しいプロファイルを含むポリシーと同意ルールの組み合わせを設定できますが、設定された組織ルールに違反するデータは含めないようにすることができます。サンプルシナリオとしては、機密データを含めないようにしたいが、同意したユーザーをソーシャルメディア経由でマーケティングのターゲットにすることができる場合があります。このシナリオに必要な手順の概要を以下のインフォグラフィックに示します。
