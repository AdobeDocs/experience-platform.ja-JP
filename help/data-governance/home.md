...
description: Adobe Experience Platform データガバナンスを使用すると、顧客データを管理し、データの使用に適した規制、制限、ポリシーへのコンプライアンスを確保できます。カタログ化、データ系列、データ使用ラベル、データ使用ポリシー、マーケティングアクションのデータに関する制御の使用など、Experience Platform 内の様々なレベルで重要な役割を果たします。
...
## [!DNL Data Governance] フレームワーク

[!DNL Data Governance] フレームワークは、データの分類プロセスや、データ使用ポリシーの作成プロセスを簡素化および効率化します。データラベルが適用され、データ使用ポリシーが設定されたら、マーケティングアクションを評価して、データが正しく使用されていることを確認できます。

[!DNL Data Governance] フレームワークには、ラベル、ポリシーおよび適用という 3 つの主要な要素があります。
...
[!DNL Data Governance] を使用すると、データスチュワードは、適用されるポリシーのタイプに従ってデータを分類するために、使用ラベルをデータセットおよびフィールドレベルで適用できます。

[!DNL Data Governance] フレームワークには、次の 3 つの方法でデータを分類するために使用できる、事前定義されたデータ使用ラベルが含まれています。
...
マーケティングアクションの例としては、データセットをサードパーティのサービスにエクスポートする場合などがあります。設定されているポリシーで個人情報（PII）など特定の種類のデータはエクスポートできず、「I」ラベル（ID データ）がデータセットに適用された場合は、データ使用ポリシーの違反を知らせる応答が [!DNL Policy Service] から返されます。

データ使用ラベルが適用されると、データスチュワードは [!DNL Policy Service] API または [!DNL Experience Platform] ユーザーインターフェイスを使用してポリシーを作成することができます。
...
このドキュメントでは、[!DNL Data Governance]と[!DNL Data Governance]フレームワークの概要について説明しました。これで、[データ使用状況ラベルのユーザーガイド](labels/user-guide.md)に進み、エクスペリエンスデータへの使用状況ラベルの追加を開始できます。

## 付録

次のセクションでは、[!DNL Data Governance]に関する追加情報を示します。

### [!DNL Data Governance] 用語

次のテーブルに、[!DNL Data Governance]と[!DNL Data Governance]フレームワークに関連する主な用語の概要を示します。
...
| **機密ラベル** | 「S」で始まる機密ラベルは、自分自身および組織が機密性が高いと見なすデータの分類に使用されます。 |

## その他のリソース

次のビデオは、[!DNL Data Governance]フレームワークについて理解を深めるためのものです。

>[!VIDEO](https://video.tv.adobe.com/v/29708?quality=12&enable10seconds=on&speedcontrol=on)

次のビデオでは、Experience Platform の様々な[!DNL Data Governance]機能の紹介を行っています。
