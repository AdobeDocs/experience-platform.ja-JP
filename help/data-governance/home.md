...
マーケティングアクションの例としては、データセットをサードパーティのサービスにエクスポートする場合などがあります。個人を特定できる情報（PII）を書き出せないというポリシーが設定されていて、「I」ラベル（ID データ）がデータセットに適用されている場合、[!DNL Policy Service] は、このデータセットをサードパーティの宛先に書き出すアクションを防ぎます。こうしたアクションのいずれかが試行された場合、ポリシーサービスは、データ使用ポリシーに違反したことを示すメッセージを送信します。
