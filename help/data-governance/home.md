...
データガバナンスを使用すると、データスチュワードは、適用されるポリシーのタイプに従ってデータを分類するために、使用状況ラベルをスキーマフィールドレベルとデータセットレベルで適用できます。
...
マーケティングアクションの例としては、データセットをサードパーティのサービスにエクスポートする場合などがあります。個人を特定できる情報 (PII) をエクスポートできないと宣言するポリシーが存在し、フィールドレベルまたはデータセットに「I」ラベル（ID データ）が適用されている場合、 [!DNL Policy Service] は、このデータセットをサードパーティの宛先に書き出すアクションを防ぎます。 こうしたアクションのいずれかが試行された場合、ポリシーサービスは、データ使用ポリシーに違反したことを示すメッセージを送信します。
...
| **データセットラベル** | スキーマにラベルを追加できます。 データセット内のすべてのフィールドは、スキーマのラベルを継承します。 |
| **フィールドラベル** | フィールドラベルは、スキーマから継承されるデータガバナンスラベル、またはフィールドに直接適用されるデータガバナンスラベルです。 フィールドに適用されたデータガバナンスラベルは、スキーマレベルには継承されません。 |
...
| **スキーマラベル** | データガバナンス、同意、アクセス制御のラベルをスキーマレベルで管理します。 これにより、そのスキーマを使用するすべてのデータセットにラベルが反映されます。 |
