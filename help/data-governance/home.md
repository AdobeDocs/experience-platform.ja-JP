...
データガバナンスを使用すると、データ管理人は、適用されるポリシーのタイプに従ってデータを分類するために、使用ラベルをスキーマフィールドレベルおよびデータセットレベルで適用できます。
...
マーケティングアクションの例としては、データセットをサードパーティのサービスにエクスポートする場合などがあります。個人を特定できる情報（PII）を書き出せないというポリシーが施行されていて、フィールドラベルまたはデータセットに「I」ラベル（ID データ）が適用されている場合、[!DNL Policy Service] により、このデータセットをサードパーティの宛先に書き出すアクションが妨げられます。こうしたアクションのいずれかが試行された場合、ポリシーサービスは、データ使用ポリシーに違反したことを示すメッセージを送信します。
...
| **興味ベースのターゲティング** | 次の 3 つの条件が満たされた場合、興味ベースのターゲット設定（パーソナライゼーションとも呼ばれます）が発生します。オンサイトで収集されたデータは、ユーザーの興味について推測するために使用され、別のサイトやアプリ（オフサイト）などの別のコンテキストで使用され、それらの推測に基づいて提供されるコンテンツや広告を選択するために使用されます。 |
...
| **スキーマラベル** | データガバナンス、同意、アクセス制御のラベルをスキーマレベルで管理します。これにより、そのスキーマを使用するすべてのデータセットにラベルが生成されます。 |
...
次のビデオでは、Experience Platform のスキーマとデータセットにデータ使用ラベルを適用する方法に関するガイダンスを示します。
