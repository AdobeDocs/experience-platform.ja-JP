...
データガバナンスのコンテキストでは、系列を使用すると、データ使用ラベルをデータセットから、データを使用するダウンストリームサービス（リアルタイム顧客プロファイルや宛先など）に伝播させることができます。これにより、Platform を介したデータのジャーニーのいくつかの重要なポイントでポリシーを評価および適用でき、ポリシー違反が発生した理由に関して、データコンシューマーにコンテキストが提供されます。
...
有料メディアのアクティベーションを含むストリーミング先に対する同意ポリシーの実施が強化されました。 この機能強化は、プライバシーとセキュリティシールドまたはヘルスケアシールドのお客様が利用でき、同意ステータスの変更に応じて、ストリーミング先からプロファイルを積極的に削除できます。 また、適切なオーディエンスが常にターゲットにされるように、同意の変更が即座に反映されます。

これらの改善により、マーケターがセグメント式に同意属性を手動で追加する必要がなくなり、マーケティング戦略に対する信頼を高めることができます。 これにより、同意が取り下げられたり、同意ポリシーの対象として認定されなくなったりした場合に、プロファイルが誤ってマーケティングエクスペリエンスのターゲットにされることを防ぐことができます。 様々なマーケティングワークフローで同意や環境設定データを管理する方法に関するルールを設定するマーケティング同意ポリシーが、ダウンストリームソリューションのアクティベーションワークフローで自動的に適用されるようになりました。
