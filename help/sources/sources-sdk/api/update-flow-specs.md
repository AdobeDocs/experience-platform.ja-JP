...
で作成されたソース `generic-rest-extension` すべてをテンプレートに使用 `RestStorageToAEP` フロー仕様。 このフロー仕様は、 `/flowSpecs/` エンドポイント、および `flowSpec.id` / `6499120c-0b15-42dc-936e-847ea3c24d72`.
...
次のリクエストでは、 `6499120c-0b15-42dc-936e-847ea3c24d72` 接続の仕様。
...
接続操作を使用して、接続仕様のフィールドをPUTできます。 PUTリクエストを通じてPOST仕様を更新する場合、本文には、接続リクエストで新しい接続仕様を作成する際に必要となるすべてのフィールドを含める必要があります。

>[!IMPORTANT]
>
>次のリストを更新する必要があります： `sourceConnectionSpecIds` 新規ソースが作成されるたびに、新規ソースに対応するフロー仕様の情報。 これにより、新しいソースが既存のフロー仕様でサポートされるようになり、新しいソースでデータフロー作成プロセスを完了できます。
...
次のリクエストは、のフロー仕様を更新します。 `6499120c-0b15-42dc-936e-847ea3c24d72` 接続仕様 ID を含めるには `f6c0de0c-0a42-4cd9-9139-8768bf2f1b55`.
