...
有効な [!DNL Shopify] に接続するためのパートナーアカウント [!DNL Shopify] API パートナーアカウントをまだお持ちでない場合は、 [[!DNL Shopify] パートナーダッシュボード](https://www.shopify.com/partners).

### アプリケーションを作成する

有効な [!DNL Shopify] パートナーアカウントを使用する場合は、パートナーダッシュボードを使用して先に進み、アプリを作成できます。 でアプリを作成する方法に関する包括的な手順については、 [!DNL Shopify]を読む [[!DNL Shopify] はじめにのガイド](https://www.shopify.com/partners/blog/17056443-how-to-generate-a-shopify-api-token).
...
これで、クライアント ID、クライアント秘密鍵、認証コードが揃い、アクセストークンを取得できます。 アクセストークンを取得するには、ドメインの `myshopify.com` この URL をに付加する際の URL [!DNL Shopify's] API エンドポイント： `/admin/oauth/access_token`.
...
次のリクエストを実行すると、 [!DNL Shopify] インスタンス。
...
Web フックを使用すると、アプリケーションとの同期を維持できます [!DNL Shopify] ショップで特定のイベントが発生した後に、データを取得したりアクションを実行したりします。 ストリーミング用 [!DNL Shopify] データをExperience Platformに送信する場合、Web フックを使用して、http エンドポイントとサブスクリプション用のトピックを定義できます。
...
正常な応答は、Webhook に関する情報 ( 対応する `id`、アドレス、およびその他のメタデータ情報。
...
* [!DNL Shopify] は、HTTP 2xx ステータス応答を成功通知として処理します。 その他のステータスコードの応答は、失敗と見なされます。 [!DNL Shopify] には、失敗した Webhook 通知の再試行メカニズムが用意されています。 存在する場合 **5 秒待った後の応答なし**, [!DNL Shopify] 接続を再試行します **19 回** 次の期間に **48 時間**. 再試行期間の終わりまでに応答がない場合は、 [!DNL Shopify] ウェブフックを削除します。
