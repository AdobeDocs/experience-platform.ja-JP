...
アプリが作成されたら、**ーザーパートナーダッシュボードの「** クライアント資格情報 **」タブから** クライアント ID **および** クライアント秘密鍵 [!DNL Shopify] を取得します。 クライアント ID とクライアント秘密鍵は、次の手順で認証コードとアクセストークンを取得するために使用されます。
...
| `api_key` | [!DNL Shopify] クライアント ID。 クライアント ID は、**ーザーパートナーダッシュボードの「** クライアント資格情報 [!DNL Shopify]」タブから取得できます。 |
...
これで、クライアント ID、クライアント秘密鍵、認証コードが用意できたので、アクセストークンを取得できます。 アクセストークンを取得するには、API エンドポイント `myshopify.com` を使用してこの URL を追加する際に、ドメインの [!DNL Shopify's] URL`/admin/oauth/access_token`POST リクエストを行います。
...
| パラメーター | 説明 |
| --- | --- | 
| `webhook.address` | ストリーミングメッセージが送信される http エンドポイント。 Webhook のテンプレートは `https://dcs.adobedc.net/collection/%7BINLET_ID%7D?{x-adobe-flow-id}={FLOW_ID}.` です。 |
...
* 同じリソースに対して異なるトピックの配信順を並べ替えることができるとは限りません。 例えば、`products/update` Webhook より前に `products/create` Webhook が配信される可能性があります。
