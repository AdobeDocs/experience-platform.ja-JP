---
title: SAP Hybris ソースの概要
description: API またはユーザーインターフェイスを使用して SAP Hybris をAdobe Experience Platformに接続する方法を説明します。
badge: ベータ
hide: true
hidefromtoc: true
...
この [!DNL SAP Hybris] ソースを使用すると、 [[!DNL SAP] サブスクリプション請求](https://www.sap.com/products/financial-management/subscription-billing.html) 以下のビジネスパートナー API エンドポイント：
...
また、 [!DNL SAP Hybris] を実行して顧客データを取得する場合、 [顧客と連絡先の関係](https://api.sap.com/api/BusinessPartner_APIs/path/GET_relationships-customer-contacts) また、API は、顧客の連絡先情報を取得するためにも呼び出されます。
...
次を持ってくる前に [!DNL SAP Hybris] Experience Platformにデータを送信する場合は、まず、次の条件を満たす必要があります。
...
* [!DNL SAP] サービスキー。 この [!DNL SAP] サービスキーを使用すると、 [!DNL SAP Subscription Billing] Experience Platformを介した API [!DNL SAP Hybris] には、以下が必要です。
   * クライアント ID
   * クライアントシークレット
   * URL. URL のパターンは次のとおりです。 `https://subscriptionbilling.authentication.eu10.hana.ondemand.com`. この値は後での値の取得に使用されます。 `region` および `tokenEndpoint` いつ [ベース接続を作成](../../tutorials/api/create/crm/sap-hybris.md#base-connection) API を使用する場合、または [接続 [!DNL SAP Hybris] アカウント](../../tutorials/ui/create/crm/sap-hybris.md#connect-account) Platform UI を使用します。
...
以下のドキュメントでは、API やユーザーインターフェイスを使用して [!DNL SAP Hybris] と Platform を接続する方法について説明します。

* [ソース接続とデータフローを作成して、 [!DNL SAP Hybris] API を使用した Platform へのデータの取得](../../tutorials/api/create/crm/sap-hybris.md).
* [接続 [!DNL SAP Hybris] UI を使用してExperience Platformにアカウント](../../tutorials/ui/create/crm/sap-hybris.md).
* [UI を使用した CRM ソースのデータフローの作成](../../tutorials/ui/dataflow/crm.md)
