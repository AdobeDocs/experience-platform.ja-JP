...
Adobe Experience Platformは、AWS、[!DNL Google Cloud Platform]、[!DNL Azure]などのクラウドプロバイダーにネイティブの接続を提供し、これらのシステムからデータを取り込むことができます。 取り込まれたデータは、JSON、パーケ、または区切り形式で形式設定できます。 クラウドストレージプロバイダのサポートには、[!DNL Apache] HDFSが含まれます。
...
## [!DNL Apache] HDFSを[!DNL Platform]に接続

以下のドキュメントは、APIまたはユーザーインターフェイスを使用して[!DNL Apache] HDFSを[!DNL Platform]に接続する方法に関する情報を提供しています。
