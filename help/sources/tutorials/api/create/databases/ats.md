...
title: フローサービスAPIを使用してAzureテーブルストレージコネクタを作成する
topic: overview
...
# フローサービスAPIを使用してAzureテーブルストレージコネクタを作成する
...
このチュートリアルでは、フローサービスAPIを使用して、Azureテーブルストレージ（以下「ATS」と呼ばれる）をエクスペリエンスプラットフォームに接続する手順を説明します。
...
次の節では、Flow Service APIを使用してATSに正常に接続するために知っておく必要がある追加情報を示します。
...
| 資格情報 | 説明 |
| ---------- | ----------- |
| `connectionString` | Azureテーブル接続インスタンスに接続するストレージ文字列。 |
| `connectionSpec.id` | 接続の作成に必要な一意の識別子。 ATSの接続指定IDはです `ecde33f2-c56f-46cc-bdea-ad151c16cd69`。 |

使い始めの詳細については、このATSの [ドキュメント](https://docs.microsoft.com/en-us/azure/storage/common/storage-introduction)。
...
接続はソースを指定し、そのソースの資格情報を含みます。 異なるデータを取り込むために複数のソースコネクタを作成するために使用できるコネクタは、ATSアカウントごとに1つだけ必要です。
...
ATS接続を作成するには、POST要求の一部として一意の接続指定IDを指定する必要があります。 ATSの接続指定IDはです `ecde33f2-c56f-46cc-bdea-ad151c16cd69`。
...
| パラメーター | 説明 |
| --------- | ----------- |
| `auth.params.connectionString` | ATSアカウントに関連付けられている接続文字列。 |
...
このチュートリアルに従うと、Flow Service APIを使用してATS接続を作成し、接続の一意のID値を取得したことになります。 このIDは、次のチュートリアルで、Flow Service APIを使用してデータベースを [探索する方法を学ぶ際に使用できます](../../explore/database-nosql.md)。
