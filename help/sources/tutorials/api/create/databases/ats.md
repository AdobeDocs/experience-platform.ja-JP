---
keywords: Experience Platform；ホーム；人気のあるトピック；[!DNL Azure Table Storage];[!DNL Azure Table Storage];Azure テーブルストレージ
solution: Experience Platform
title: フローサービス API を使用した Azure テーブルストレージベース接続の作成
...
# [!DNL Flow Service] API を使用して [!DNL Azure Table Storage] ベース接続を作成する
...
以下の節では、[!DNL Flow Service] API を使用して [!DNL Azure Table Storage] に正常に接続するために知っておく必要がある追加情報を示します。
...
| `connectionSpec.id` | 接続仕様は、ベース接続とソース接続の作成に関連する認証仕様を含む、ソースのコネクタプロパティを返します。 [!DNL Azure Table Storage] の接続仕様 ID は `ecde33f2-c56f-46cc-bdea-ad151c16cd69` です。 |
...
ベースPOSTID を作成するには、要求パラメーターの一部として [!DNL Azure Table Storage] 認証資格情報を指定しながら、`/connections` エンドポイントに接続要求を行います。
...
| `connectionSpec.id` | [!DNL Azure Table Storage] 接続仕様 ID:`ecde33f2-c56f-46cc-bdea-ad151c16cd69`. |
...
このチュートリアルでは、[!DNL Flow Service] API を使用して [!DNL Azure Table Storage] 接続を作成し、接続の一意の ID 値を取得しました。 この ID は、次のチュートリアルでフローサービス API](../../explore/database-nosql.md) を使用してデータベースを調べる方法を学ぶ際に使用できます。[
