...
# Flow Service APIを使用してAzure HDInsights ConnectorでApache Sparkを作成する
...
| `username` | Spark Serverへのアクセスに使用するユーザー名です。 |
| `password` | ユーザーに対応するパスワード。 |
| `connectionSpec.id` | 接続を作成するために必要な一意の識別子。 Sparkの接続仕様IDは次のとおりです。 `6a8d82bc-1caf-45d1-908d-cadabc9d63a6` |
...
| `auth.params.username` | Spark接続に関連付けられているユーザー名です。 |
...
このチュートリアルに従うことで、Flow Service APIを使用してSpark接続を作成し、接続の一意のID値を取得したことになります。 このIDは、Flow Service APIを使用してデータベースを [調査する方法を学習する際に、次のチュートリアルで使用できます](../../explore/database-nosql.md)。