...
`s3SessionToken` は一時的なもので、有効期限が切れたら更新する必要があります。 API にセッションリクエストを実行することで、ベースPATCHに関連付けられたセッショントークン [!DNL Flow Service] 更新できます。 [!DNL S3] の一時的セキュリティ資格情報について詳しくは、[[!DNL S3]  概要 ](../../../../connectors/cloud-storage/s3.md#azure) を参照してください。
