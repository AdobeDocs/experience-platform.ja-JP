...
>[!NOTE]
>
>FTP コネクタはベータ版です。 機能とドキュメントは変更される場合があります。詳しくは、 [ソースの概要](../../../../home.md#terms-and-conditions) ベータ版のコネクタの使用に関する詳細は、を参照してください。
...
以下の節では、 [!DNL FTP] サーバーの [!DNL Flow Service] API.
...
| `username` | へのアクセス権を持つユーザー名 [!DNL FTP] サーバー。 |
...
正常な応答は、一意の識別子 (`id`) に含まれます。 この ID は、次のチュートリアルで FTP サーバーを調べるために必要です。
