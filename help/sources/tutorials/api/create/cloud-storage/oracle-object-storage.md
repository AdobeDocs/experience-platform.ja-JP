...
# [!DNL Flow Service] API を使用して [!DNL Oracle Object Storage] ベース接続を作成する
...
以下の節では、[!DNL Flow Service] API を使用して [!DNL Oracle Object Storage] に正常に接続するために知っておく必要がある追加情報を示します。
...
| `connectionSpec.id` | 接続仕様は、ベース接続とソース接続の作成に関連する認証仕様を含む、ソースのコネクタプロパティを返します。 [!DNL Oracle Object Storage] の接続仕様 ID は次のとおりです。`c85f9425-fb21-426c-ad0b-405e9bd8a46c`. |
...
ベースPOSTID を作成するには、要求パラメーターの一部として [!DNL Oracle Object Storage] 認証資格情報を指定しながら、`/connections` エンドポイントに接続要求を行います。
...
| `auth.params.folderPath` | ユーザーがアクセスを制限している場合に必要な、許可されたフォルダーパス。 |
...
このチュートリアルでは、[!DNL Flow Service] API を使用して [!DNL Oracle Object Storage] 接続を作成し、一意の接続 ID を取得しました。 この接続 ID を使用して [ フローサービス API](../../explore/cloud-storage.md) を使用してクラウドストレージを調べることができます。
