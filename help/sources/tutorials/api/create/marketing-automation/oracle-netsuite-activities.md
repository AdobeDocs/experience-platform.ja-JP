---
title: Flow Service API を使用した、Oracle NetSuite アクティビティのソース接続とデータフローの作成
description: ソース接続とデータフローを作成し、Flow Service API を使用してOracle NetSuite イベントデータをExperience Platformに取り込む方法について説明します。
...
[!DNL Oracle NetSuite Activities]API[[!DNL Flow Service]  を使用して &#x200B;](https://www.adobe.io/experience-platform-apis/references/flow-service/) アカウントからAdobe Experience Platformにイベントデータを取り込む方法については、次のチュートリアルを参照してください。
...
次の節では、[!DNL Oracle NetSuite Activities] API を使用してに正常に接続するために必要な追加情報を示 [!DNL Flow Service] ています。
...
## [!DNL Oracle NetSuite Activities] API を使用した [!DNL Flow Service] のExperience Platformへの接続
...
ベース接続 ID を作成するには、`/connections` エンドポイントに対して POST リクエストを実行し、その際に [!DNL Oracle NetSuite Activities] 認証資格情報をリクエスト本文の一部として指定します。
...
次のリクエストは、[!DNL Oracle NetSuite Activities] のベース接続を作成します。
...
| `{SOURCE_PARAMS}` | Experience Platformに取り込むソースファイルのパラメーターを定義します。 `{SOURCE_PARAMS}` で受け入れ可能な形式タイプを取得するには、文字列全体を base64 にエンコードする必要があります。 <br>[!DNL Oracle NetSuite Activities] の値が空です。</li></ul> |
...
| `baseConnectionId` | [!DNL Oracle NetSuite Activities] のベース接続 ID。この ID は、前の手順で生成されました。 |
| `connectionSpec.id` | ソースに対応する接続仕様の ID。 |
| `data.format` | 取り込む [!DNL Oracle NetSuite Activities] データの形式。現在、サポートされているデータ形式は `json` のみです。 |
...
次のリクエストは、[!DNL Oracle NetSuite Activities] のターゲット接続を作成します。
...
| `data.format` | 取り込む [!DNL Oracle NetSuite Activities] データの形式。 |
...
[!DNL Oracle NetSuite Activities] からExperience Platformにデータを取り込むための最後の手順は、データフローを作成することです。 現時点で、次の必要な値の準備ができています。
