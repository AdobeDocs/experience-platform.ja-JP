...
以下のチュートリアルでは、ソース接続とデータフローを作成し、[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/) を使用してExperience Platformにデータを取り [!DNL Mailchimp Members] む手順を詳しく説明します。
...
[!DNL Mailchimp] 認証資格情報を取得したら、データをExperience Platformに取り込むためにデータフローを作成するプロセス [!DNL Mailchimp Members] 開始できます。 データフローを作成する最初の手順は、ベース接続を作成することです。
