...
# （ベータ版）のソース接続とデータフローの作成 [!DNL Mixpanel] の使用 [!DNL Flow Service] API
...
| 認証情報 | 説明 | 例 |
| --- | --- | --- |
| `host` | この [!DNL Mixpanel] 生データ書き出し API エンドポイント。 詳しくは、 [!DNL Raw Data Export API] セクション [Mixpanel API リファレンスドキュメント](https://developer.mixpanel.com/reference/overview) を参照してください。 | `https://data.mixpanel.com` |
...
ベース接続 ID を作成するには、 `/connections` エンドポイントを [!DNL Mixpanel] 認証資格情報をリクエスト本文の一部として使用します。
...
| `auth.params.host` | 登録プロセス中に作成されたアカウントに固有の一意のドメイン。 |
