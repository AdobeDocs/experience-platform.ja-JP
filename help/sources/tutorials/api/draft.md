---
title: フローサービスエンティティ API のドラフトの作成
description: フローサービス API を使用して、ベース接続、ソース接続、ターゲット接続、データフローのドラフトを作成する方法を説明します
...
# のドラフトを作成 [!DNL Flow Service] API を使用するエンティティ

以下を使用して、 `mode=draft` クエリーパラメーター [[!DNL Flow Service] API](<https://www.adobe.io/experience-platform-apis/references/flow-service/>) を設定する [!DNL Flow Service] ベース接続、ソース接続、ターゲット接続、データフローなどのエンティティをドラフト状態にします。

ドラフトは、後で新しい情報で更新し、準備が整ったら、 `op=publish` クエリパラメーター。

このチュートリアルでは、 [!DNL Flow Service] エンティティをドラフト状態に変更し、後で完了するためにワークフローを一時停止して保存できます。
...
また、使用しているソースの接続仕様 ID と対応するフロー仕様 ID がドラフトモードに対して有効になっているかどうかも確認する必要があります。

>[!BEGINTABS]

>[!TAB 接続仕様の詳細を検索]

+++Request 次のリクエストは、 [!DNL Azure File Storage]:
...
正常な応答は、ソースの接続仕様情報を返します。 ソースでドラフトモードがサポートされているかどうかを確認するには、 `items[0].attributes.isDraftModeSupported` の値は `true`.
...
+++Request 次のリクエストは、クラウドストレージソースのフロー仕様の詳細を取得します。
...
正常な応答は、ソースのフロー仕様情報を返します。 ソースでドラフトモードがサポートされているかどうかを確認するには、 `items[0].attributes.isDraftModeSupported` の値は `true`.
...
## ドラフトベース接続を作成する {#create-a-draft-base-connection}

ドラフトベース接続を作成するには、 `/connections` エンドポイント [!DNL Flow Service] API と提供 `mode=draft` をクエリパラメーターとして設定します。
...
| パラメーター | 説明 |
| --- | --- |
| `mode` | ベース接続の状態を決定する、ユーザーが指定したクエリパラメーター。 ベース接続をドラフトとして設定するには、 `mode` から `draft`. |

**リクエスト**

次のリクエストは、 [!DNL Azure File Storage] ソース：
...
正常な応答は、ベース接続 ID と、ドラフトベース接続に対応する etag を返します。 この ID を後で使用して、ベース接続を更新および公開できます。
...
## ドラフトベース接続を公開 {#publish-your-draft-base-connection}

下書きを公開する準備が整ったら、にPOSTリクエストを送信します。 `/connections` エンドポイントを開き、公開するドラフトベース接続の ID と、公開用のアクション操作を指定します。
...
| パラメーター | 説明 |
| --- | --- |
| `op` | 問い合わせられたベース接続の状態を更新するアクション操作。 ドラフトベース接続を公開するには、 `op` から `publish`. |

**リクエスト**

次のリクエストは、のドラフトベース接続を公開します。 [!DNL Azure File Storage] これは、前の手順で作成されたものです。
...
正常な応答は、ID と、公開されたベース接続に対応する e タグを返します。
...
ドラフトソース接続を作成するには、 `/sourceConnections` エンドポイント [!DNL Flow Service] API と提供 `mode=draft` をクエリパラメーターとして設定します。
...
| パラメーター | 説明 |
| --- | --- |
| `mode` | ソース接続の状態を決定する、ユーザーが指定したクエリパラメーター。 ソース接続をドラフトとして設定するには、 `mode` から `draft`. |
...
正常な応答は、ソース接続 ID と、ドラフトソース接続に対応する e タグを返します。 この ID を後で使用して、ソース接続を更新および公開できます。
...
## ドラフトソース接続を公開 {#publish-your-draft-source-connection}

>[!NOTE]
>
>関連付けられたベース接続がドラフト状態のままの場合、ソース接続を公開することはできません。 ソース接続を公開する前に、まずベース接続が公開されていることを確認してください。

下書きを公開する準備が整ったら、にPOSTリクエストを送信します。 `/sourceConnections` エンドポイントを開き、公開するドラフトソース接続の ID と、公開用のアクション操作を指定します。
...
| パラメーター | 説明 |
| --- | --- |
| `op` | 問い合わせ元の接続の状態を更新するアクション操作。 ドラフトソース接続を公開するには、 `op` から `publish`. |

**リクエスト**

次のリクエストは、のドラフトソース接続を公開します [!DNL Azure File Storage] これは、前の手順で作成されたものです。
...
正常な応答は、ID と、公開されたソース接続に対応する e タグを返します。
...
ドラフトターゲット接続を作成するには、 `/targetConnections` エンドポイント [!DNL Flow Service] API と提供 `mode=draft` をクエリパラメーターとして設定します。
...
| パラメーター | 説明 |
| --- | --- |
| `mode` | ターゲット接続の状態を決定する、ユーザーが指定したクエリパラメーター。 ターゲット接続をドラフトとして設定するには、 `mode` から `draft`. |
...
正常な応答は、ターゲット接続 ID と、下書きターゲット接続に対応する e タグを返します。 この ID を後で使用して、ターゲット接続を更新および公開できます。
...
## 下書きターゲット接続を公開 {#publish-your-draft-target-connection}

>[!NOTE]
>
>関連付けられたベース接続がドラフト状態のままの場合、ターゲット接続を公開することはできません。 ターゲット接続を公開する前に、まずベース接続が公開されていることを確認してください。

下書きを公開する準備が整ったら、にPOSTリクエストを送信します。 `/targetConnections` エンドポイントを開き、公開するドラフトターゲット接続の ID と、公開用のアクション操作を指定します。
...
| パラメーター | 説明 |
| --- | --- |
| `op` | 問い合わせ先の接続の状態を更新するアクション操作。 ドラフトターゲット接続を公開するには、 `op` から `publish`. |

**リクエスト**

次のリクエストは、の下書きターゲット接続を公開します [!DNL Azure File Storage] これは、前の手順で作成されたものです。
...
正常な応答は、ID と、公開された Target 接続に対応する e タグを返します。
...
正常な応答は、フロー ID と、ドラフトデータフローに対応する e タグを返します。 この ID を後で使用して、データフローを更新および公開できます。
...
## ドラフトのデータフローを公開 {#publish-your-draft-dataflow}

>[!NOTE]
>
>関連するソース接続とターゲット接続がまだドラフト状態の場合は、データフローを公開できません。 データフローを公開する前に、まずソース接続とターゲット接続が公開されていることを確認してください。
...
このチュートリアルでは、 [!DNL Flow Service] エンティティとこれらのドラフトの公開。 ソースの詳細については、 [ソースの概要](../../home.md).