...
このチュートリアルでは、有効なデータフローのID値が必要です。 有効なデータフローIDがない場合は、 [ソースの概要から選択したコネクタを選択し](../../home.md) 、このチュートリアルを試行する前に概要を説明した手順に従います。
...
このチュートリアルに従うと、 [!DNL Flow Service] APIを使用してデータフローの指標とエラー情報を取得できます。 これで、取り込みスケジュールに応じてデータフローを監視し続け、ステータスと取り込み率を追跡できます。 ユーザー・インタフェースを使用して同じタスクを実行する方法の詳細は、ユーザー・インタフェースを使用したデータ・フローの [監視に関するチュートリアルを参照してください](../ui/monitor.md)
