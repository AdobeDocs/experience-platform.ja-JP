...
ソースのベース接続 ID を指定したうえで [!DNL Flow Service] API に対してGETリクエストを行うことで、クラウドストレージフォルダーの構造に関する情報を取得できます。
...
このチュートリアルでは、クラウドストレージシステムを調べ、[!DNL Platform] に取り込むファイルのパスを見つけ、その構造を確認しました。 次のチュートリアルでこの情報を使用して [ クラウドストレージからデータを収集し、Platform に取り込む ](../collect/cloud-storage.md) ことができます。
