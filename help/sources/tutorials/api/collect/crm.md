...
title: ソースコネクタとAPIを通じてCRMデータを収集する
topic: overview
...
# ソースコネクタとAPIを通じてCRMデータを収集する

このチュートリアルでは、CRMシステムからデータを取得し、ソースコネクタとAPIを使用してプラットフォームにデータを取り込む手順を説明します。

## はじめに

このチュートリアルでは、有効なベース接続を通じてCRMシステムにアクセスでき、テーブルのパスや構造など、プラットフォームに取り込むテーブルに関する情報が必要です。 この情報がない場合は、このチュートリアルを試す前に、Flow Service APIを使 [用したCRMシステムの詳細に関するチュートリアル](../explore/crm.md) を参照してください。
...
以前の手順では、ソースデータを構造化するためのアドホックXDMスキーマが作成されていました。 ソースデータをPlatformで使用するには、必要に応じてソースデータを構成するターゲットスキーマも作成する必要があります。 次に、ターゲットスキーマを使用して、ソースデータが含まれるプラットフォームデータセットを作成します。
...
成功した応答は、CRMシステムからプラットフォームにデータを取り込む役割を持つデータフロー仕様の詳細を返します。 新しいデータフローを作 `id` 成するための次の手順で必要なフィールドの値を格納します。
...
CRMデータを収集する最後の手順は、データフローを作成することです。 現在は、次の必須の値が用意されています。
...
このチュートリアルに従うと、CRMシステムからデータをスケジュールに基づいて収集するソースコネクタを作成できます。 受信データは、リアルタイム顧客情報やData Science Workspaceなどのダウンストリームプラットフォームサービスで使用できるようになりました。 詳しくは、次のドキュメントを参照してください。
