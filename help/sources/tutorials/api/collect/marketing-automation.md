...
このチュートリアルでは、有効な接続を通じて、サードパーティのマーケティング自動化システムにアクセスでき、ファイルのパスや構造など、プラットフォームに組み込むファイルに関する情報が必要です。 この情報がない場合は、[Flow Service API](../explore/marketing-automation.md)を使用したサードパーティのマーケティング自動化システムの調査のチュートリアルを参照してから、このチュートリアルを試してください。
...
このチュートリアルに従うことで、ソースコネクタを作成して、マーケティング自動化システムからデータをスケジュールに基づいて収集します。 受信データは、[!DNL Real-time Customer Profile]や[!DNL Data Science Workspace]などのダウンストリームプラットフォームサービスで使用できるようになりました。 詳しくは、次のドキュメントを参照してください。
