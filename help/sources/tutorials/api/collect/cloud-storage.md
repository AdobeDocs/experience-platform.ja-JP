...
| `params.cdcEnabled` | 変更履歴の取り込みが有効かどうかを示すブール値。 リレーショナルスキーマと共に使用する場合、変更データキャプチャは、取り込み時に評価され、ターゲットスキーマには保存されない `_change_request_type` ータコントロール列（`u` — upsert、`d` – 削除）に依存します。 このプロパティは、次のクラウドストレージソースでサポートされています。 <ul><li>[!DNL Azure Blob]</li><li>[!DNL Data Landing Zone]</li><li>[!DNL Google Cloud Storage]</li><li>[!DNL SFTP]</li></ul>この機能の概要については、[Data Mirrorの概要 &#x200B;](../../../../xdm/data-mirror/overview.md) を参照してください。 実装の詳細については、使用に関するガイド [&#x200B; ソースでのデータキャプチャの変更 &#x200B;](../change-data-capture.md) および [&#x200B; リレーショナルスキーマのテクニカルリファレンス &#x200B;](../../../../xdm/schema/relational.md) を参照してください。 |
...
次の例では、名前に `premium` が含まれるすべての CSV ファイルの取り込みを指定するために、ファイルパスで正規表現を使用しています。
...
ソース接続を作成する場合は、`recursive` パラメーターを使用して、深くネストされたフォルダーからデータを取り込むことができます。
...
次の例では、`recursive: true` パラメーターを使用 [!DNL Flow Service] て、取り込みプロセス中にすべてのサブフォルダーを再帰的に読み取るように指示します。
...
マッピングセットを作成するには、ターゲット XDM スキーマ `$id` および作成するマッピングセットの詳細を入力する際に、[[!DNL Data Prep] API](https://developer.adobe.com/experience-platform-apis/references/data-prep/) の `mappingSets` エンドポイントに POST リクエストを行います。
