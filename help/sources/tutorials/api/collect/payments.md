...
title: ソースコネクタと API を使用した支払データの収集
...
# ソースコネクタと API を使用して支払データを収集する

このチュートリアルでは、サードパーティの支払い申請書からデータを取得し、ソースコネクタと [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/) を使用して Platform に取り込む手順を説明します。

## はじめに

このチュートリアルでは、有効な接続を通じて支払いアプリケーションにアクセスできるほか、Platform に取り込むファイルに関する情報（ファイルのパスと構造を含む）を入手できる必要があります。 この情報がない場合は、このチュートリアルを試す前に、[ フローサービス API](../explore/payments.md) を使用した支払い申し込みの調査に関するチュートリアルを参照してください。
...
| プロパティ | 説明 |
| -------- | ----------- |
| `baseConnectionId` | アクセスするサードパーティの支払い申し込みの一意の接続 ID。 |
...
このチュートリアルでは、支払い申請からデータを収集するソースコネクタをスケジュールに従って作成しました。 受信データは、[!DNL Real-time Customer Profile] や [!DNL Data Science Workspace] など、ダウンストリームの Platform サービスで使用できるようになりました。 詳しくは、次のドキュメントを参照してください。
