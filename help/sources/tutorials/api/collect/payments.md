---
keywords: Experience Platform；ホーム；人気の高いトピック；支払データの収集；支払データ
solution: Experience Platform
title: フローサービス API を使用した支払いソースのデータフローの作成
...
# を使用して、支払いソースのデータフローを作成します [!DNL Flow Service] API

このチュートリアルでは、支払いソースからデータを取得し、 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>データフローを作成するには、有効な [!DNL PayPal] ベース接続 ID。 この ID がない場合、 [を作成する [!DNL PayPal] ベース接続](../create/payments/paypal.md) このチュートリアルを実行する前に、
...
このチュートリアルでは、支払い申請からデータをスケジュールに基づいて収集するソースコネクタを作成しました。 受信データは、次のようなダウンストリームの Platform サービスで使用できるようになりました。 [!DNL Real-time Customer Profile] および [!DNL Data Science Workspace]. 詳しくは、次のドキュメントを参照してください。
