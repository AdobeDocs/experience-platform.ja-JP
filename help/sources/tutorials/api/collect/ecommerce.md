---
keywords: Experience Platform；ホーム；人気の高いトピック；e コマースデータの収集；e コマースデータ
solution: Experience Platform
title: フローサービス API を使用した e コマースソースのデータフローの作成
...
# を使用して、E コマースソースのデータフローを作成します。 [!DNL Flow Service] API

このチュートリアルでは、e コマースソースからデータを取得し、を使用して Platform に取り込む手順を説明します。 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>データフローを作成するには、有効な [!DNL Shopify] ベース接続 ID。 この ID がない場合、 [を作成する [!DNL Shopify] ベース接続](../create/ecommerce/shopify.md) このチュートリアルを実行する前に、
...
* [[!DNL Experience Data Model (XDM) System]](../../../../xdm/home.md):Experience Platformが顧客体験データを整理する際に使用する標準化されたフレームワーク。
   ...
   * [スキーマレジストリ API](../../../../xdm/api/getting-started.md):スキーマレジストリ API の呼び出しを正常に実行する方法を説明します。 これには、`{TENANT_ID}`、「コンテナ」の概念、リクエストをおこなうために必要なヘッダー（Accept ヘッダーとその可能な値に特に注意）が含まれます。
* [[!DNL Catalog Service]](../../../../catalog/home.md):カタログは、内のデータの場所と系列のレコードのシステムです [!DNL Experience Platform].
* [[!DNL Batch ingestion]](../../../../ingestion/batch-ingestion/overview.md):Batch Ingestion API を使用すると、にデータを取り込むことができます。 [!DNL Experience Platform] をバッチファイルとして使用します。
...
| プロパティ | 説明 |
| -------- | ----------- |
| `baseConnectionId` | e コマースソースの接続 ID。 |
...
データフローは、ソースからデータを収集し、それらをに取り込む役割を果たします [!DNL Platform]. GETフローを作成するには、まずに [!DNL Flow Service] API データフロー仕様は、e コマースソースからデータを収集する役割を果たします。
...
| `transformations.params.mappingId` | e コマースソースに関連付けられたマッピング ID。 |
| `scheduleParams.startTime` | エポックタイムでのデータフローの開始時間。 |
| `scheduleParams.frequency` | この `frequency` データフローがデータを収集する場所です。 指定できる値は次のとおりです。 `once`, `minute`, `hour`, `day`または `week`. |
| `scheduleParams.interval` | 間隔は、2 つの連続したフロー実行の間隔を示します。 間隔の値は、ゼロ以外の整数である必要があります。 間隔は、 `frequency` は次のように設定されます `once` およびは次よりも大きいか等しい必要があります `15` その他 `frequency` 値。 |

**応答**

正常な応答は ID を返します `id` 新しく作成されたデータフローの
...
このチュートリアルでは、データ e コマースをスケジュールに従って収集するソースコネクタを作成しました。 受信データをダウンストリームで使用できるようになりました [!DNL Platform] 次のようなサービス： [!DNL Real-time Customer Profile] および [!DNL Data Science Workspace]. 詳しくは、次のドキュメントを参照してください。
