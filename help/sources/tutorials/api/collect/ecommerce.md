...
このチュートリアルでは、e コマースソースからデータを取得し、[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/) を使用してExperience Platformに取り込む手順を説明します。
...
データフローは、ソースからデータを収集し、それらを [!DNL Experience Platform] に取り込みます。データフローを作成するには、まずは [!DNL Flow Service] API に GET リクエストを実行することで、データフローの仕様を取得する必要があります。データフロー仕様は、eコマースソースからのデータ収集を行います。
...
このチュートリアルでは、設定したスケジュールに従ってデータ eコマースを収集するソースコネクタを作成しました。 [!DNL Real-Time Customer Profile] や [!DNL Data Science Workspace] など、ダウンストリームの [!DNL Experience Platform] サービスで受信データを使用できるようになりました。詳しくは、次のドキュメントを参照してください。
