...
type: Tutorial
description: このチュートリアルでは、プロトコルアプリケーションからデータを取得し、ソースコネクタと API を使用してExperience Platformに取り込む手順について説明します。
...
このチュートリアルでは、[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/) を使用してプロトコルソースからデータを取得し、Experience Platformに取り込む手順を説明します。
...
データフローは、ソースからデータを収集し、それらをExperience Platformに取り込む役割を果たします。 データフローを作成するには、まず、プロトコルアプリケーションからデータを収集する役割を担うデータフロー仕様を取得する必要があります。
...
このチュートリアルでは、スケジュールに従ってプロトコルアプリケーションからデータを収集するソースコネクタを作成しました。これで、[!DNL Real-Time Customer Profile] や [!DNL Data Science Workspace] などのダウンストリームのExperience Platform サービスで受信データを使用できるようになりました。 詳しくは、次のドキュメントを参照してください。
