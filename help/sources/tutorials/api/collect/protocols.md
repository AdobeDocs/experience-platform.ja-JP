...
type: Tutorial
description: このチュートリアルでは、プロトコルアプリケーションからデータを取得し、ソースコネクタとAPIを使用してPlatformに取り込む手順について説明します。
...
このチュートリアルでは、有効なベース接続を通じてプロトコルシステムにアクセスし、テーブルのパスや構造など、Platformに取り込むファイルに関する情報を入手する必要があります。 この情報がない場合は、このチュートリアルを試す前に、[フローサービスAPI](../explore/protocols.md)を使用したプロトコルシステムの調査に関するチュートリアルを参照してください。
...
以下の節では、[!DNL Flow Service] APIを使用してプロトコルアプリケーションに正常に接続するために知っておく必要がある追加情報を示します。
...
データフローは、ソースからデータを収集し、Platformに取り込む役割を果たします。 データフローを作成するには、まず、プロトコルアプリケーションからデータを収集するデータフロー仕様を取得する必要があります。
...
| `transformations.params.deltaColum` | 新しいデータと既存のデータを区別するために使用される指定された列。 増分データは、選択した列のタイムスタンプに基づいて取り込まれます。 汎用ODataを使用する場合にサポートされる`deltaColumn`の形式は`yyyy-MM-ddTHH:mm:ssZ`です。 |
...
このチュートリアルでは、スケジュールに従ってプロトコルアプリケーションからデータを収集するソースコネクタを作成しました。 受信データは、[!DNL Real-time Customer Profile]や[!DNL Data Science Workspace]など、ダウンストリームのPlatformサービスで使用できるようになりました。 詳しくは、次のドキュメントを参照してください。
