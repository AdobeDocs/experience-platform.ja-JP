...
title: ソースコネクタと API を使用したプロトコルデータの収集
topic-legacy: overview
type: Tutorial
description: このチュートリアルでは、プロトコルアプリケーションからデータを取得し、ソースコネクタと API を使用して Platform に取り込む手順を説明します。
...
# ソースコネクタと API を使用したプロトコルデータの収集

このチュートリアルでは、サードパーティのプロトコルアプリケーションからデータを取得し、ソースコネクタと [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/) を使用してAdobe Experience Platformに取り込む手順を説明します。

## はじめに

このチュートリアルでは、有効な基本接続を通じてプロトコルシステムにアクセスし、テーブルのパスや構造など、Platform に取り込むファイルに関する情報を入手する必要があります。 この情報がない場合は、このチュートリアルを試す前に、[Flow Service API](../explore/protocols.md) を使用したプロトコルシステムの詳細に関するチュートリアルを参照してください。
...
データフローは、ソースからデータを収集し、Platform に取り込みます。 データフローを作成するには、まず、プロトコルアプリケーションからデータを収集する責任を持つデータフロー仕様を取得する必要があります。
...
| `transformations.params.deltaColum` | 新しいデータと既存のデータの区別に使用する指定された列。 増分データは、選択した列のタイムスタンプに基づいて取り込まれます。 汎用 OData を使用する場合にサポートされる `deltaColumn` の形式は `yyyy-MM-ddTHH:mm:ssZ` です。 |
...
このチュートリアルでは、スケジュールに従ってプロトコルアプリケーションからデータを収集するソースコネクタを作成しました。 受信データは、[!DNL Real-time Customer Profile] や [!DNL Data Science Workspace] など、ダウンストリームの Platform サービスで使用できるようになりました。 詳しくは、次のドキュメントを参照してください。
