---
keywords: Experience Platform；ホーム；人気の高いトピック；プロトコルデータの収集；プロトコルデータ
solution: Experience Platform
title: フローサービス API を使用したプロトコルソース用のデータフローの作成
...
# プロトコルソース用のデータフローの作成 [!DNL Flow Service] API

このチュートリアルでは、プロトコルソースからデータを取得し、を使用して Platform に取り込む手順を説明します。 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>データフローを作成するには、Platform 上の次のいずれかのプロトコルソースを持つ有効なベース接続 ID が既に存在する必要があります。<ul><li>[汎用 OData](../create/protocols/odata.md)</li><li>[汎用 REST](../create/protocols/generic-rest.md)</li></ul>
...
データフローは、ソースからデータを収集し、それらを Platform に取り込む役割を果たします。 データフローを作成するには、まず、プロトコルアプリケーションからデータを収集するデータフロー仕様を取得する必要があります。
...
| `transformations.params.deltaColum` | 新しいデータと既存のデータを区別するために使用される指定された列。 増分データは、選択した列のタイムスタンプに基づいて取り込まれます。 次の場合にサポートされる形式： `deltaColumn` 汎用 OData を使用する場合、 `yyyy-MM-ddTHH:mm:ssZ`. |
...
このチュートリアルでは、スケジュールに従ってプロトコルアプリケーションからデータを収集するソースコネクタを作成しました。 受信データは、次のようなダウンストリームの Platform サービスで使用できるようになりました。 [!DNL Real-time Customer Profile] および [!DNL Data Science Workspace]. 詳しくは、次のドキュメントを参照してください。
