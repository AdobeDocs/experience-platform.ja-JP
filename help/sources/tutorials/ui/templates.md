...
>[!NOTE]
>
>変更データキャプチャワークフローが必要なソースを操作する場合や、複数のデータモデルをサポートする場合、テンプレートでモデルベースのスキーマを自動生成できます。 これらのスキーマにより、Data Mirrorの機能でリアルタイムのデータ同期が可能になります。\
>モデルベースのスキーマを含むテンプレートを使用する場合、自動生成されたアセットには、必須のプライマリキー、バージョン識別子、タイムスタンプ識別子のフィールドが含まれます。\
...
| 商談 | 商談テンプレートには、タイプ、販売ステージ、関連アカウントなど、ビジネス商談の詳細が取り込まれます。 |
...
| 商談 | 商談テンプレートには、タイプ、販売ステージ、関連するアカウントなど、ビジネス商談の詳細が取り込まれます。 |
| 商談連絡先の役割 | 「商談担当者役割」テンプレートは、特定の商談に関連付けられた引合の役割に関する詳細を取得します。 |
...
デフォルトでは、自動生成されたデータフローはドラフト状態に設定され、マッピングルールやスケジュールされた頻度など、設定をさらにカスタマイズできます。 データフロー名の横にある省略記号（`...`）をクリックし、「**[!UICONTROL マッピングをプレビュー]** を選択して、ドラフトデータフロー用に作成されたマッピングセットを確認します。
...
>[!TIP]
>
>ドラフトデータフローには、ソースワークスペースの [!UICONTROL  データフロー ] カタログページからアクセスできます。 上部のヘッダーから **[!UICONTROL データフロー]** を選択し、更新するデータフローをリストから選択します。
>
...
次に、データフローの詳細を確認し、エラー診断、部分取り込み、アラート通知の設定を指定します。 終了したら、「**[!UICONTROL 次へ]**」を選択します。

![ ドラフトデータフローのデータフローの詳細手順。](../../images/tutorials/templates/dataflow-detail.png)

>[!NOTE]
>
>任意の時点で **[!UICONTROL ドラフトとして保存]** を選択して、データフローに加えた変更を停止して保存できます。
...
最後に、データフローの詳細を確認し、「**[!UICONTROL 保存して取り込み]**」を選択してドラフトを公開します。
