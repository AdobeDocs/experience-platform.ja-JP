---
keywords: Experience Platform；ホーム；人気の高いトピック；同意と環境設定；同意；onetrust;OneTrust
solution: Experience Platform
title: UI での同意および環境設定ソースを使用したデータフローの作成
...
# UI での同意および環境設定ソースを使用したデータフローの作成

データフローは、ソースからAdobe Experience Platformのデータセットにデータを取得して取り込むスケジュール済みタスクです。 このチュートリアルでは、Platform UI を使用して、同意および環境設定ソースのデータフローを作成する手順を説明します。

>[!NOTE]
>
>データフローを作成するには、 [!DNL OneTrust Integration] ソース。 に関するチュートリアルを参照してください。 [作成 [!DNL OneTrust Integration] UI のソース接続](../../ui/create/consent-and-preferences/onetrust.md) を参照してください。
...
同意および環境設定ソースアカウントを作成した後、 **[!UICONTROL データを追加]** の手順が表示され、同意および環境設定アカウントのテーブル階層を調査するためのインターフェイスが提供されます。
...
このチュートリアルに従うことで、データフローを作成し、同意および環境設定ソースから Platform にデータを取り込むことができます。 [!DNL Real-time Customer Profile] や [!DNL Data Science Workspace] など、ダウンストリームの [!DNL Platform] サービスで受信データを使用できるようになりました。詳しくは、次のドキュメントを参照してください。
