...
さらに、このチュートリアルでは、既に顧客成功アカウントを作成済みである必要があります。 UIで異なる顧客成功コネクタを作成するためのチュートリアルのリストは、[ソースコネクタの概要](../../../home.md)にあります。

## データの選択

顧客成功コネクタを作成すると、**[!UICONTROL データを選択]**&#x200B;の手順が表示され、ファイル階層を調べるためのインタラクティブインターフェイスが提供されます。
...
## XDMスキーマへのデータフィールドのマップ

**[!UICONTROL マッピング]**&#x200B;の手順が表示され、ソースデータを[!DNL Platform]データセットにマッピングするインタラクティブなインターフェイスが提供されます。

インジェスト先のインバウンドデータのデータセットを選択します。 既存のデータセットを使用するか、新しいデータセットを作成できます。

### 既存のデータセットを使用する

データを既存のデータセットにインジェストするには、**[!UICONTROL 「既存のデータセットを使用]**」を選択し、データセットアイコンをクリックします。
...
データを新しいデータセットにインジェストするには、**[!UICONTROL 「新しいデータセットを作成]**」を選択し、表示されるフィールドにデータセットの名前と説明を入力します。

「**[!UICONTROL スキーマを選択]**」検索バーにスキーマ名を入力して、スキーマフィールドを添付できます。 ドロップダウンアイコンを選択して、既存のスキーマの一覧を表示することもできます。 または、**[!UICONTROL 詳細検索]**&#x200B;を選択して、既存のスキーマの画面にそれぞれの詳細を含めてアクセスすることもできます。

この手順では、[!DNL Real-time Customer Profile]のデータセットを有効にし、エンティティの属性と動作の全体的なビューを作成できます。 有効なすべてのデータセットのデータが[!DNL Profile]に含まれ、データフローを保存する際に変更が適用されます。
...
このチュートリアルに従うと、顧客の成功ソースからデータを取り込むためのデータフローが正しく作成され、監視データセットに関する洞察が得られます。 受信データは、[!DNL Real-time Customer Profile]や[!DNL Data Science Workspace]などのダウンストリーム[!DNL Platform]サービスで使用できるようになりました。 詳しくは、次のドキュメントを参照してください。
