...
title: UI での顧客成功ソース接続のデータフローの設定
topic-legacy: overview
type: Tutorial
description: データフローとは、ソースから  [!DNL Platform]  データセットにデータを取得して取り込むスケジュールタスクです。 このチュートリアルでは、顧客成功アカウントを使用して新しいデータフローを設定する手順を説明します。
...
# UI での顧客成功接続のデータフローの設定

データフローとは、ソースから [!DNL Platform] データセットにデータを取得して取り込むスケジュールタスクです。 このチュートリアルでは、顧客成功アカウントを使用して新しいデータフローを設定する手順を説明します。
...
さらに、このチュートリアルでは、既にお客様の成功アカウントを作成している必要があります。 UI で様々な顧客成功コネクタを作成するためのチュートリアルのリストは、[ ソースコネクタの概要 ](../../../home.md) に記載されています。
...
- **[!UICONTROL 接続の詳細]**:ソースの種類、選択したソースファイルの関連パス、およびそのソースファイル内の列数を表示します。
- **[!UICONTROL マッピングの詳細]**:データセットが準拠するスキーマなど、ソースデータの取り込み先のデータセットを示します。
- **[!UICONTROL スケジュールの詳細]**:取り込みスケジュールのアクティブな期間、頻度、間隔を示します。
...
このチュートリアルでは、データフローを作成して、顧客の成功ソースからデータを取り込み、監視データセットに関するインサイトを得ました。 受信データは、[!DNL Real-time Customer Profile] や [!DNL Data Science Workspace] など、ダウンストリームの [!DNL Platform] サービスで使用できるようになりました。 詳しくは、次のドキュメントを参照してください。
