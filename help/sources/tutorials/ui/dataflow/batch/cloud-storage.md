...
クラウドストレージアカウントを作成すると、**[!UICONTROL データの追加]** 手順が表示され、クラウドストレージのファイル階層を調べ、Platform に取り込むフォルダーまたは特定のファイルを選択するためのインターフェイスが表示されます。
...
このチュートリアルでは、外部クラウドストレージからデータを取り込むためのデータフローを正常に作成し、データセットの監視に関するインサイトを得ました。 データフローの作成について詳しくは、以下のビデオを参照して、理解を補ってください。 さらに、[!DNL Real-Time Customer Profile] や [!DNL Data Science Workspace] などのダウンストリームの [!DNL Platform] サービスで受信データを使用できるようになりました。 詳しくは、次のドキュメントを参照してください。
