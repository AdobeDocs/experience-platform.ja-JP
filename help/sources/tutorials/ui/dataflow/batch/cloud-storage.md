...
必要に応じて、フィールドを直接マッピングするか、マッパー関数を使用してソースデータを変換し、計算済み値または計算済み値を取得することができます。 データマッピングとマッパー関数の詳細については、XDMスキーマフィールド](../../../../../ingestion/tutorials/map-a-csv-file.md)へのCSVデータのマッピングに関するチュートリアルを参照してください。[
...
1回限りの取り込みを設定するには、「頻度」ドロップダウン矢印を選択し、「**[!UICONTROL 1回]**」を選択します。 1回限りの頻度の取り込み用にデータフローセットを引き続き編集できます。これにより、開始時間が将来の期間に留まる限りです。 開始時間が過ぎると、1回限りの頻度の値は編集できなくなります。 **** 1回限りの **** 取得データフローを設定すると、インターバランドバックフィルが表示されない。
