...
1 回限りの取り込みを設定するには、頻度ドロップダウン矢印を選択し、「**[!UICONTROL 1 回]**」を選択します。 開始時間が将来になる限り、1 回限りの頻度取り込み用にデータフローセットを引き続き編集できます。 開始時間が過ぎると、1 回限りの頻度の値は編集できなくなります。 **** 1 回限りの **** 取得データフローの設定時に、インターバランドバックフィルが表示されない。
