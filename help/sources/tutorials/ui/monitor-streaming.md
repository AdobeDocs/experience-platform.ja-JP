...
このチュートリアルでは、[!UICONTROL Sources] Workspace を使用してストリーミングソースのデータフローを監視する手順を説明します。
...
この [!UICONTROL Dataflows] ページには、ソースデータ、アカウント名、データフロー実行ステータスに関する情報など、組織内の既存のすべてのデータフローのリストが含まれています。
...
[!UICONTROL Dataflow Activity] ページには、ストリーミングデータフローに関する特定の情報が表示されます。 上部のバナーには、取り込まれたレコードと、選択した日付範囲でのすべてのストリーミングデータフロー実行に対して失敗したレコードの累積数が含まれています。

![dataflow-activity](../../images/tutorials/monitor-streaming/dataflow-activity.png)

デフォルトでは、表示されるデータには、過去 7 日間の取り込みレートが含まれています。 「**[!UICONTROL Last 7 days]**」を選択すると、表示されるレコードの時間枠を調整できます。

カレンダーポップアップウィンドウが表示され、代替取り込み時間枠のオプションが表示されます。 データフロー実行時間枠を設定して、過去 7 日間または過去 30 日間のフロー実行を表示できます。 または、選択したカスタム時間枠を設定するようにインタラクティブカレンダーを設定します。 終了したら「**[!UICONTROL Apply]**」を選択します。
...
* **[!UICONTROL Dataflow run start]**：データフローの実行が開始された時刻。
* **[!UICONTROL Processing time]**：データフローの処理に要した時間。
* **[!UICONTROL Records Received]**：ソースコネクタからデータフローで受信したレコードの合計数です。
* **[!UICONTROL Records Ingested]**:[!DNL Data Lake] に取り込まれたレコードの合計数です。
* **[!UICONTROL Records with Warnings]**：取り込まれた警告のあるレコードの合計数。 マッパー変換エラーはすべて警告としてレポートされ、部分的に取り込まれた行には、警告を含む `success` というラベルが付けられます。 **メモ**：警告を含むレコードの取り込みのサポートは、ストリーミングソースでのみ使用できます。
* **[!UICONTROL Records Failed]**：データのエラーが原因で [!DNL Data Lake] に取り込まれなかったレコードの数。
* **[!UICONTROL Ingestion Rate]**:[!DNL Data Lake] に取り込まれたレコードの成功率です。 この指標は、[!UICONTROL Partial Ingestion] が有効な場合に適用されます。
* **[!UICONTROL Status]**：データフローの状態（[!UICONTROL Completed] または [!UICONTROL Processing]）を表します。 [!UICONTROL Completed] れは、対応するデータフロー実行のすべてのレコードが 1 時間以内に処理されたことを意味します。 [!UICONTROL Processing] は、データフロー実行がまだ終了していないことを意味します。

[!UICONTROL Dataflow run overview] ページには、対応するデータフロー実行 ID、ターゲットデータセット、組織 ID など、データフローに関する追加情報が含まれています。

エラーを含むフロー実行には、実行の失敗につながった特定のエラーと失敗したレコードの合計数を表示する [!UICONTROL Dataflow run errors] パネルも含まれています。

![dataflow-run-overview](../../images/tutorials/monitor-streaming/dataflow-run-overview.png)

### 警告のあるレコードの表示 {#warnings}

フロー実行中に発生したマッパー変換警告のリストが [!UICONTROL Records with warnings] に表示されます。 部分的に取り込まれた行は成功と見なされ、マッパー変換エラーが見つかった場合は警告が追加されます。
...
エラー診断を表示するには、「**[!UICONTROL Preview error diagnostics]**」を選択します。

![&#x200B; 警告を含むレコード &#x200B;](../../images/tutorials/monitor-streaming/records-with-warnings.png)

[!UICONTROL Error diagnostics preview] ウィンドウでは、データフローの実行に関する最大 100 個のエラーや警告をプレビューできます。 ここから、[!DNL Data Access] API を使用して、取り込み失敗マニフェストをダウンロードして詳細を確認することもできます。

![&#x200B; 診断 &#x200B;](../../images/tutorials/monitor-streaming/diagnostics.png)

## 次の手順

このチュートリアルでは、[!UICONTROL Sources] ワークスペースを正常に使用して、ストリーミングデータフローを監視し、失敗したデータフローの原因となったエラーを特定しました。 詳しくは、次のドキュメントを参照してください。
