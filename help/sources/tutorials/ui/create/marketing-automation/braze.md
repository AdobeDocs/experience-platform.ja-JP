...
Experience Platformの UI では、左側のナビゲーションを使用し、「**[!UICONTROL Schemas]**」を選択して、[!UICONTROL Schemas] ワークスペースにアクセスします。 次に、「**[!UICONTROL Create schema]**」を選択し、次に「**[!UICONTROL Experience Event]**」を選択します。 続行するには、「**[!UICONTROL Next]**」を選択します。

![&#x200B; 完了したスキーマ。](../../../../images/tutorials/create/braze/schema.png)

スキーマの名前と説明を指定します。 次に、[!UICONTROL Composition] パネルを使用してスキーマ属性を設定します。 [!UICONTROL Field groups] の下で「**[!UICONTROL Add]**」を選択し、[!UICONTROL Braze Currents User Event] フィールドグループを追加します。 終了したら「**[!UICONTROL Save]**」を選択します。
...
*マーケティング自動化* カテゴリで、「**[!UICONTROL Braze Currents]**」を選択し、次に「**[!UICONTROL Add data]**」を選択します。
...
次に、*time* から *timestamp* へのマッピングを削除し、追加（`+`）アイコンを選択してから、**[!UICONTROL Add calculated field]** を選択します。 表示されたボックスに *time \* 1000* と入力し、**[!UICONTROL Save]** を選択します。

新しい計算フィールドを追加したら、新しいソースフィールドの横にある **[!UICONTROL Map target field]** を選択し、スキーマのルートレベルで *timestamp* にマッピングします。 次に、「**[!UICONTROL Validate]**」を選択して、エラーがなくなったことを確認します。
...
終了したら「**[!UICONTROL Next]**」を選択します。 レビューページを使用してデータフローの詳細を確認し、「**[!UICONTROL Finish]**」を選択します。
