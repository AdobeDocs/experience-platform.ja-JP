---
title: UIを使用して、Talon.OneからExperience Platformにバッチデータを取り込みます
description: UIを使用してTalon.OneからAdobe Experience Platformにバッチデータを取り込む方法を説明します。 このガイドでは、設定、データ選択、データフロー設定について説明します。
badge: ベータ版
...
# UIを使用して[!DNL Talon.One]からExperience Platformにバッチデータを取り込む
...
[!DNL Talon.One]からデータを取り込むには、**[!UICONTROL Talon.One Batch Source Connector]**&#x200B;の下の&#x200B;*[!UICONTROL Loyalty]* ソースカードを選択し、**[!UICONTROL Add data]**&#x200B;を選択します。
...
![Talon.One バッチ ソース コネクタ カードが選択されたソース カタログ。](../../../../images/tutorials/create/talon-one-batch/catalog.png)

### 新しいアカウントを作成

>[!TIP]
>
>新しいアカウントを作成する前に、[概要の](../../../../connectors/loyalty/talon-one.md#prerequisites)前提条件[!DNL Talon.One]を確認して、どの認証情報が必要かを確認してください。
...
[!DNL Talon.One]のスキーマからAdobe Experience Platformにデータをマッピングする場合、現在、1つのトランザクション内で同じタイプの複数の効果をキャプチャすることはできません。 例えば、トランザクションに複数の`setDiscount`効果（異なるキャンペーンの割引など）が含まれている場合、マッピング中にそれらの効果のうち1つだけが保持され、他の効果は上書きされます。
