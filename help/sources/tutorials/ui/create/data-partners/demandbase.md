...
[!DNL Demandbase] を使用するには、**[!UICONTROL Demandbase Intent]** の下の [!UICONTROL Data & Identity Partners] ソースカードを選択してから、「**[!UICONTROL Add data]**」を選択します。
...
新しいアカウントを作成するには、「**[!UICONTROL New account]**」を選択してから、アカウント名を入力し、オプションで、アカウントの詳細の説明を入力します。 次に、Experience Platformに対してソースを認証するための適切な認証値を指定します。 [!DNL Demandbase Intent] アカウントを接続するには、次の資格情報が必要です。
...
* **開始時刻**：開始時刻は、データフローの最初の実行イテレーションが実行されるタイミングを示します。 [!DNL Demandbase] は、週に 1 回、月曜日、午後 12:00 （UTC）にAdobeにデータをドロップします。 したがって、取り込み開始時刻は UTC の午後 12:00 以降に設定する必要があります。 さらに、ファイルをAdobeにドロップする際にスケジュールが変更される可能性があるので、[!DNL Demandbase] で取り込み時間を確認する必要があります。
