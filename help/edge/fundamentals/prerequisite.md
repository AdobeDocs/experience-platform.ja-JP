...
- 組織内のユーザーに対して適切な権限を設定します。 すべてのExperience Cloudのお客様がデータ収集ツールにアクセスできるので、組織の各ユーザーに必要なのは、スキーマ、ID、データストリームに対する権限だけです。 これらの権限の設定方法の詳細については、 [データ収集権限の管理](https://experienceleague.adobe.com/docs/experience-platform/collection/permissions.html?lang=en).
