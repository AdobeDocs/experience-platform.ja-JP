...
IAB TCF 2.0をExperience Platform LaunchとAEP Web SDK拡張と共に使用するには、XDMスキーマを使用できる状態にする必要があります。 いずれの設定も行っていない場合は、先に進む前に [Adobe Experience PlatformWeb SDK JavaScriptクイック開始ガイドを参照して開始](../../getting-started/quick-start-without-launch.md) します。
...
このコードブロックは `useractioncomplete` イベントをリッスンし、同意文字列とフ `gdprApplies` ラグを渡して同意を設定します。 顧客のカスタムIDを持っている場合は、必ず `identityMap` 変数を入力してください。 呼び出しの詳細については、 [同意の](../../fundamentals/supporting-consent.md) サポートに関するガイドを参照してくだ `setConsent`さい。
