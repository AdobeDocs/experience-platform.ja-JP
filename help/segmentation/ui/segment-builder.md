...
>[!NOTE]
>
>Platform 内で作成されたオーディエンスの場合、次の条件を満たすオーディエンスのみ **同じ** 結合ポリシーが表示されます。

この **[!UICONTROL オーディエンス]** タブには、Adobe Audience ManagerやCustomer Journey Analyticsなどの外部ソースから読み込まれたすべてのオーディエンスと、内で作成されたオーディエンスが表示されます [!DNL Experience Platform].
...
>title="結合ポリシー"
>abstract="結合ポリシーを使用すると、様々なデータセットを結合してプロファイルを形成できます。 Platform にはデフォルトの結合ポリシーが用意されています。または、プロファイルに新しいデフォルトの結合ポリシーを作成できます。 このオーディエンスのマーケティング目的に合った結合ポリシーを選択します。"
...
>title="セグメント定義プロパティ"
>abstract="「セグメント定義のプロパティ」セクションには、結果のセグメント定義の推定サイズが表示され、プロファイルの合計数に対する適格なプロファイルの数が表示されます。 これにより、オーディエンスそのものを作成する前に、必要に応じてセグメント定義を調整できます。"
...
セグメント定義を作成する場合、 **[!UICONTROL オーディエンスプロパティ]** ワークスペースの右側の「」セクションには、結果のセグメント定義の推定サイズが表示され、オーディエンスそのものを作成する前に必要に応じてセグメント定義を調整できます。

**[!UICONTROL 認定プロファイル]** は、 **実際** セグメント定義のルールに一致するプロファイルの数。 この数は、セグメント評価ジョブが実行されてから 24 時間ごとに更新されます。

認定プロファイルのタイムスタンプは、最新の **バッチ** セグメント評価ジョブおよびは **ではない** ストリーミングまたはエッジセグメント化を使用して評価されたセグメント定義に対して表示されます。 セグメント定義を編集すると、次のセグメント評価ジョブが実行されるまで、認定プロファイルの数は同じままです。

**[!UICONTROL 推定プロファイル]** は **近似** からのプロファイルの数 **サンプルジョブ**. 新しいルールまたは条件を追加して選択すると、この値の更新されたバージョンを確認できます **[!UICONTROL 見積もりを更新]**. 情報バブルを選択すると、エラーしきい値と最新のサンプルジョブ時間が表示されます。

![認定プロファイルと推定プロファイルは、「オーディエンスプロパティ」セクションでハイライト表示されます。](../images/ui/segment-builder/audience-estimates.png)

この **[!UICONTROL オーディエンスプロパティ]** 「」セクションでは、名前、説明、評価タイプなど、セグメント定義に関する重要な情報を指定することもできます。 セグメント定義名は、組織で定義されたものの中から目的のセグメント定義を識別する際に使用されます。このため、セグメント定義名は、内容がわかりやすく簡潔、かつ一意である必要があります。
...
>[!NOTE]
>
>オーディエンスの推定は、当日のサンプルデータのサンプルサイズを使用して生成されます。プロファイルストアのエンティティ数が 100 万個未満の場合、完全なデータセットが使用されます。エンティティの数が 1～2,000 万の場合は 100 万のエンティティが使用され、2,000 万を超えるエンティティの場合は合計エンティティの 5% が使用されます。
>
...
また、セグメント定義の作成が完了したら、その評価方法を変更することもできます。 評価方法を「エッジ」または「ストリーミング」から「バッチ」に変更する場合は、次のようになります **ではない** エッジまたはストリーミングに戻すことができます。 評価方法を変更すると、次のようになります **のみ** を選択すると有効になります **[!UICONTROL 保存]** ポップオーバーで。 ダイアログをキャンセルすると、 **維持** 元の評価方法。
