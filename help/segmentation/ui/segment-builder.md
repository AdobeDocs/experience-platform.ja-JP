...
これらの構成要素は、**[!UICONTROL Fields]** のワークスペースの左側にある「[!DNL Segment Builder]」セクションに表示されます。 **[!UICONTROL Fields]** には、主要な各構成要素（「[!UICONTROL Attributes]」、「[!UICONTROL Events]」、「[!UICONTROL Audiences]」）のタブが含まれています。
...
| データタイプ | よくあるユースケース |
| --------- | ---------------- |
| 文字列 | 名前、メールアドレス、製品カテゴリ |
| 数値 | 年齢、売上高、製品数、ロイヤルティスコア |
| ブール | 環境設定、ステータスインジケーター、ユーザーフラグ |
| 列挙 | 定義済みリスト（性別やステータスなど） |
| 日付/時刻 | 購入日、訪問時間、誕生日 |
...
| 間 | 数値は、指定された値の **間** です。 これらの値は、指定された数値の **値を含む** です。 | 年齢 **25 ～ 45** |
...
| 次の前 | 値が選択した日付より前に発生しました。 | メンバーシップ参加日は 2025 年 1 月 3 日 **前** です |
...
時間関数と日付関数について詳しくは、[ 時間制約の節 ](#time-constraints) を参照してください。
...
計算済み属性の詳細（作成方法、内で使用できる関数、管理方法など）については、[ 計算属性の概要 ](/help/profile/computed-attributes/overview.md) を参照してください。
...
その後は、セグメント定義に「[!DNL ExperienceEvents]」 [!UICONTROL Event Types] 簡単にドラッグ&amp;ドロップできます。
...
オーディエンスの検証の仕組みについて詳しくは、[ オーディエンスの検証ガイド ](/help/segmentation/validation.md) を参照してください。
...
集計関数を作成するには、左側のパネルからイベントを選択して、[!UICONTROL Events] コンテナに挿入します。

![「イベント」セクションがハイライト表示されている様子](../images/ui/segment-builder/events.png)

イベントコンテナ内にイベントを配置した後、省略記号アイコン（...）を選択し、続いて「**[!UICONTROL Aggregate]**」を選択します。
...
カウント関数を作成するには、左側のパネルからイベントを選択して、[!UICONTROL Events] コンテナに挿入します。

![イベントフィールドがハイライト表示されている様子](../images/ui/segment-builder/events.png)

イベントコンテナ内にイベントを配置した後、「[!UICONTROL At least 1]」ボタンを選択します。
...
| 変更後  | 比較する属性またはイベント **選択した日付の後** ある必要があります）。 選択した日付は、カスタムの指定日でも、日、週、月、または年前の範囲で選択した日付でもかまいません。 | ○ | ![ 使用されている「After」時間制約の例。](../images/ui/segment-builder/time-constraints/after.png){width="100" zoomable="yes"} |
...
![ コンテナを使用してブール論理を組み合わせ、包含/除外論理を使用する方法を示す画像 ](/help/segmentation/images/ui/segment-builder/mixed-boolean-container.png)
...
![ コンテナを使用してイベントを順序付ける方法を示す画像 ](/help/segmentation/images/ui/segment-builder/event-sequence-container.png)
...
セグメント定義の結合ポリシーを選択するには、「**[!UICONTROL Fields]**」タブの歯車アイコンを選択し、使用する結合ポリシーを **[!UICONTROL Merge Policy]** ドロップダウンメニューで選択します。
...
また、セグメント定義の作成が完了したら、その評価方法を変更することもできます。 評価方法を「Edge」または「ストリーミング」から「バッチ」に変更すると **Edgeまたはストリーミングに戻すことはで** ません。 評価方法の変更は、ポップオーバーで **を選択した場合に** のみ **[!UICONTROL Save]** 有効になります。 ダイアログをキャンセルすると、元の評価方法が **維持** されます。
