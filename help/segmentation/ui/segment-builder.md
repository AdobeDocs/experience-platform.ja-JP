...
>title="外部オーディエンス"
>abstract="「オーディエンス」タブを使用して読み込んだオーディエンスは、オーディエンスポータルを通じて自動的に表示されるようになりました。Audience Manager、Customer Journey Analytics、Segment Match、他のカスタム統合から取得したオーディエンスは、最初にセグメントビルダーで設定しなくてもすぐに使用できます。2025年9月1日（PT）以降、すべてのオーディエンスは統合検索を通じてのみ取得され、以前のワークフローはサポートされなくなります。"
...
>title="オーディエンスのプロパティ"
>abstract="「オーディエンスプロパティ」セクションには、結果オーディエンスの推定サイズが表示され、プロファイルの合計数に対する適格なプロファイルの数が表示されます。 これにより、オーディエンスそのものを作成する前に、必要に応じてオーディエンスを調整できます。"
...
>title="認定プロファイル数"
>abstract="認定プロファイルは、オーディエンスのルールに一致する実際のプロファイル数を示します。 この数は、セグメント評価ジョブの実行後、24時間ごとに更新されます。"

>[!CONTEXTUALHELP]
>id="platform_segments_createsegment_segmentbuilder_estimatedprofiles"
>title="推定プロファイル数"
>abstract="推定プロファイルは、サンプルジョブに基づいて、オーディエンスのルールに適合するプロファイルの概算数を示します。"
...
この数は、次の 2 つのシナリオで更新されます。

1. 顧客データの変化が 3% を超えているか、最後のサンプルジョブが 3 日を超えています。
2. オーディエンスのルールが変更または削除されました。
