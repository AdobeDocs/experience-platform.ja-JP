...
セグメント定義の作成が完了したら、セグメント定義の評価方法を変更することもできます。 評価方法を Edge または Streaming から Batch に変更した場合は、 **not** を Edge または Streaming に戻すことができます。 評価方法の変更は次のようになります。 **のみ** 選択後に有効になる **[!UICONTROL 保存]** をポップオーバーに追加します。 ダイアログをキャンセルすると、 **維持する** 元の評価方法。
