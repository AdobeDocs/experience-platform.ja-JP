...
これらの構築要素は、[!DNL Segment Builder] ワークスペースの左側の **[!UICONTROL フィールド]** セクションに表示されます。 **** フィールドには、主要な構築ブロックごとのタブが含まれます。「[!UICONTROL 属性]」、「[!UICONTROL イベント]」および「[!UICONTROL オーディエンス]」の 3 つのフィールドがあります。
...
[!DNL Segment Builder] の集計は、データ型が数値（倍精度または整数）の XDM 属性のグループに対する計算です。 セグメントビルダーでサポートされる 4 つの集計関数は、SUM、AVERAGE、MIN および MAX です。
