...
>[!NOTE]
>
>次の場合： **ではない** コンポジションにタイトルを付けると、「コンポジション」というタイトルに続き、デフォルトで作成日と時刻が表示されます。 さらに、各組成 **が** 独自の一意の名前を持ちます。
...
>
>さらに、のみ **公開日** セグメントビルダーを使用して作成したオーディエンスを使用できます。 オーディエンスコンポジションを使用して作成されたオーディエンスと、外部で生成されたオーディエンスは次のとおりです **ではない** 使用可能。
...
>[!IMPORTANT]
>
>のみ **公開日** セグメントビルダーを使用して作成したオーディエンスを使用できます。 オーディエンスコンポジションを使用して作成されたオーディエンスと、外部で生成されたオーディエンスは次のとおりです **ではない** 使用可能。
...
>[!IMPORTANT]
>
>属性別に除外する場合は、以下のみを指定できます **1** 除外する値。 コンマやセミコロンなどの任意の種類の区切り記号を使用すると、その正確な値のみが除外されます。 例えば、値をに設定します `red, blue` は次の用語を除外する結果となります `red, blue` 属性から取得しますが、は取得しません **ではない** その結果、どちらかの用語が除外される `red` または `blue`.
...
さらに、を有効にすることで、返されるプロファイルの数を制限できます。 **[!UICONTROL プロファイル制限を追加]** 切り替えます。 この切替スイッチが有効になっている場合は、内で返されるプロファイルの最大数を設定できます **[!UICONTROL 含まれるプロファイル]** フィールド。
...
**[!UICONTROL 分割]**&#x200B;ブロックタイプを使用すると、新しいオーディエンスを様々なサブオーディエンスに分割できます。オーディエンスは、割合に基づいて分割することも、属性別に分割することもできます。 オーディエンスをサブオーディエンスに分割する場合、この分割は次のようになります **持続**. つまり、プロファイルは、各評価で同じサブオーディエンスにあります。
