...
オーディエンスポータルを開くには、 **[!UICONTROL 参照]** 「セグメント化」セクション内のタブです。
...
![「評価の概要」ボタンがハイライト表示されています。](../images/ui/audience-portal/browse-audience-evaluation-summary.png)
...
![オーディエンス評価の円グラフがハイライト表示され、バッチセグメント化評価時間も表示されます。](../images/ui/audience-portal/evaluation-summary.png)

### Customize {#customize}

次を選択して、オーディエンスポータルにフィールドを追加できます ![フィルター属性アイコン](../images/ui/audience-portal/filter-attribute.png). 追加のフィールドには、ライフサイクルステータス、更新頻度、最終更新者、説明、作成者、アクセスラベルなどがあります。
...
表示するフィールドを選択した後で、表示される列の幅を変更することもできます。 列の間の領域をドラッグするか、 ![矢印アイコン](../images/ui/audience-portal/arrow-icon.png) サイズを変更する列の **[!UICONTROL 列のサイズ変更]**.
...
![一括アクションに使用できるオプションがハイライト表示されています。](../images/ui/audience-portal/bulk-actions.png)
...
![オーディエンスポータルで、「すべてのオーディエンスをスケジュール」の切替スイッチがハイライト表示されます。](../images/ui/audience-portal/browse-audiences-scheduled.png)
...
>
>さらに、外部で生成されたオーディエンスに機密情報や医療関連の情報が含まれている場合は、 **が** 任意の宛先に対してアクティブ化する前に、必要なデータ使用ラベルを適用します。 外部で生成されたオーディエンスの変数は、リアルタイム顧客プロファイル内ではなくデータレイクに保存されるので、次のことができるようになります **ではない** csv ファイル内に同意データを含めます。 データ使用ラベルの適用について詳しくは、のドキュメントを参照してください。 [ラベルの管理](../../access-control/abac/ui/labels.md).
...
Audience Portal に関するよくある質問については、 [よくある質問](../faq.md).
