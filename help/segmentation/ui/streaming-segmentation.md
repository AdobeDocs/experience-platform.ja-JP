...
>[!NOTE]
>
>ストリーミングセグメント化は、ストリーミングソースを使用して取り込まれたすべてのデータで機能します。 バッチベースのソースを使用して取り込まれたデータは、ストリーミングのセグメント化の対象となっている場合でも、毎晩評価されます。
>
...
特に、 **[!UICONTROL 合計選定]** 指標が表示され、このセグメントのバッチおよびストリーミングの評価に基づいて、対象オーディエンスの合計数が示されます。

の下には、ストリーミング評価方法を使用して過去 24 時間に更新された新しいオーディエンスの数を示す折れ線グラフが表示されます。 ドロップダウンを調整して、過去 24 時間、先週または過去 30 日間を表示できます。 この **[!UICONTROL 新しいオーディエンスが更新されました]** 指標は、選択した時間範囲でのオーディエンスサイズの変化に基づきます。この変化は、ストリーミングのセグメント化によって評価されます。 この指標には、日別セグメントバッチ評価の合計認定オーディエンスは含まれません。

>[!NOTE]
>
>セグメントは、ステータスがない状態から適合済み状態に移行した場合、または出口から適合済みに移行した場合に認定されると見なされます。 セグメントが適合済みから終了、または既存から終了に移動する場合、セグメントは未適合と見なされます。
>
>これらのステータスの詳細については、 [セグメントの概要](./overview.md#browse).

![](../images/ui/streaming-segmentation/monitoring-streaming-segment-graph.png)

最後のセグメント評価に関する追加情報は、の横の情報バブルを選択すると表示されます。 **[!UICONTROL 合計選定]**.
