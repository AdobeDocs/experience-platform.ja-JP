...
>title="チャーン"
>abstract="チャーンは、セグメントジョブが最後に実行された時点と比較して、オーディエンス内で変化しているプロファイルの割合を表します。"
...
>abstract="毎日スケジュールされている更新に、バッチセグメント化を使用して評価されたすべてのオーディエンスを含めることができるようにします。 スケジュールされている更新からすべてのオーディエンスを削除するには、無効にします。"

「」を選択します **[!UICONTROL 参照]** tab キーを押すと、組織のすべてのオーディエンスのリストが表示されます。 この表示には、プロファイル数、接触チャネル、作成日、最終更新日、タグ、分類など、オーディエンスに関する情報が一覧表示されます。
...
| アクション | 接触チャネル | 説明 |
| ------ | ------- | ----------- |
| [!UICONTROL 編集] | セグメント化サービス | セグメントビルダーを開いてオーディエンスを編集します。 オーディエンスが API を通じて作成された場合、 **ではない** セグメントビルダーを使用して編集できる。 セグメントビルダーの使用について詳しくは、[セグメントビルダー UI ガイド](./segment-builder.md)を参照してください。 |
...
| [!UICONTROL 削除] | オーディエンス構成、カスタムアップロード、セグメント化サービス | 選択したオーディエンスを削除します。 ダウンストリームの宛先で使用されるオーディエンス、または他のオーディエンスの依存オーディエンス **できません** 削除されます。 オーディエンスの削除について詳しくは、を参照してください。 [セグメント化に関する FAQ](../faq.md#lifecycle-states). |
| [!UICONTROL パッケージに追加] | オーディエンス構成、カスタムアップロード、セグメント化サービス | サンドボックス間でオーディエンスを移動します。 この機能について詳しくは、を参照してください。 [サンドボックスツールガイド](../../sandboxes/ui/sandbox-tooling.md). |

>[!IMPORTANT]
>
>オーディエンスを削除する前に、オーディエンスが **ではない** アカウントベースのオーディエンスのコンポーネントとして使用されるか、Adobe Journey Optimizerで使用されます。
...
選択 **[!UICONTROL オーディエンスを読み込み]** では、外部で生成されたオーディエンスを読み込むことができます。 オーディエンスのインポートについて詳しくは、の節を参照してください。 [ユーザーガイドのオーディエンスの読み込み](#import-audience).
...
以下を選択できます。 **[!UICONTROL 評価概要]** オーディエンスの評価の概要を示す円グラフを表示します。

![「評価の概要」ボタンがハイライト表示されています。](../images/ui/overview/browse-audience-evaluation-summary.png)
...
![オーディエンス評価の円グラフがハイライト表示され、バッチセグメント化評価時間も表示されます。](../images/ui/overview/evaluation-summary.png)

### Customize {#customize}

にフィールドを追加できます [!UICONTROL 参照] 選択してページ ![フィルター属性アイコン](../images/ui/overview/filter-attribute.png). 追加のフィールドには、ライフサイクルステータス、更新頻度、最終更新者、説明、作成者、アクセスラベルなどがあります。
...
表示するフィールドを選択した後で、表示される列の幅を変更することもできます。 列の間の領域をドラッグするか、 ![矢印アイコン](../images/ui/overview/arrow-icon.png) サイズを変更する列の **[!UICONTROL 列のサイズ変更]**.

![「列をサイズ変更」ボタンがハイライト表示されている様子](../images/ui/overview/browse-audience-resize-column.png)
...
| [!UICONTROL ライフサイクルステータス] | オーディエンスのライフサイクルステータスに基づいてフィルタリングできます。使用可能なオプションは次のとおりです [!UICONTROL 削除済み], [!UICONTROL ドラフト], [!UICONTROL Inactive]、および [!UICONTROL 公開済み]. |
| [!UICONTROL 更新頻度] | オーディエンスの更新頻度（評価方法）に基づいてフィルタリングできます。 利用可能なオプションには、[!UICONTROL スケジュール済み]、[!UICONTROL 連続]、および[!UICONTROL オンデマンド]があります。 |
...
さらに、最大 25 種類のオーディエンスを選択し、これらのオーディエンスに対して様々なアクションを実行できます。 次のようなアクションがあります [フォルダーへの移動](#folders), [タグの編集または適用](#tags), [アクセスラベルの適用](../../access-control/abac/ui/labels.md)、および [削除中](#browse).

![一括アクションに使用できるオプションがハイライト表示されています。](../images/ui/overview/bulk-actions.png)

これらのオーディエンスに一括アクションを適用すると、次の条件が適用されます。

- あなた **可能** 別のページからオーディエンスを選択します。
- あなた **できません** 宛先のアクティベーションで使用されているオーディエンスを削除します。
- フィルターを選択すると、選択したオーディエンスが対象になります **遺言** リセット。
...
>[!IMPORTANT]
>
>外部で生成されたオーディエンスを読み込むには **が** 次の権限があります。 [!UICONTROL セグメントの表示], [!UICONTROL セグメントの管理]、および [!UICONTROL オーディエンスを読み込み]. これらの権限について詳しくは、 [アクセス制御の概要](../../access-control/home.md#permissions).
...
外部で生成されたオーディエンスを読み込む場合は、いずれかの列をプライマリ ID フィールドとして選択し、名前空間値を指定する必要があります。 残りのフィールドはすべて考慮されます **ペイロード属性**. 次の属性が考慮されます **耐久性のない**&#x200B;これらは、パーソナライゼーションの目的でのみ、このオーディエンスに関連付けられるので、次のようになります **ではない** がプロファイルに接続しました。

![[!UICONTROL オーディエンスの詳細]ページが表示されます。](../images/ui/overview/import-audience-audience-details.png)

また、オプションで、外部で生成されたオーディエンスに詳細を追加することもできます。例えば、ID を付与したり、結合ポリシーを定義したり、列データタイプを編集したりすることができます。

>[!NOTE]
>
>カスタムの外部オーディエンス ID を使用する場合は、次のガイドラインに従う必要があります。
>
> - 0.43188884 **が** 文字（a ～ z または A ～ Z）、アンダースコア（_）、またはドル記号（$）で始めます。
...
>
>さらに、外部で生成されたオーディエンスに機密情報や医療関連の情報が含まれている場合は、 **が** 任意の宛先に対してアクティブ化する前に、必要なデータ使用ラベルを適用します。 データ使用ラベルの適用について詳しくは、のドキュメントを参照してください。 [ラベルの管理](../../access-control/abac/ui/labels.md).
