...
description: エッジセグメント化を使用して、Platform のセグメント定義を即座にエッジ上で評価し、同じページや次のページのパーソナライゼーションの使用例を可能にする方法を説明します。
...
- セグメント定義には、単一のイベントと `inSegment` イベントの組み合わせが含まれています。
   - ただし、セグメント定義が `inSegment` イベントはプロファイルのみ、セグメント定義 **遺言** をエッジセグメント化に対して有効にする。

## 次の手順

このガイドでは、Adobe Experience Platformでのエッジセグメント化を使用してセグメント定義を評価する方法について説明します。 Experience Platform ユーザーインターフェイスの使用方法について詳しくは、[セグメント化ユーザーガイド](./overview.md)を参照してください。 Experience PlatformAPI を使用して同様のアクションを実行し、セグメント定義を操作する方法については、 [エッジセグメント化 API ガイド](../api/edge-segmentation.md).

## 付録

次の節では、エッジセグメント化に関するよくある質問を一覧表示します。

### Edge ネットワーク上でセグメント定義を使用できるまで、どれくらいかかりますか？

Edge ネットワークでセグメント定義を使用できるようになるまで、最大 1 時間かかります。
