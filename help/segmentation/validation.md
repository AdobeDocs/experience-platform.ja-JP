...
>title="クエリ効率アラート"
>abstract="オーディエンスに含まれるクエリが多すぎるので、論理的に不必要に複雑になります。 続行する前にオーディエンスの定義を簡略化してください。"
...
### [!BADGE &#x200B; 重要な検証 &#x200B;]{type=Negative} シーケンシャル・イベントの複雑性 {#sequential-event-complexity}

>[!CONTEXTUALHELP]
>id="platform_segmentation_segmentbuilder_chaincountcheck"
>title="イベント シーケンス制限"
>abstract="オーディエンスに含まれる順次イベントが多すぎます。 オーディエンス定義内で連続するイベントを最大 6 つまで持つことができます。 続行する前に、オーディエンス定義から順次イベントをいくつか削除してください。"
...
### [!BADGE &#x200B; パフォーマンスの最適化 &#x200B;]{type=Caution} 集計数 {#aggregated-count}

>[!CONTEXTUALHELP]
>id="platform_segmentation_segmentbuilder_countaggregationcheck"
>title="カウント フィルターの警告"
>abstract="オーディエンスに含まれる集計イベントが多すぎます。 オーディエンス内で使用できる集計イベントは最大 3 つです。 パフォーマンスの問題を回避するには、オーディエンス定義から一部の集計イベントを削除する必要があります。"
...
>title="ネストされたデータの警告"
>abstract="オーディエンスにネストされたデータレイヤーが多すぎます。 オーディエンス内で最大 2 つのデータレイヤーを使用する必要があります。 パフォーマンスの問題を回避するには、オーディエンス定義を統合する必要があります。"
...
>title="オーディエンスサイズの警告"
>abstract="オーディエンスの記述が大きすぎます。 サンドボックス内の合計プロファイルの 30% 以上を選定するオーディエンス定義を記述することは避けてください。 パフォーマンスの問題を回避するには、オーディエンスの定義を厳しくする必要があります。"
