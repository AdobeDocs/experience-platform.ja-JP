...
バッチセグメントを使用して評価されたセグメントは、24時間ごとに評価されます。 ただし、既存のセグメントの場合、インクリメンタルセグメント化では、セグメントの評価を最大1時間行うまで、バッチセグメント化を使用して行います。 新しく、または最近変更したセグメントは、次の完全なバッチセグメントジョブが実行され、増分セグメント化が利用できるようになるまで待つ必要があります。
