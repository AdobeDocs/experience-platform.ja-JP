...
| `tags` | 文字列の配列 | 外部オーディエンスに適用するタグ。 タグの配列を追加する場合は、**を使用します** 必ず `tagId`。 タグについて詳しくは、[&#x200B; タグ管理ガイド &#x200B;](/help/administrative-tags/ui/managing-tags.md) を参照してください。 |
...
>次のエンドポイントを使用するには、外部オーディエンスの `audienceId` が必要です。 `audienceId` エンドポイントへの呼び出しが成功すると、`GET /external-audiences/operations/{OPERATION_ID}` を取得できます。
>
>また、以前に取り込まれたオーディエンスのデータを、このエンドポイントを使用して更新することもできます。
...
オーディエンス ID を指定した状態で次のエンドポイントに対して POST リクエストを実行することで、外部オーディエンスのデータ有効期限を延長できます。

データの有効期限は、取り込み時に設定された元の期間だけ延長されます。 期間を指定しなかった場合は、デフォルトの 30 日間の延長が適用されます。 データの有効期限を延長すると、オーディエンスは、最後に成功した取り込みのデータで再度取り込まれます。
...
+++ 外部オーディエンスのデータ有効期限を拡張するリクエストのサンプル。
...
応答が成功すると、HTTP ステータス 200 が、オーディエンスの詳細と共に返されます。

+++ データの有効期限を延長する際の応答のサンプル。
