...
>[!NOTE]
>
>プレビュージョブの作成時に、推定ジョブが自動的に作成されます。 これらの 2 つのジョブは同じ ID を共有します。
...
| `graphType` | クラスターの取得元のグラフタイプです。 サポートされている値は次のとおりです。 `none` （ID のステッチを実行しない）および `pdg` （は、非公開の ID グラフに基づいて ID ステッチを実行します）。 |
...
| パラメーター | 説明 |
| --------- | ----------- |
| `{PREVIEW_ID}` | The `previewId` 取得するプレビューの値。 |
...
| パラメーター | 説明 |
| --------- | ----------- |
| `{PREVIEW_ID}` | 見積ジョブは、プレビュージョブが作成された場合にのみトリガーされ、2 つのジョブは参照用に同じ ID 値を共有します。 特に、これは `previewId` プレビュージョブの作成時に返される値。 |
