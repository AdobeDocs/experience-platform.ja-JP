...
- [[!DNL Segmentation]](../home.md):セグメント定義や他の外部ソースを使用してオーディエンスを作成する機能を [!DNL Real-Time Customer Profile] データ。
...
セグメント定義が [上記のストリーミングセグメントタイプ](#query-types).
...
### 「過去 X 日間」の下の数がセグメント定義の詳細セクション内でゼロのままの間、「合計認定」セグメント定義の数が増加し続けるのはなぜですか？
...
「過去 X 日間」の下にある数値は、ストリーミングセグメント化の対象となるオーディエンスの数&#x200B;**のみ**&#x200B;を表し、データをシステムにストリーミングしそのストリーミング定義に照合してカウントする場合に&#x200B;**のみ**&#x200B;増加します。この値は **のみ** ストリーミングセグメントの定義について説明します。 その結果、この値 **may** バッチセグメント定義の場合は 0 と表示されます。
