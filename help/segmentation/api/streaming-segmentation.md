...
- [[!DNL Segmentation]](../home.md)：のセグメント定義やその他の外部ソースを使用してオーディエンスを作成できます [!DNL Real-Time Customer Profile] データ。
...
に対してGETリクエストを行うことで、組織内でストリーミングセグメント化が有効になっているすべてのセグメント定義のリストを取得できます `/segment/definitions` エンドポイント。

**API 形式**

ストリーミングが有効なセグメント定義を取得するには、クエリパラメーターを含める必要があります `evaluationInfo.continuous.enabled=true` をリクエストパスに追加します。
...
セグメント定義がと一致する場合、自動的にセグメント定義のストリーミングが有効になります [上記のストリーミングセグメントタイプ](#query-types).
...
>[!NOTE]
>
>これは、標準の「セグメント定義の作成」リクエストです。 セグメント定義の作成について詳しくは、のチュートリアルを参照してください。 [セグメント定義の作成](../tutorials/create-a-segment.md).
...
| `properties.segments` | **（次の場合に必須： `type` 等しい `batch_segmentation`）** 使用 `["*"]` すべてのセグメント定義が含まれていることを確認します。 |
...
Adobe Experience Platform ユーザーインターフェイスを使用して同様のアクションを実行し、セグメント定義を操作する方法については、 [セグメントビルダーユーザーガイド](../ui/segment-builder.md).
...
ほとんどのインスタンスでは、ストリーミングセグメント化の選定解除はリアルタイムで発生します。ただし、セグメントのセグメントを使用するストリーミングセグメント定義では、次のことが可能です **ではない** リアルタイムで選定解除を行い、24 時間後には選定解除を行います。
...
セグメント定義は、クエリタイプとイベント履歴期間の組み合わせに基づいて、バッチまたはストリーミングセグメント化として定義されます。 ストリーミングセグメントとして評価されるセグメント定義のリストは、にあります。 [ストリーミングセグメント化クエリタイプ セクション](#query-types).
...
「過去 X 日間」の下にある数値は、ストリーミングセグメント化の対象となるオーディエンスの数&#x200B;**のみ**&#x200B;を表し、データをシステムにストリーミングしそのストリーミング定義に照合してカウントする場合に&#x200B;**のみ**&#x200B;増加します。この値 **のみ** ストリーミングセグメント定義で表示されます。 そのため、この値は **年 5 月** バッチセグメント定義の場合は 0 と表示されます。

つまり、「過去 X 日間」の下にある数値がゼロで、折れ線グラフにもゼロと表示されている場合は、 **ではない** そのセグメント定義に適合するプロファイルをシステムにストリーミングします。
