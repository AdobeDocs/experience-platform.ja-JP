...
新しいセグメントジョブを作成するには、 `/segment/jobs` エンドポイントを追加し、新しいオーディエンスを作成するセグメント定義の ID を本文に含めます。
...
正常な応答は、HTTP ステータス 200 と、新しく作成されたセグメントジョブに関する情報を返します。
...
特定のセグメントジョブに関する詳細な情報を取得するには、に対してGETリクエストを実行します `/segment/jobs` エンドポイントを検索し、リクエストパスで取得するセグメントジョブの ID を指定します。
...
複数のセグメントジョブに関する詳細な情報を取得するには、 `/segment/jobs/bulk-get` エンドポイントと  `id` リクエスト本文のセグメントジョブの値。
...
>[!NOTE]
>
>次の応答はスペースを節約するために切り捨てられ、各セグメントジョブの詳細の一部のみが表示されました。 完全な応答には、リクエストされたセグメントジョブの完全な詳細がリストされます。
...
特定のセグメントジョブを削除するには、 `/segment/jobs` エンドポイントを探し、削除するセグメントジョブの ID をリクエストパスで指定します。

>[!NOTE]
>
>削除リクエストに対する API 応答が直ちに発生します。 ただし、セグメントジョブの実際の削除は非同期的におこなわれます。 つまり、セグメントジョブに対して削除リクエストを実行するタイミングと適用するタイミングに時間差があります。
