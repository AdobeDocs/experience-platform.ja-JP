...
| 非アクティブ | **非アクティブ** 状態のオーディエンスとは、現在使用されていないオーディエンスです。 Experience Platform内には引き続き存在しますが、ドラフトまたは公開済みとしてマークされるまで使用できま **ん**。 | いいえ。ただし、表示できます。 | × | × | 更新はありません。 | Experience Platformによる評価または更新は行われなくなりました。 | × | ○ |
...
セグメントビルダーを使用して作成したオーディエンスの場合、セグメントビルダーで「[!UICONTROL Save as draft]」を選択すると、オーディエンスをドラフト状態に設定できます。
...
セグメントビルダーまたはオーディエンスコンポジションを使用して作成されたオーディエンスの場合、それぞれの UI で「[!UICONTROL Publish]」を選択することで、オーディエンスを公開済みの状態に設定できます。

外部で作成されたオーディエンスは、自動的に公開済みに設定されます。

### オーディエンスを非アクティブ状態にするにはどうすればよいですか？

オーディエンスポータルでクイックアクションメニューを開き、「[!UICONTROL Deactivate]」を選択すると、公開済みのオーディエンスを非アクティブ状態にすることができます。
...
オーディエンスを削除状態にすることができます。これを行うには、オーディエンスポータルでクイックアクションメニューを開き、「[!UICONTROL Delete]」を選択します。
...
はい。オーディエンスが現在「ドラフト」状態の場合、ドラフト状態または公開済み状態のオーディエンスを参照できます。 ただし、このオーディエンスを公開するには、他の親オーディエンスを公開する **必須** です。
...
はい。Adobe Journey Optimizer キャンペーン、Real-Time CDPジャーニー、Adobe Journey Optimizer宛先で、オーディエンスコンポジションのオーディエンスを使用できます。
...
1. **常に**[!UICONTROL Audience] ブロックから開始して、開始アクティビティを選択します。 最大 **1** 個の [!UICONTROL Audience] ブロックを持つことができます。
2. オプションで、[!UICONTROL Exclude] ブロックの後に [!UICONTROL Audience] ブロックを追加できます。
3. オプションで、[!UICONTROL Enrich] ブロックの後に [!UICONTROL Exclude] ブロックを追加できます。 コンポジションごとに **1** の [!UICONTROL Enrich] ブロックのみを使用できます。
4. オプションで、[!UICONTROL Rank] または [!UICONTROL Split] ブロックを追加できます。 使用できるのは、構成ごとにこれらのブロックの 1 つ&#x200B;**だけ**&#x200B;です。
5. オーディエンスを保存する **は** 常に） [!UICONTROL Save] ブロックで終了します。
...
プロファイルのオーディエンスのメンバーシップを確認するには、調べたいプロファイルの詳細ページにアクセスします。**[!UICONTROL Attributes]**、**[!UICONTROL View JSON]** の順に選択すると、`segmentMembership` オブジェクトにオーディエンスの ID が含まれていることを確認できます。
...
ストリーミングセグメント化の不適格は、オーディエンスの構成に応じて発生します。 イベントベースのオーディエンスの場合、ルックバックウィンドウの有効期限が切れると、リアルタイムで不適格となります。 プロファイルベースのオーディエンスまたはプロファイル属性を使用するオーディエンスの場合、ストリーミングソースを介して、または毎日のバッチ評価ジョブ中にプロファイル属性値が変更されると、不適格が発生します。
