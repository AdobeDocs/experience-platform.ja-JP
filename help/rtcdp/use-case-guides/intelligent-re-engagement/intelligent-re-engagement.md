...
インテリジェントな再エンゲージメントにより、カスタマイズされたクロスチャネルドリップキャンペーンを設定して、顧客に特定のアクションを実行するよう促すことができます。 このナッジキャンペーンは、意図的な E メール、SMS、有料広告の提供を行う送信顧客を含む、限られた時間の操作を目的としています。 顧客が適切なアクションを実行すると、すぐに微調整キャンペーンが終了します。
...
* [ADOBE REAL-TIME CUSTOMER DATA PLATFORM(REAL-TIME CDP)](https://experienceleague.adobe.com/docs/platform-learn/tutorials/rtcdp/understanding-the-real-time-customer-data-platform.html)  — 複数のデータソースをまたいでデータを集計し、キャンペーンに燃料を供給します。 次に、このデータを使用して、キャンペーンオーディエンスを作成し、E メールや Web プロモーションタイルで使用するパーソナライズされたデータ要素（名前やアカウント関連の情報など）を表示します。 また、CDP は、(Adobe Targetを介して )E メールと Web にまたがってオーディエンスをアクティブ化するためにも使用されます。
...
3 つの再エンゲージメントジャーニーが作成されています。

>[!BEGINTABS]

>[!TAB 再エンゲージメントジャーニー]

再エンゲージメントのジャーニーは、Web サイトとアプリの両方で、廃止された製品の閲覧をターゲットにします。 このジャーニーは、製品が購入も買い物かごに追加もされずに表示された場合にトリガーされます。 過去 24 時間以内にリストが追加されない場合、ブランドエンゲージメントは 3 日後にトリガーされます。

![顧客インテリジェントな再エンゲージメントジャーニーの概要を視覚的に説明します。](../intelligent-re-engagement/images/re-engagement-journey.png)

1. データは、Edge Network（推奨される方法）を使用して、Web SDK/Mobile SDK/Edge Network API 取り込みに集計されます。
...
この放棄された買い物かごジャーニーは、買い物かごに配置されたが、Web サイトとアプリの両方で購入されなかった製品をターゲットにします。 有料メディアキャンペーンの開始と停止に使用します
...
2. As a **顧客**&#x200B;を使用する場合、に対してマークされたデータセットを作成します [!UICONTROL プロファイル].
3. As a **顧客**&#x200B;では、プロファイルをReal-Time CDPに読み込み、ガバナンスポリシーを構築して、責任を持って使用するようにします。
4. As a **顧客**&#x200B;を使用する場合、プロファイルのリストから焦点を絞ったオーディエンスを作成し、 **ユーザー** は買い物かごに品目を配置しましたが、まだ購入を完了していません。 The **[!UICONTROL 買い物かごに追加]** イベントは、30 分待って購入を確認するタイマーを開始します。 購入が行われていない場合、 **ユーザー** が **[!UICONTROL 買い物かごを放棄]** オーディエンス。
...
この注文確認ジャーニーは、Web サイトとアプリの両方で製品の購入をターゲットにします。
...
5. As a **顧客**&#x200B;に設定した場合、確認ジャーニーがAdobe Journey Optimizerに作成されます。
...
上記の概要の各手順を完了するには、以下の節を参照してください。この節には、その他のドキュメントへのリンクが含まれています。
...
Experience Data Model(XDM) のリソースは、 [!UICONTROL スキーマ] Adobe Experience Platformのワークスペース アドビから提供されるコアリソースを表示および調査し、組織のカスタムリソースおよびスキーマを作成できます。

スキーマを作成するには、次の手順を実行します。

1. に移動します。 **[!UICONTROL データ管理]** > **[!UICONTROL スキーマ]** を選択し、 **[!UICONTROL スキーマを作成]**.
2. 選択 **[!UICONTROL XDM 個人プロファイル]/[!UICONTROL XDM ExperienceEvent]**.
3. に移動します。 **[!UICONTROL フィールドグループ]** を選択し、 **[!UICONTROL 追加]**.
4. 検索ボックスを使用して、フィールドグループを検索して選択し、「 」を選択します。 **[!UICONTROL フィールドグループを追加]**.
5. スキーマに名前を付け、必要に応じて説明を追加します。
6. 「**[!UICONTROL 保存]**」を選択します。

![スキーマを作成する手順の記録。](../intelligent-re-engagement/images/create-a-schema.gif)
...
顧客属性スキーマは、 [!UICONTROL XDM 個人プロファイル] スキーマ。次のフィールドグループが含まれます。
...
![フィールドグループのリストを強調表示する顧客属性スキーマ。](../intelligent-re-engagement/images/customer-attributes.png)

#### 顧客デジタルトランザクションスキーマのフィールドグループ要件

顧客のデジタルトランザクションスキーマは、 [!UICONTROL XDM ExperienceEvent] スキーマ。次のフィールドグループが含まれます。
...
![フィールドグループのリストを強調表示する顧客のデジタルトランザクションスキーマ。](../intelligent-re-engagement/images/customer-digital-transactions.png)

#### 顧客オフライントランザクションスキーマのフィールドグループ要件

顧客オフライントランザクションスキーマは [!UICONTROL XDM ExperienceEvent] スキーマ。次のフィールドグループが含まれます。
...
![フィールドグループのリストを強調表示する顧客オフライントランザクションスキーマ。](../intelligent-re-engagement/images/customer-offline-transactions.png)

#### AdobeWeb コネクタスキーマのフィールドグループ要件
...
データセットは、スキーマ（列）とフィールド（行）で構成されるデータコレクション（通常はテーブル）を格納し管理するための構造です。インテリジェントな再エンゲージメントジャーニーの場合、各スキーマに 1 つのデータセットがあります。

スキーマからデータセットを作成するには、次の手順を実行します。

1. に移動します。 **[!UICONTROL データ管理]** > **[!UICONTROL データセット]** を選択し、 **[!UICONTROL データセットを作成]**.
2. 「**[!UICONTROL スキーマからデータセットを作成]**」をクリックします。
3. 作成した関連する再エンゲージメントスキーマを選択します。
4. データセットに名前を付け、必要に応じて説明を追加します。
5. 「**[!UICONTROL 完了]**」を選択します。

![スキーマからデータセットを作成する手順の記録。](../intelligent-re-engagement/images/dataset-from-schema.gif)
...
![プロファイルのデータセットを有効にします。](../intelligent-re-engagement/images/enable-dataset-for-profile.png)
...
再エンゲージメントジャーニーを設定する際には、次の同意ポリシーを考慮し、使用する必要があります。
...
個人の電子メールアドレスは、デバイスではなく個人の識別に使用したり、個人への連絡に使用したりできる、個人の識別に直接関わるデータとして使用されます。

* personalEmail.address = I1

#### マーケティングポリシー

再エンゲージメントジャーニーに対する追加のマーケティングポリシーはありませんが、以下を必要に応じて考慮する必要があります。
...
1. に移動します。 **[!UICONTROL 顧客]** > **[!UICONTROL オーディエンス]** を選択し、 **[!UICONTROL オーディエンスを作成]**.
2. 選択 **[!UICONTROL ルールを作成]** を選択し、 **[!UICONTROL 作成]**.
3. に移動します。 **[!UICONTROL フィールド]** を選択し、 **[!UICONTROL イベント]** タブをクリックします。
4. 検索ボックスを使用してイベントタイプを探し、これをビルダーにドラッグします。 最後に、イベントタイプをドラッグしてイベントルールを追加します。
5. スキーマに名前を付け、必要に応じて説明を追加します。
6. 「**[!UICONTROL 保存]**」を選択します。

![オーディエンスを作成する手順の記録。](../intelligent-re-engagement/images/create-an-audience.gif)

オーディエンスの構築方法について詳しくは、 [Audience Builder UI ガイド](/help/segmentation/ui/segment-builder.md).

#### ブランドの再エンゲージメントジャーニー用のオーディエンスの作成

各再エンゲージメントジャーニーのオーディエンスは、セグメント認定のために、特定のイベントで設定する必要があります。 これらの詳細は、各ジャーニーの対応するタブで以下に示します。
...
![一連のルールを示す再エンゲージメントオーディエンスのスクリーンショット。](../intelligent-re-engagement/images/re-engagement-audience.png)

>[!TAB 放棄された買い物かごのジャーニー]

次のイベントは、製品を買い物かごに追加したが、過去 24 時間に購入を完了しなかった、または買い物かごをクリアしなかったプロファイルに使用されます。

今すぐ 30 分～ 1440 分前に EventType = commerce.productListAdds を含めます。
...
![一連のルールを示す再エンゲージメントオーディエンスのスクリーンショット。](../intelligent-re-engagement/images/abandoned-cart-audience.png)

>[!ENDTABS]

オーディエンスの構築について詳しくは、 [Audience Builder UI ガイド](/help/segmentation/ui/segment-builder.md).
...
各ユースケースが持つことのできる複数のジャーニーに対して、具体的な情報が必要です。 各ジャーニーブランチに必要な特定のデータは、対応するタブの下に表示されます。

>[!BEGINTABS]

>[!TAB 再エンゲージメントジャーニー]

![Adobe Journey Optimizerでのカスタマー再エンゲージメントのジャーニーの概要](../intelligent-re-engagement/images/re-engagement-ajo.png)
...
![Adobe Journey Optimizerで離脱した顧客の買い物かごのジャーニーの概要](../intelligent-re-engagement/images/abandoned-cart-ajo.png)
...
![Adobe Journey Optimizerでの顧客の注文確認のジャーニーの概要](../intelligent-re-engagement/images/order-confirmation-ajo.png)
