...
>[!TIP]
>
>オーディエンスのサイズと複雑さに応じて、オーディエンスの評価とオーディエンスのアクティベーションに要する時間が若干変更される場合があります。
>
>特に、ビジネスプロセスがセグメント化とアクティベーションのタイムリーな完了に依存している場合は、セグメント化の実行時間を監視することをお勧めします。 これにより、ワークフローを引き続きスムーズに実行でき、予期しない変更にプロアクティブに対処できます。
...
### B2B エンティティを含んだオーディエンスのオーディエンスサイズ
...
新しいアーキテクチャでは、「セグメントジョブの作成」エンドポイントと柔軟なオーディエンス評価は*サポートされていません。
