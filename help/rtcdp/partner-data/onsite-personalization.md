...
まず、パートナー ID の ID 名前空間を作成する必要があります。左側のパネルで **[!UICONTROL Customer]**/**[!UICONTROL Identities]** に移動し、画面の右上隅にある「**[!UICONTROL Create identity namespace]**」を選択します。
...
次に、後で web プロパティから収集する時系列データを格納する [!UICONTROL Experience Event] スキーマを作成し、[!UICONTROL XDM ExperienceEvent] をスキーマの基本クラスとして使用していることを確認します。 [Experience Platform の UI を使用したスキーマの作成](/help/xdm/ui/resources/schemas.md#create)を参照してください。
...
データ管理を設定したら、web プロパティにリアルタイムのイベント[データ収集](/help/collection/home.md)を実装する必要があります。リアルタイムイベント呼び出しを収集してAdobeに送り返すには、Real-Time CDP data collection library - [!UICONTROL Web SDK] を使用してプロパティを実装する必要があります。 この項目は、いくつかのデータ収集コンポーネントをまたいだいくつかの個別のタスクで構成されます。
...
まず、画面の右上隅にあるアプリケーションスイッチャーを使用して、「**[!UICONTROL Data Collection]**」セクションに移動します。

>[!TIP]
>
>アプリケーション切り替えボタンで [!UICONTROL Data Collection] が表示されない場合は、システム管理者にアクセス権を要求してください。

![データ収集セクションに移動する、アプリ切り替えボタン。](/help/rtcdp/assets/partner-data/onsite-personalization/app-switcher-data-collection.png)

UI の **[!UICONTROL Data Collection]** セクションは、以下の画像に似ています。
...
データストリームの作成時に、「**[!UICONTROL Event schema]**」フィールドで、以前に作成したスキーマを選択します。
...
次に、プロパティ内に様々な拡張機能をインストールする必要があります。タグプロパティを選択し、「[!UICONTROL Extensions]」セクションに移動します。

![新しいタグプロパティを選択します。](/help/rtcdp/assets/partner-data/onsite-personalization/select-tag-property.png)

[!UICONTROL Core] 拡張機能は既にインストールされています。 次の節で説明するように、さらに 2 つの拡張機能をインストールする必要があります。
...
Web SDKを設定するための画面で、「**[!UICONTROL Datastreams]**」セクションに移動し、使用しているExperience Platform サンドボックスに関する情報を指定します。 次のドロップダウンから、適切なサンドボックスと前の手順で作成したデータストリームを選択します。他のすべての環境に対して同じサンドボックスとデータストリームの値を選択できます。他の設定はそのままにし、「**[!UICONTROL Save]**」を選択します。
...
[Experience Cloud の ID サービス拡張機能](/help/tags/extensions/client/id-service/overview.md)を使用すると、すべての Experience Cloud ソリューションにわたって、訪問者に固有のデバイスベースのファーストパーティ ID を作成できます。拡張機能カタログで **[!UICONTROL ID Service]** を検索して、インストールします。 拡張機能をインストールする際は、すべてのデフォルト設定をそのまま使用します。

![拡張機能カタログ内の ID サービス拡張機能の表示。](/help/rtcdp/assets/partner-data/onsite-personalization/id-service-extension.png)

#### 環境の設定

次に、左側のナビゲーションから **[!UICONTROL Environments]** のセクションに進みます。 この手順では、web サイトを Adobe Edge Network に接続して、訪問者情報をリアルタイムに取得し、配信する必要があります。
...
まず、`partnerData` 要素を設定します。**[!UICONTROL Data Elements]** セクションに移動し、「**[!UICONTROL Create New Data Element]**」を選択します。

![新しいデータ要素の作成.](/help/rtcdp/assets/partner-data/onsite-personalization/create-data-element.gif)

データ要素に `partnerData` という名前を付け、[!UICONTROL extension] の値は [!UICONTROL Core] のままにし、**[!UICONTROL Data Element Type]** を **[!UICONTROL JavaScript Variable]** に設定します。 `partnerData` という名前のフィールドに **[!UICONTROL JavaScript variable name]** と入力し、「**[!UICONTROL Save]**」を選択します。
...
2 つ目のデータ要素を設定するには、新しい変数に `pageVisit` という名前を付け、**[!UICONTROL Extension]** を **[!UICONTROL Adobe Experience Platform]** に設定し、データタイプとして **[!UICONTROL XDM Object]** を選択します。
...
スキーマから、データパートナーに期待する値に対応するサードパーティ属性を選択します。次に、「**[!UICONTROL Provide entire object]**」というラジオボタンを選択します。 データベースに似たアイコンを選択し、前に作成した `partnerData` データ要素を選択します。

#### ルールを設定

**[!UICONTROL Rules]** セクションでは、作成したデータ要素に読み込まれた属性を使用してパーソナライゼーションリクエストをAdobeに送信するように web サイトを設定できます。 詳しくは、[ルールの作成](/help/tags/ui/managing-resources/rules.md)を参照してください。

「**[!UICONTROL Create new Rule]**」を選択します。このルールに **[!UICONTROL Personalize]** という名前を付け、**[!UICONTROL Events]** の横の「+」記号を選択します。 イベントとして「**[!UICONTROL Page Bottom]**」を選択し、保存します。

![ルールのイベントタイプ部分の選択。](/help/rtcdp/assets/partner-data/onsite-personalization/add-events-rule.png)

「**[!UICONTROL Actions]**」の横にある「+」記号を選択します。 拡張機能を **[!UICONTROL Adobe Experience Platform Web SDK]** に更新し、**[!UICONTROL Action Type]** を **[!UICONTROL Send event]** に設定します。

![ルールのアクションタイプの部分の選択。](/help/rtcdp/assets/partner-data/onsite-personalization/add-action-rule.png)

右側の **[!UICONTROL Type]** ドロップダウンセレクターから、イベントタイプとして「`web.webpagedetails.pageViews`」を選択します。
...
アクション設定のリストを下にスクロールし、「**[!UICONTROL Render visual personalization decisions]**」というチェックボックスをオンにします。 これは、Adobe Target またはその他の類似製品を介して提供されたエクスペリエンスを web ページ上で視覚的にレンダリングするために重要です。「**[!UICONTROL Keep Changes]**」を選択し、ルールを **[!UICONTROL Save]** きます。
...
**[!UICONTROL Publishing Flow]** を選択し、次に **[!UICONTROL Add Library]** を選択します。

「**[!UICONTROL Add all Changed Resources]**」を選択し、ライブラリに名前を付け、環境を「**[!UICONTROL Development]**」に設定して「**[!UICONTROL Save & Build to Development]**」を選択します。
