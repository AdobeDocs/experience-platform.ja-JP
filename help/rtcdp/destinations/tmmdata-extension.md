...
title: TMMData拡張
seo-title: TMMData拡張
description: TMMData拡張は、Adobe Real-time Customer DataPlatformの分析の対象です。 拡張機能の詳細については、Adobe Exchangeの拡張機能ページを参照してください。
seo-description: TMMData拡張は、Adobe Real-time Customer DataPlatformの分析先です。 拡張機能の詳細については、Adobe Exchangeの拡張機能ページを参照してください。
...
[!DNL TMMData's] Adobe Marketing Cloudの基盤プラットフォームは、社内/外部データ、オンライン/オフラインデータなど、重要なデータソースにアクセスし、ブレンドするツールをマーケティングチームに提供し、包括的なクロスチャネル分析と、自動キャンペーン設定や他の解析およびBIツールの直接インポートを行います。

[!DNL TMMData] は、アドビのリアルタイム顧客データPlatformのanalytics拡張です。 拡張機能の詳細については、 [Adobe Exchangeの拡張機能ページを参照してください](hhttps://exchange.adobe.com/experiencecloud.details.100148.tmmdata-foundation-platform.html)。
