...
B2B マーケターは、マーケティングfunnelの最上位で複数の課題に直面します。 効果的であるためには、B2B マーケターは、高価値のターゲットに集中できるように、多数の人物を選定するための自動化された方法を必要としています。 選定は、マーケティングコンバージョンだけでなく、最終的な販売結果と連携する必要があります。
...
>[!NOTE]
> 
>アルゴリズムでは、Person`sourceAccountKey.sourceKey` フィ :personComponents ルドグループのフィールドのみが検査されます。
...
個人プロファイルの予測スコアを表示するには、左側のパネルの「顧客」セクションで「**[!UICONTROL Profiles]**」を選択したあと、ID 名前空間と ID 値を入力します。 終了したら、「**[!UICONTROL View]**」を選択します。

次に、リストからプロファイルを選択します。

![&#x200B; 顧客プロファイル &#x200B;](/help/rtcdp/accounts/images/b2b-view-customer-profile.png)

**[!UICONTROL Detail]** ページに、予測スコアが含まれるようになりました。 予測スコアの横にあるグラフアイコンをクリックします。
