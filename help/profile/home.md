...
![リアルタイム顧客プロファイルとAdobe Experience Platformの他のサービスとの関係。 次の図は、プロファイルがAdobe Experience Platformのコアコンポーネントの 1 つであることを示しています。](images/profile-overview/profile-in-platform.png)
...
リアルタイム顧客プロファイルは、 **主要エンティティ**、および様々なサポートエンティティ。 プライマリエンティティは、特性、行動、プロファイルのセグメントメンバーシップで構成されます。 その他のエンティティを使用すると、セグメント化エンジンで、プロファイルのプライマリエンティティ外のデータを利用し、次の情報を含めることができます。

- **ディメンショナルエンティティ**:イベントやプロファイルレコード間で共有される情報のデータモデリングプロセスを簡略化するために使用されるエンティティ。 これは、ルックアップエンティティまたは分類エンティティとも呼ばれます。
- **B2B エンティティ**:プロファイルとビジネス間アカウントおよび商談との関係を記述するエンティティ。

![プロファイルエンティティの構成を説明する図です。](./images/profile-overview/profile-entity-composition.png)
