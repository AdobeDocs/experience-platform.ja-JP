...
Adobe Experience Platform を使用すると、複数のソースからデータを取得し、個々の顧客に対して堅牢なプロファイルを構築できます。に取り込まれたデータ [!DNL Platform] は、に保存されます。 [!DNL Data Lake]データセットがプロファイルに対して有効になっている場合、そのデータはに保存されます [!DNL Real-Time Customer Profile] データストアも。 不要になったデータやエラーで追加されたデータを削除するには、プロファイルストアからデータセットに関連付けられたプロファイルデータを削除する必要が生じる場合があります。 これには、 [!DNL Real-Time Customer Profile] を作成するための API [!DNL Profile] システムジョブ、または `delete request`必要に応じて、変更、監視、削除することもできます。

>[!NOTE]
>
>からデータセットやバッチを削除しようとしている場合 [!DNL Data Lake]にアクセスしてください [Catalog Service の概要](../../catalog/home.md) を参照してください。
...
また、オプションのクエリーパラメーターを使用して、応答で返される削除リクエストのリストをフィルタリングすることもできます。複数のパラメーターを使用するには、アンパサンド（`&`）に設定します。
...
| `_page.next` | 結果のページが追加で存在する場合は、 [参照リクエスト](#view-a-specific-delete-request) （を使用） `"next"` 値が指定されました。 |
| `jobType` | 作成されるジョブのタイプ。この場合、常にが返されます `"DELETE"`. |
| `status` | 削除リクエストのステータス。使用可能な値は次のとおりです `"NEW"`, `"PROCESSING"`, `"COMPLETED"`, `"ERROR"`. |
| `metrics` | 処理されたレコードの数を含むオブジェクト（`"recordsProcessed"`）と、リクエストが処理されている時間（秒）、またはリクエストが完了するまでにかかった時間（`"timeTakenInSec"`）に設定します。 |
...
データセットとそれに関連付けられたすべてのプロファイルデータをプロファイルストアから削除するには、データセット ID をPOSTリクエストの本文に含める必要があります。 この操作により、特定のデータセットのすべてのデータが削除されます。[!DNL Experience Platform] を使用すると、レコードスキーマと時系列スキーマの両方に基づいてデータセットを削除できます。
...
レコードと時系列の動作について詳しくは、 [XDM データの動作に関する節](../../xdm/home.md#data-behaviors) が含まれる [!DNL XDM System] の概要。
...
正常な応答は、新しく作成された削除リクエストの詳細を返します。この詳細には、システムで生成された一意の読み取り専用 ID が含まれます。これは、リクエストを検索し、そのステータスを確認するために使用できます。作成時のリクエストの `"status"` は、処理が開始されるまで `"NEW"` です。この `"batchId"` 応答の値はと一致する必要があります。 `"batchId"` 値はリクエストで送信されます。
...
| `metrics` | 処理されたレコードの数を含む配列（`"recordsProcessed"`）と、リクエストが処理されている時間（秒）、またはリクエストが完了するまでにかかった時間（`"timeTakenInSec"`）に設定します。 |

削除リクエストのステータスが「」になったら `"COMPLETED"` data Access API を使用して削除されたデータにアクセスすることで、データが削除されたことを確認できます。 データアクセス API を使用してデータセットやバッチにアクセスする手順については、[データアクセスのドキュメント](../../data-access/home.md)を参照してください。

## 削除リクエストの削除

[!DNL Experience Platform] では、前のリクエストを削除できます。これは、削除ジョブが完了しなかった場合や処理ステージで停止した場合など、様々な理由で役立つ場合があります。 削除リクエストを削除するには、`/system/jobs` エンドポイントに対して DELETE リクエストを実行し、削除する削除リクエストの ID をリクエストパスに含めます。
