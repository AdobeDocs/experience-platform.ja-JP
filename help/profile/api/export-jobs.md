...
このチュートリアルの以降の手順では、 [!DNL XDM Individual Profile] スキーマを [!DNL Catalog] API また、 [!DNL Platform] ユニオンスキーマを参照するデータセットを作成するユーザーインターフェイス。 UI を使用する手順については、 [オーディエンスのエクスポートに関するこの UI チュートリアル](../../segmentation/tutorials/create-dataset-export-segment.md) しかしここでも当てはまります 完了したら、このチュートリアルに戻り、[新しいエクスポートジョブを開始する](#initiate)手順に進むことができます。
...
また、書き出しジョブのエンドポイントを使用して、 [!DNL Profile] データ。 詳しくは、 [セグメント化 API でのジョブの書き出し](../../segmentation/api/export-jobs.md) を参照してください。
