...
| プロパティ | 説明 |
|---|---|
| `self.href` | このパスは、宛先を直接参照(GET)するために使用され、宛先を更新(PUT)または削除(DELETE)するためにも使用できます。 |
...
組織で投影先が不要になった場合は、エンドポイントにDELETEリクエストを行い、削除する宛先のIDをリクエストパスに含めて、投影先を削除でき `/config/destinations` ます。
...
次のリクエストでは、Experience Data ModelスキーマクラスXDM Individualプロファイルに関連付けられているすべての投影設定をリストします。 XDMとプラットフォーム内でのその役割に関する詳細は、 [XDMシステムの概要を読んでください](../../xdm/home.md)。
...
このガイドでは、パラメータを正しくフォーマットする方法など、エッジ投影法と目的を設定するための手順を説明しました `selector` 。 組織のニーズに合わせて新しいエッジの宛先と投影を作成できるようになりました。 プロファイルAPIで利用できるその他のアクションを見つけるには、 [リアルタイムプロファイルAPI開発者ガイドを参照してください](getting-started.md)。