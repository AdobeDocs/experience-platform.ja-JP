...
編集した内容を削除してデフォルトの表示に戻る場合は、すべてのデフォルトのカードと属性をすばやく簡単に復元できます。 その場合は、「ダッシュボードを **[!UICONTROL 変更]**」を選択し、「デフォルトのカードを **[!UICONTROL 復元]**」を選択します。 これにより、行ったすべてのカスタマイズ（サイズ変更を含む）が削除されます。 その後、「 **[!UICONTROL 保存]** 」を選択して変更を保存できます。デフォルトに戻したくない場合は、「 **[!UICONTROL キャンセル]** 」を選択して編集内容を保持します。
