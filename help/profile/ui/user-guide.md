...
[!DNL Profile] ストアへのレコードの取り込みで、カウントが 3% を超えて増加または減少すると、ジョブがトリガーされてカウントが更新されます。 ストリーミングデータワークフローの場合は、3% の増加または減少しきい値に達したかどうかを判断するために、1 時間ごとにチェックが行われます。 存在する場合、プロファイル数を更新するジョブが自動的にトリガーされます。 バッチ取り込みの場合、プロファイルストアにバッチを正常に取り込んでから 15 分以内に、3% の増加または減少のしきい値に達した場合は、プロファイル数を更新するジョブが実行されます。
