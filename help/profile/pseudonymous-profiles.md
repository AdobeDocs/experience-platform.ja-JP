...
[!UICONTROL Profile settings] ポップオーバーが表示されます。 このポップオーバーでは、偽名プロファイルデータの有効期限の日数と、データの有効期限に使用する ID 名前空間を設定できます。
...
「**[!UICONTROL Apply]**」を選択して、データの有効期限設定を保存します。
...
### 偽名プロファイルデータの有効期限は、どのようなユースケースで使用する必要がありますか？

+++ 回答

- Web SDKを使用してExperience Platformに直接データを送信する場合。
...
- 偽名プロファイルデータの有効期限は、**サンドボックス**&#x200B;レベルで機能します。本番稼働用サンドボックスと開発用サンドボックスに異なる設定を選択できます。
