...
* **[!UICONTROL エッジでアクティブ化結合ポリシー]**：この結合ポリシーをエッジでアクティブにするかどうかを選択できるトグルボタン。すべてのプロファイルコンシューマーがエッジで同じビューを使用していることを確認するために、結合ポリシーをエッジでアクティブとしてマークできます。オーディエンスをエッジでアクティブ化する（エッジオーディエンスとしてマークする）には、エッジでアクティブとマークされる結合ポリシーに結び付ける必要があります。 オーディエンスが **not** は、エッジでアクティブとマークされた結合ポリシーに結び付けられ、オーディエンスはエッジでアクティブとマークされず、ストリーミングオーディエンスとしてマークされます。 また、組織の各サンドボックスは、 **1 つ** edge 上でアクティブな結合ポリシー。
...
結合ポリシーを作成または更新すると、結合ポリシーが組織で定義されたデータ使用ポリシーに違反しているかどうかを確認するチェックが実行されます。データ使用ポリシーは、Adobe Experience Platform データガバナンスの一部であり、特定の [!DNL Platform] データに対して実行が許可される、または制限されるマーケティングアクションの種類を記述したルールです。例えば、結合ポリシーを使用してサードパーティの宛先に対してアクティブ化されたオーディエンスを作成し、組織がデータ使用ポリシーを使用していて、特定のデータをサードパーティに書き出せなかった場合、 **[!UICONTROL データガバナンスポリシー違反が検出されました]** 結合ポリシーを保存しようとした際の通知。
...
これで、組織の結合ポリシーを作成して設定しました。これを使用して、Platform 内の顧客プロファイルの表示を調整し、プロファイルデータからオーディエンスを作成できます。 詳しくは、 [セグメントの概要](../../segmentation/home.md) を使用して、オーディエンスを作成および操作する方法について詳しくは、 [!DNL Experience Platform] UI と API
