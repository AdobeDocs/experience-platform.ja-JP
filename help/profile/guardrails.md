...
| ガードレール | 上限 | 上限のタイプ | 説明 |
| --------- | ----- | ---------- | ----------- |
| サンドボックスあたりのオーディエンス | 4000 | パフォーマンスガードレール | 各サンドボックスに 4,000 個未満のオーディエンスがある限り、1 つの組織は合計 4,000 個を超えるオーディエンスを持つことができます。 バッチ、ストリーミング、エッジの各オーディエンスを含みます。 追加のオーディエンスを作成しようとすると、システムのパフォーマンスに影響を与える可能性があります。 詳細を表示： [オーディエンスの作成](/help/segmentation/ui/segment-builder.md) セグメントビルダーを使用します。 |
...
| すべてのサンドボックスにわたるエッジスループット | 1500 RPS | パフォーマンスガードレール | エッジのセグメント化では、Adobe Experience Platform Edge Network に入る 1 秒あたり 1500 件のインバウンドイベントのピーク値をサポートしています。 Edge のセグメント化では、Adobe Experience Platform Edge Network に入った後にインバウンドイベントを処理するのに最大 350 ミリ秒かかる場合があります。 詳細を表示： [エッジオーディエンス](/help/segmentation/ui/edge-segmentation.md). |
| サンドボックスごとのストリーミングオーディエンス | 500 | パフォーマンスガードレール | 個々のサンドボックスに 500 人未満のストリーミングオーディエンスがある限り、1 つの組織は合計で 500 人を超えるストリーミングオーディエンスを持つことができます。 これには、ストリーミングオーディエンスとエッジオーディエンスの両方が含まれます。 追加のストリーミングオーディエンスを作成しようとすると、システムのパフォーマンスに影響を与える可能性があります。 詳細を表示： [ストリーミングオーディエンス](/help/segmentation/ui/streaming-segmentation.md). |
| すべてのサンドボックスにわたるストリーミングスループット | 1500 RPS | パフォーマンスガードレール | ストリーミングセグメント化では、1 秒あたり 1500 件のインバウンドイベントのピーク値がサポートされます。 ストリーミングセグメント化では、プロファイルがセグメントのメンバーシップの対象として認定されるまでに最大 5 分かかる場合があります。 詳細を表示： [ストリーミングオーディエンス](/help/segmentation/ui/streaming-segmentation.md). |
...
| 最大オーディエンスサイズ | 30% | パフォーマンスガードレール | オーディエンスの最大メンバーシップは、システム内のプロファイルの合計数の 30%です。 30%を超えるプロファイルを持つオーディエンスをメンバーまたは複数の大規模なオーディエンスとして作成することは可能ですが、システムのパフォーマンスに影響を与えます。 |

{style="table-layout:auto"}

## 予想される可用性

次の節では、 **期待** Real-Time CDPの宛先など、ダウンストリームサービスでのオーディエンスの可用性と結合ポリシー：

| サンドボックスタイプ | 時間 |
| ------------ | ---- |
| 既存のサンドボックス | 1 時間 |
| 新しいサンドボックス | 2 時間 |
| サンドボックスを新しくリセット | 2 時間 |
...
プロファイルデータの保守はリレーショナルストアではありませんが、プロファイルでは、シンプルで直感的な方法でオーディエンスを作成するために、小さなディメンションエンティティとの統合を可能にします。 この統合は、 [マルチエンティティセグメント化](../segmentation/multi-entity-segmentation.md).
