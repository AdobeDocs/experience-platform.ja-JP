...
Scala では、`clientContext` を読み込んで Platform の値を取得したり返したりできるので、`var userToken` などの変数を定義する必要はありません。 以下の Scala の例では、データセットを読み取 `clientContext` ために必要な値をすべて取得して返すために使用しています。
