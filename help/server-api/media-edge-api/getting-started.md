...
このガイドでは、Media Edge API サービスとの最初のインタラクションを成功させるための手順を説明します。これには、メディアセッションの開始と、Customer Journey Analytics（CJA）などの Adobe Experience Platform ソリューションに送信されるイベントの追跡が含まれます。Media Edge API サービスは、セッション開始エンドポイントで開始されます。セッションが開始されると、次の 1 つ以上のイベントをトラックできます。
...
各イベントには、独自のエンドポイントがあります。 すべての Media Edge API エンドポイントは POST メソッドであり、イベントデータ用の JSON リクエスト本文が含まれます。Media Edge API エンドポイント、パラメーター、例について詳しくは、[Media Edge Swagger](swagger.md) ファイルを参照してください。
...
* `datastreamId` は、POST セッション開始リクエストの必須パラメーターです。`datastreamId` を取得するには、[データストリームの設定](https://experienceleague.adobe.com/docs/experience-platform/datastreams/configure.html?lang=ja)を参照してください。
...
また、上記の例の `eventType` のデータタイムマッピングは、次のようになります。
...
セッション開始エンドポイントのパラメーターと例について詳しくは、[Media Edge Swagger](swagger.md)ファイルを参照してください。
...
バッファー開始イベントは、メディアプレーヤーでバッファリングが開始された際にシグナルで通知します。バッファー再開は API サービスのイベントではありません。代わりに、バッファー開始後に再生イベントが送信されると推論されます。バッファー開始イベントリクエストを行うには、次のエンドポイントへの呼び出しのペイロードで `sessionId` を使用します。
...
上記のリクエスト例では、前の呼び出しで返されたのと同じ `sessionId` がバッファー開始リクエストの必須パラメーターとして使用されます。

成功時の応答はステータスが 200 で、コンテンツは含まれていません。

バッファー開始エンドポイントのパラメーターと例について詳しくは、[Media Edge Swagger](swagger.md) ファイルを参照してください。
...
再生エンドポイントのパラメーターと例について詳しくは、[Media Edge Swagger](swagger.md) ファイルを参照してください。
...
セッション完了エンドポイントのパラメーターと例について詳しくは、[Media Edge Swagger](swagger.md) ファイルを参照してください。
...
| 207 | Edge ネットワークに接続するサービスの 1 つに関する問題 ( 詳しくは、 [トラブルシューティングガイド](troubleshooting.md)) |
| 400 レベル | リクエストが正しくありません |
| 500 レベル | サーバーエラー |

## この機能に関するその他のヘルプ

* [Media Edge トラブルシューティングガイド](troubleshooting.md)
