---
keywords: Experience Platform；ホーム；人気のトピック；CJA；ジャーニー分析；Customer Journey Analytics；キャンペーンオーケストレーション；オーケストレーション；カスタマージャーニー；ジャーニー；ジャーニーオーケストレーション；機能；ワークフロー
title: アプリケーションドキュメント
description: Adobe Experience Platformのドキュメントは、ユーザーインターフェイスと API の両方の概要、チュートリアル、ガイドなど、複数の形式で提供されています。Experience Platformアプリケーションで使用できる、最も一般的なドキュメントタイプを簡単に説明します。
exl-id: 24fdb183-f75b-40e0-828e-9dd49422a7b1
source-git-commit: cb37eda87b8fcc0d0284db7a0bab8d48eab5aae6
workflow-type: tm+mt
source-wordcount: '105'
ht-degree: 22%

---

# アプリケーションドキュメント

Adobeは、Experience Platformを基盤として複数のアプリケーションを構築しています。 以下のリンクを使用して、これらのアプリケーションのドキュメントを確認できます。

* [[!DNL Customer Journey Analytics]](https://experienceleague.adobe.com/docs/customer-journey-analytics.html?lang=ja)
* [[!DNL Journey Orchestration]](https://experienceleague.adobe.com/docs/journey-orchestration.html?lang=ja)
* [[!DNL Adobe Journey Optimizer]](https://experienceleague.adobe.com/docs/journey-optimizer.html?lang=ja)
* [[!DNL Adobe Real-Time Customer Data Platform (Real-Time CDP)]](../rtcdp/overview.md)
* [[!DNL Adobe Mix Modeler]](https://experienceleague.adobe.com/docs/mix-modeler.html?lang=ja)

次のビデオでは、Experience Platform上に構築された各アプリケーションのユースケースについて説明します。

>[!VIDEO](https://video.tv.adobe.com/v/3428519/?learn=on&captions=jpn)
