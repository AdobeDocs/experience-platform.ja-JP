...
| [!UICONTROL プロファイルデータセット] | The [!DNL Profile]作成した顧客の同意フィールドを含む有効なデータセット [以前](#prerequisites). |
...
SDK 拡張機能がインストールされている場合、デフォルトのデータ収集同意値 (`collect.val`) を使用します。 これは、ユーザーに応じて異なるデフォルト値 ( `pending` （EU ユーザー向け）および `in` （北米のユーザー向け）

この使用例では、次の機能を実装して、ユーザーの地域に基づいてデフォルトの同意を設定できます。

1. Web サーバー上のユーザーの地域を特定します。
1. の前に `script` タグ（埋め込みコード）を Web ページ上で設定し、 `script` タグを設定します。 `adobeDefaultConsent` 変数の値を指定します。
...
1. イベントハンドラーで、 `adobeDefaultConsent` 変数を読み込み、JavaScript を使用してタグライブラリスクリプトを読み込みます。
1. を使用するデータ要素を設定する `adobeDefaultConsent` JavaScript 変数を使用し、このデータ要素をユーザーのデフォルトの同意値として使用します。
...
データ要素を作成したら、 Web SDK 拡張機能の設定ページに戻ります。 の下 [!UICONTROL プライバシー] セクション、選択 **[!UICONTROL データ要素によって提供されます]**&#x200B;を選択し、指定したダイアログを使用して、前に作成したデフォルトの同意データ要素を選択します。
...
The `setConsent` コマンドは、次の 2 つのアクションを実行します。

1. ユーザーのプロファイル属性をプロファイルストアで直接更新します。 データレイクにはデータは送信されません。
1. を作成します。 [エクスペリエンスイベント](../../../xdm/classes/experienceevent.md) には、同意の変更イベントのタイムスタンプ付きのアカウントが記録されます。 このデータはデータレイクに直接送信され、経時的に同意設定の変更を追跡するために使用できます。
...
>[!NOTE]
>
>他の同意標準を `Adobe` ( 例： `IAB TCF`) を追加する場合、 `consent` 配列を作成します。 各オブジェクトには、 `standard`, `version`、および `value` 彼らが表す同意基準のために。
