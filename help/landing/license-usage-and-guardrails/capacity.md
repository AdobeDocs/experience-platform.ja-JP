...
>title="ストリーミングスループット"
>abstract="ストリーミングスループット値は、本番稼働用サンドボックスと開発用サンドボックス全体で、プロファイルサービスへのストリーミング取得に対する 1 秒ごとの合計ピークインバウンドイベントを測定します。"
...
ライセンス済み処理能力の 100% を超える使用量が発生すると、処理能力を超えていると見なされます。 容量を超過した場合は、次の制限が適用されます。

>[!NOTE]
>
>Adobe Journey Optimizerへのアクセス権を持っている場合、次の制限は適用されません **適用されません**。

- イベント処理キューが 12 時間を超える場合は、イベントデータ **可能** をストリーミングパーソナライゼーションから削除できます
- 削除されたイベントデータはプロファイルに取り込まれる **ありません**
   - イベントが削除された日時を確認できます
   - イベントは、使用権限に応じて、データレイク内で使用できます
   - 必要 *応じて* クエリサービスを使用して、データを直接再取り込みできます
