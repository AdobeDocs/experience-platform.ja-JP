...
`/roles` エンドポイントにGET リクエストを送信することで、組織に属する既存のすべての役割をリストできます。
...
| `subjectAttributes` | サブジェクトと、サブジェクトがアクセス権を持つExperience Platform リソースとの相関関係を示す属性。 |
| `subjectAttributes.labels` | クエリされた役割に適用されたデータ使用ラベルを表示します。 |

## 役割の検索 {#lookup}

個々のロールを検索するには、リクエストパスに対応するロールを含むGET リク `roleId` ストを実行します。
...
また、{ROLE_ID} を指定して `/roles` エンドポイントに対してGET リクエストを実行することで、件名を取得できます。
...
新しい役割を作成するには、`/roles` エンドポイントに対して POST リクエストを実行し、その際、役割の名前、説明、役割タイプの値を指定します。
...
ロールのプロパティを更新するには、適用したい操作に対応するロール ID と値を指定して、`/roles` エンドポイントに対してPATCH リクエストを行います。
...
`/roles` エンドポイントにPUT リクエストを実行し、更新するロールに対応するロール ID を指定することで、ロールを更新できます。
...
ロールに関連付けられた件名を更新するには、更新する件名のロール ID を指定したうえで、`/roles` エンドポイントに対してPATCH リクエストを行います。
...
ロールを削除するには、削除するロールの ID を指定したうえで、`/roles` エンドポイントに対してDELETE リクエストを行います。
...
ロールに対して検索（GET）リクエストを実行することで、削除を確認できます。 役割が管理から削除されたので、HTTP ステータス 404 （見つかりません）が返されます。

## API 資格情報の追加 {#apicredential}

API 資格情報を追加するには、主体のロール ID を指定 `/roles` て、エンドポイントにPATCH リクエストを行います。
