...
>[!NOTE]
>
>Adobe Experience Platform Launch は、Experience Platform のデータ収集テクノロジースイートとしてリブランドされています。 その結果、製品ドキュメント全体でいくつかの用語の変更がロールアウトされました。 用語の変更点の一覧については、次の[ドキュメント](../../../launch-term-updates.md)を参照してください。

>[!IMPORTANT]
>
>拡張機能は、 [!DNL Adobe Experience Platform Web SDK] 拡張子。 AppMeasurement で使用する必要のあるバージョンに関する情報を確認するには、このリンクを参照してください。

このリファレンスは、Web SDK Plugins 拡張機能の設定に関する情報、およびこの拡張機能を使用して [!DNL Adobe Experience Platform Web SDK] 拡張機能。
...
>[!IMPORTANT]
>
>Common Web SDK Plugins 拡張機能は、 [!DNL Adobe Experience Platform Web SDK] ただし、拡張機能が期待どおりに動作するために、拡張機能をインストールする必要はありません。

拡張機能レベルでは、追加の設定は必要ありません。

## [!DNL Adobe Experience Platform Web SDK] 拡張機能へのプラグインの追加

AppMeasurement とは異なり、提供されるネイティブデータ要素を使用する前に、プラグインを初期化または追加するための設定は必要ありません。 各データ要素について詳しくは、以下を参照してください。

## Common Web SDK Plugins 拡張機能のデータ要素

Common Web SDK Plugins 拡張機能は、次のデータ要素を提供します。
...
Adobe Experience Platformのネイティブのデータ収集 UI を活用して、 [getAndPersistValue プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getandpersistvalue.html). この `getAndPersistValue` データ要素を使用すると、cookie に値を保存して後で訪問中に取得できます。 これは、Adobe Experience Platform Launch の[!UICONTROL ストレージ期間]機能と同様の役割を果たします。
...
Adobe Experience Platformのネイティブのデータ収集 UI を活用して、 [getGeoCoordinates プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getgeocoordinates.html). この `getGeoCoordinates` データ要素を使用すると、訪問者のデバイスの緯度と経度を取り込むことができます。
...
ネイティブのデータ収集 UI を活用して、 [getNewRepeat プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getnewrepeat.html). この `getNewRepeat` データ要素を使用すると、サイトの訪問者が新しい訪問者か、再訪問者かを希望の日数以内に判断できます。
...
ネイティブのデータ収集 UI を活用して、 [getPageName プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getpagename.html). この `getPageName` データ要素は、読みやすく、わかりやすい形式の現在の URL を作成します。
...
ネイティブのデータ収集 UI を活用して、 [getPreviousValue プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getpreviousvalue.html). この `getPreviousValue` データ要素を使用すると、変数を以前のヒットに設定された値に設定できます。
...
ネイティブのデータ収集 UI を活用して、 [getQueryParam プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getqueryparam.html). この `getQueryParam` データ要素を使用すると、URL に含まれるクエリー文字列パラメーターの値を抽出できます。 これは、ランディングページの URL から内部および外部の両方のキャンペーンコードを抽出する場合に役立ちます。また、検索用語や他のクエリー文字列パラメーターを抽出する場合にも役立ちます。このデータ要素は、複数のクエリー文字列パラメーターを含むハッシュや URL を含む複雑な URL を解析する際に堅牢な機能を提供します。
...
ネイティブのデータ収集 UI を活用して、 [getTimeParting プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/gettimeparting.html). この `getTimeParting` データ要素を使用すると、サイトで測定可能なアクティビティが発生した時間の詳細を取り込むことができます。 このデータ要素は、指定した日付範囲で繰り返し可能な時間の除算で指標を分類する場合に役立ちます。 例えば、すべての日曜日とすべての木曜日など、2 つの異なる曜日間のコンバージョン率を比較できます。また、すべての朝とすべての晩など、1 日の時間を比較することもできます。
...
ネイティブのデータ収集 UI を活用して、 [getTimeSinceLastVisit プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/gettimesincelastvisit.html). この `getTimeSinceLastVisit` データ要素を使用すると、訪問者が最後の訪問後にサイトに戻ってきた時間を追跡できます。
...
ネイティブのデータ収集 UI を活用して、 [getValOnce プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getvalonce.html). この `getValOnce` データ要素は、変数が同じ値に複数回設定されるのを防ぎます。
...
ネイティブのデータ収集 UI を活用して、 [getVisitDuration プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getvisitduration.html). この `getVisitDuration` データ要素は、訪問者がその時点までサイトに滞在した時間を分単位で追跡します。
...
ネイティブのデータ収集 UI を活用して、 [getVisitNum プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getvisitnum.html). この `getVisitNum` データ要素は、目的の日数内にサイトに来訪したすべての訪問者の訪問回数を返します。
...
ネイティブのデータ収集 UI を活用して、 [p_fo プラグイン](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/p-fo.html). この `p_fo` データ要素は、特定の JavaScript オブジェクトが存在するかどうかを確認するユーティリティです。 オブジェクトが存在しない場合は、オブジェクトを作成して `true` を返します。JavaScript オブジェクトが既にページ上に存在する場合は、`false` を返します。このデータ要素は、ページ上でコードを 1 回だけ実行する場合に役立ちます。

この `p_fo` データ要素では、次の引数を使用します。

* **オン** （必須、文字列）:データ要素が作成する JavaScript オブジェクトの名前（オブジェクトがまだページ上に存在しない場合）。
