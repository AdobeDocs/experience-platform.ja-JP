...
[値比較](#value-comparison-value-comparison)条件のラッパー。 比較の結果に基づいて、はフォームで使用可能な2つの値のいずれかを返します。 「If...それで…さもなければ…」 シナリオに追加のルールが必要ない。
...
* プロパティ名 — Launchプロパティの名前を取得するために`_satellite.property.name`を返します。
* プロパティID - LaunchプロパティのIDを取得するために`_satellite.property.id`を返します
* ルール名 — 実行されたルールの名前を含む`event.$rule.name`を返します。
* ルールID — 実行されたルールのIDを含む`event.$rule.id`を返します。
...
これは、一般的なJavaScript操作のラッパーです。 データ要素を入力として受け取ります。 データ要素の値に対して、次のいずれかの変換の結果を返します。

* 基本的な文字列操作（replace、substring、regexの一致、最初と最後のインデックス、split、slice）
