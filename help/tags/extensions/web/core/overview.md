---
title: コア拡張機能の概要
description: Adobe Experience PlatformのCoreタグ拡張について説明します。
...
Coreタグ拡張機能は、Adobe Experience Platformでリリースされたデフォルトの拡張機能です。

このドキュメントでは、Core拡張機能を使用してルールを作成する際に使用できるオプションに関する情報を提供します。

## Core 拡張機能のイベントタイプ {#core-extension-event-types}

このトピックでは、Core 拡張機能で使用できるイベントのタイプについて説明します。複数の異なるイベントタイプに設定できるオプションについて詳しくは、[オプション](#options)の節を参照してください。

### ブラウザーベースのイベント

#### Tab Blur

tab-blurイベントは、タブがフォーカスを失ったときのトリガーを設定します。 このイベントタイプに関する設定はありません。

#### Tab Focus

タブフォーカスイベントは、タブがフォーカスされたときのトリガーを設定します。 このイベントタイプに関する設定はありません。

### フォーム

#### Blur

blurイベントは、フォームがトリガーを失ったときのアクションをフォーカスします。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

#### Focus

focusイベントは、フォームがフォーカスを取得したときのアクションをトリガー化します。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

#### Submit

送信イベントは、フォームがトリガーされたときのアクションを送信します。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

### キーボード制御イベント

#### Key Press

イベントトリガーは、キーが押されたときに発生します。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

### メディアベースのイベント

#### Media Ended

イベントトリガー（メディアが終了したとき）。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

#### メディア読み込みデータ

イベントトリガー（メディアがデータを読み込む際）。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

#### Media Pause

イベントトリガー：メディアが一時停止されたとき。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

#### Media Play

イベントトリガー（メディアの再生時）。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

#### Media Stalled

イベントは、メディアが停止した場合にトリガーされます。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

#### Media-Time Played

イベントトリガー（メディアが指定の時間再生された場合）。 イベントを再生するメディアの再生時間を指定する必要があります。トリガー カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。


#### Media-Volume Changed

イベントトリガー（ボリュームが増減した場合）。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

### モバイルデバイス指向のイベント

#### Orientation Change

イベントトリガー（デバイスの向きが変更された場合）。 イベントをトリガーするには、向きを変更する必要がある期間を指定する必要があります。 このイベントタイプに関する設定はありません。

#### Zoom Change

イベントトリガー（ユーザーがズームインまたはズームアウトした場合）。 このイベントタイプに関する設定はありません。

### マウス制御イベント

#### Click

イベントトリガー(指定した要素が選択（クリック）された場合)。 オプションとして、イベントがトリガーされる前に、要素で true となる必要があるプロパティ値を指定できます。

要素がリンクされたコンテンツのアンカータグ(`<a>`)の場合は、ナビゲーションを一定期間遅らせるかどうかを指定することもできます。 これは、ルールの実行に余分な時間が必要で、通常ページナビゲーションがおこなわれる前に実行が完了しない場合に便利です。

>[!WARNING]
>
>このオプションは、誤って使用した場合、ユーザーエクスペリエンスに悪影響を与える可能性があるので、慎重に使用してください。

リンク遅延を使用すると、Platformは、実際にはブラウザーがページから移動するのを防ぎます。 次に、指定されたタイムアウトの後に、元の宛先へのJavaScriptリダイレクトを実行します。 これは、ページのマークアップに `<a>` タグがあり、意図したとおりに機能しても実際にはユーザーがページから移動しない場合は特に危険です。  問題を他の方法で解決できない場合は、セレクターの定義を正確に指定して、このイベントが必要な場所でのみトリガーするようにしてください。

デフォルトのリンク遅延値は 100 ミリ秒です。 タグは、指定された時間だけ待機し、ルールのアクションの実行にはどのような方法でも接続されないことに注意してください。 遅延のせいでユーザーの待機時間が必要以上に長くなる可能性があります。また、逆に、遅延が十分に長くないためルールの必ずしもすべてのアクションが正常に完了することができない可能性もあります。この時間を長くすると、ルールの実行に時間がかかりますが、ユーザーエクスペリエンスも向上します。

遅延を有効にするには、イベントをトリガーする選択した要素と、イベントがトリガーされるまでの特定の時間の両方を指定する必要があります。

詳細設定のオプションについて詳しくは、[オプション](#options)の節を参照してください。

#### Hover

イベントトリガーは、ユーザーが指定した要素の上にカーソルを置くと発生します。 また、ルールを直ちにトリガーするか、指定したミリ秒数が経過した後にトリガーするかを設定する必要があります。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

### その他のイベント

#### Custom Event

イベントトリガー（カスタムイベントタイプが発生した場合）。 コードベース内の他の場所で定義されている名前付きJavaScript関数は、カスタムイベントタイプとして使用できます。 カスタムイベントタイプの名前を指定し、以下の[オプション](#options)の節で説明するように、他の設定を行う必要があります。

#### Data Element Changed

イベントトリガー（指定したデータ要素が変更された場合）。 データ要素の名前を指定する必要があります。 データ要素を選択するには、テキストフィールドに名前を入力するか、テキストフィールドの右側にあるデータ要素アイコンを選択し、表示されるダイアログ内のリストから選択します。

#### Direct Call

ダイレクトコールイベントは、イベント検出とルックアップシステムをバイパスします。 ダイレクト型ルールは、Platformに対して、何がおこなわれるかを正確に伝えたい場合に最適です。 また、Adobe Flash を使用しているなど、Platform が DOM でイベントを検出できない場合にも最適です。識別子テキストフィールドに`_satellite.track`文字列を指定します。

#### Element Exists

イベントトリガー（指定した要素が存在する場合）。 カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

#### Enters Viewport

イベントトリガーは、ユーザーが指定のビューポートに入った場合に発生します。 一致する要素をターゲットにする条件として、CSSセレクターを指定する必要があります。 また、ルールを直ちにトリガーするか、指定したミリ秒数が経過した後にトリガーするか、イベントが発生するたびにイベントをトリガーするか、初めてイベントを発生させるかを設定する必要があります。

カスタマイズ可能なイベント設定について詳しくは、[オプション](#options)の節を参照してください。

#### History Change

pushStateまたはhashchangeイベントが発生した場合にイベントがトリガーされます。 このイベントタイプに関する設定はありません。

#### ページでの滞在時間

イベントトリガー：ユーザーが指定された秒数の間ページに留まる場合に発生します。 イベントがトリガーされるまでに経過する必要がある秒数を指定する必要があります。

### ページ読み込みイベント

#### DOM Ready

イベントトリガーは、DOMの準備が整い、ユーザーがページを操作できる状態にします。 このイベントタイプに関する設定はありません。

#### Library Loaded（Page Top）  {#library-loaded-page-top}

イベントトリガーは、タグライブラリが読み込まれるとすぐに実行されます。 このイベントタイプに関する設定はありません。

#### Page Bottom {#page-bottom}

`_satellite.pageBottom();`が呼び出されると、イベントトリガーが呼び出されます。 タグライブラリを非同期で読み込む場合、このイベントタイプは使用しないでください。 このイベントタイプに関する設定はありません。

#### Window Loaded

onLoadがブラウザーによって呼び出され、ページの読み込みが完了した場合のイベントトリガー。 このイベントタイプに関する設定はありません。
...
すべてのAdobe Experience Platformユーザーが使用できるCore拡張機能には、ユーザー指定のJavaScriptまたはHTMLを実行するためのカスタムコードアクションが含まれています。 「Custom Code」アクションを使用するルールの処理方法を理解すると便利です。
...
>[!NOTE]
>
>これは、JavaScriptがグローバルに実行されない設定の場合にのみ機能します。 グローバルスコープでカスタムコードアクションを実行している場合、タグはプロミスが直ちに解決されたものと見なし、処理キュー内の次の項目に移動します。
...
HTML のカスタムコードアクションを作成する場合、`onCustomCodeSuccess()` という関数をカスタムコード内で使用することができます。この関数を呼び出して、カスタムコードが完了し、タグが後続のアクションの実行に進む可能性があることを示すことができます。 一方、カスタムコードが何らかの方法で失敗した場合は、`onCustomCodeFailure()` を呼び出すことができます。これにより、そのルールから後続のアクションを実行しないようにタグに通知します。
...
タグデータ要素を使用して、マークアップのJavaScript変数やオブジェクトプロパティを取り込むことができます。 これらの値は、タグデータ要素を参照することで、拡張機能やカスタムルール内で使用できます。 データのソースが変更された場合は、データ収集UI内でソースへの参照を更新するだけで済みます。

次の例では、マークアップに`Page_Name`というJavaScript変数が含まれています。
...
データ収集UIでデータ要素を作成する場合は、次のようにこの変数へのパスを指定するだけです。

データレイヤーでデータコレクターオブジェクトを使用している場合は、パスに、データ要素に取得するオブジェクトおよびプロパティを参照するドット表記を使用します（例：`_myData.pageName`、`digitalData.pageName`など）。
...
ページ情報と同様に、このデータ要素は、ルールや他のPlatformソリューション内でロジックを強化するために、共通の動作タイプを使用します。
