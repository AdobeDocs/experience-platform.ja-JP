...
## v0.20.3 （2024 年 1 月 23 日）

- `at.js` 2.11.4 をサポートするように更新しました
- 無効な地域データが配信 API に送信されるのを防ぐためのバグを修正しました。

## v0.20.2 （2023 年 11 月 29 日（PT））

- `at.js` 2.11.3 をサポートするように更新しました
- コンテンツのレンダリングに失敗したイベントで応答トークンが送信されないバグを修正しました。

## v0.20.1 （2023 年 11 月 3 日（PT））

- `at.js` 2.11.2 をサポートするように更新しました。
- カスタムイベントで送信された応答トークンに不一致が発生するバグを修正しました。

## v0.20.0 （2023 年 10 月 9 日（PT））

- `at.js` 2.11.0 をサポートするように更新しました。
- targetGlobalSettings にカスタム Adobe Experience Platform sandboxId と sandboxName を設定するサポートを追加しました。これは、getOffer/getOffers 呼び出し時に配信 API に渡されます。
- セレクターで：eq （）を連結するためのシャドウ DOM 修正。

## v0.19.3 （2023 年 9 月 18 日（PT））

- `at.js` v2.10.3 をサポートするように更新しました。
- オファーがレンダリングされない場合に、at-content-rendering-succeeded カスタムイベントが誤ってトリガーされる問題を修正しました。 正しいイベント at-content-rendering-no-offers がトリガーされるようになりました。
- at-content-rendering-failed カスタムイベントの error オブジェクトに、eventToken と responseTokens を追加しました。

## v0.19.2 （2023 年 2 月 14 日（PT））

- タイムアウトをデータ要素に設定できた問題を修正しました。

## v0.19.1 （2023 年 2 月 3 日（Pt））

- `at.js` v2.10.1 をサポートするように更新しました
- クライアントのカスタム mbox パラメーターで、ドット表記が正しくサポートされるようになりました
- VEC で行われなくなった配信呼び出し

## v0.19.0 （2022 年 9 月 19 日）

- `at.js` v2.10.0 をサポートするように更新しました
- クロスドメイントラッキングサポートを追加しました。

## v0.18.0 （2022 年 6 月 1 日（PT））

- `at.js` v2.9.0 をサポートするように更新しました
- User Agent Client Hints のサポートを追加しました。

## v0.17.1 （2022 年 1 月 28 日（PT））
...
## v0.16.0 （2021 年 10 月 28 日）

- `at.js` v2.7.0 がサポートされるように更新されました。これにより、Adobe Target からダウンロードできるようになりました。

## v0.15.2 （2021 年 8 月 16 日）

- `at.js` 2.6.1 をサポートするように更新しました。
- ページ読み込みイベントに依存せずに、起動時にオンデバイス判定を初期化します。
- アーティファクトがダウンロードされた後、最初の訪問でオンデバイス判定を使用できるようになりました。

## v0.15.1 （2021 年 7 月 20 日）

- `stringify` 関数名のクラッシュが原因で、`sessionId` や `requestId` などに対して誤った UUID 値が生成されていた問題を修正しました。

## v0.15.0 （2021 年 7 月 16 日）
