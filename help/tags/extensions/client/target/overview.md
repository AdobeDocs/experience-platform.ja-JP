...
タイムアウトを除くすべての at.js 設定は、Target ユーザーインターフェイスの at.js 設定から自動的に取得されます。拡張機能は、最初に追加されたときに Target ユーザーインターフェイスから設定を取得するだけです。追加の更新が必要な場合は、すべての設定を UI で管理する必要があります。
...
* 次のイベントが発生すると、ページが表示されます。
   ...
   * スニペット自体がタイムアウトしたとき
* 事前非表示の時間を最小限に抑えるには、「Fire Global Mbox」アクションを、事前非表示スニペットを使用するすべてのページで使用する必要があります。
