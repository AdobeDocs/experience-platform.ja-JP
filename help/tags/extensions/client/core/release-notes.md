...
* [直接呼び出しアクションのトリガー](./overview.md#direct-call-action)を変更して、直接呼び出しルールに送信するカスタムイベント情報を提供できるようにします。
...
* **クリックイベントでのリンク遅延を「サポート対象外」とマークしました。** - 詳しくは、Adobe Experience Platform の [データ収集ブログ](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-platform-launch/explainer-link-delay/ba-p/398403) を参照してください

## 2021 年 1 月 6 日（PT）

v1.9.0

* **新しい「トリガー直接呼び出し」アクション** - Core 拡張機能に、「 」という新しいアクションタイプが追加されました。 `Trigger Direct Call`.  これは、別のルールのアクションを介して直接呼出しのルールをトリガーする場合に使用できます。これは、`_satellite.track()`メソッドに直接マッピングされます。この投稿に対し、Jan Exner 氏にお礼を申し上げます。
...
* **CSP nonce のサポート**：コア拡張機能にオプションの構成パラメーターが追加されました。nonce を参照するデータ要素を追加できます。設定した場合、タグがページに追加するすべてのインラインスクリプトで、設定したナンスが使用されます。 この変更により、コンテンツセキュリティポリシーをナンスで使用できるため、引き続き CSP 環境でタグスクリプトを読み込むことができます。CSP でのタグの使用について詳しくは、 [こちら](../../../ui/client-side/content-security-policy.md) を参照してください。
