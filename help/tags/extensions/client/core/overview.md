...
デバイスの向きが変わると、イベントがトリガーします。 イベントがトリガーされるように、向きを変更する期間を指定する必要があります。このイベントタイプに関する設定はありません。
...
リンク遅延を使用する場合、Experience Platformは実際にブラウザーがページから移動するのを防ぎます。 次に、指定されたタイムアウトの後に、元の宛先への JavaScript リダイレクトを実行します。これは、ページのマークアップに `<a>` タグがあり、意図したとおりに機能しても実際にはユーザーがページから移動しない場合は特に危険です。  問題を他の方法で解決できない場合は、セレクターの定義を正確に指定して、このイベントが必要な場所でのみトリガーするようにしてください。

デフォルトのリンク遅延値は 100 ミリ秒です。 タグは常に指定された時間を待ち、ルールのアクションの実行に何らかの方法で接続されていないことに注意してください。 遅延によってユーザーが必要以上に長く待たされる可能性があります。また、すべてのルールのアクションが正常に完了するまで遅延が十分に長くない可能性もあります。 この時間を長くすると、ルールの実行に時間がかかるだけでなく、ユーザーエクスペリエンスも低下します。
...
指定したデータ要素が変更されると、イベントをトリガーします。変更のための投票は1秒に1回行われます。 データ要素の名前を指定する必要があります。 データ要素を選択するには、テキストフィールドにその名前を入力するか、テキストフィールドの右側にあるデータ要素アイコンを選択し、表示されるダイアログ内のリストから選択します。
...
![データ収集 UI での直接呼び出しイベントのスクリーンショット](../../../images/extensions/client/core/direct-call-event.png)
...
プロパティ設定の「ルールコンポーネントを順番に実行」オプションが有効になっている場合、条件が非同期タスクを実行する間、後続のルールコンポーネントを待機させることができます。
...
プロパティ設定の「ルールコンポーネントを順番に実行」オプションが有効になっている場合、アクションが非同期タスクを実行する間、後続のルールコンポーネントを待機させることができます。  JavaScript と HTML カスタムコードでは、動作は異なります。
...
タグデータ要素を使用して、マークアップの JavaScript 変数やオブジェクトプロパティを取り込むことができます。 これらの値は、タグデータ要素を参照することで、拡張機能やカスタムルール内で使用できます。 データのソースが変更された場合は、ソースへの参照を更新するだけで済みます。
...
データ要素を作成するときは、その変数へのパスを指定するだけです。

データレイヤーの一部としてデータコレクターオブジェクトを使用する場合は、パスのドット表記法を使用して、`_myData.pageName`や`digitalData.pageName`など、データ要素に取り込むオブジェクトとプロパティを参照します。
...
乱数を生成するには、このデータ要素を使用します。データのサンプリングや、ヒット IDなどのIDの作成によく使用されます。 乱数は、難読化または salt 機密データに使用できます。例として次のようなものが挙げられます。
...
ページ情報と同様に、このデータ要素は一般的なビヘイビアータイプを使用して、ルールやその他のExperience Platform ソリューション内のロジックを強化します。
