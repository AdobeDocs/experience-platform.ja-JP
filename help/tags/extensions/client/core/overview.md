...
イベントトリガー（デバイスの向きが変更された場合）。 イベントがトリガーされるように、向きを変更する期間を指定する必要があります。このイベントタイプに関する設定はありません。
...
デフォルトのリンク遅延値は 100 ミリ秒です。 タグは、指定された時間だけ待機し、ルールのアクションの実行には接続されないことに注意してください。 遅延により、ユーザーが必要以上に長い時間待つように強制される場合や、すべてのルールのアクションが正常に完了するまでに十分な遅延が生じない場合もあります。 この時間を長くすると、ルールの実行に時間がかかるだけでなく、ユーザーエクスペリエンスも低下します。
...
プロパティ設定の「Run rule components in sequence」オプションを有効にすると、条件が非同期タスクを実行する間、後続のルールコンポーネントを待機させることができます。
...
プロパティ設定の「Run rule components in sequence」オプションが有効になっている場合、アクションが非同期タスクを実行する間、後続のルールコンポーネントを待機させることができます。  JavaScript と HTML カスタムコードでは、動作は異なります。
...
データ要素を作成する場合は、次のようにこの変数へのパスを指定するだけです。
