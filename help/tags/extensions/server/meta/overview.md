...
>[!NOTE]
>
>既存のクイックスタートワークフローを使用して参照実装を作成し、次の用途に使用できます。
>* 新しい実装の開始として使用します。
>* この実装を参照用実装として活用して、設定方法を確認し、現在の実稼動実装でレプリケートできます。
...
The [!UICONTROL 検証結果] ダイアログには、Meta 拡張機能の実装結果が表示されます。 選択 **[!UICONTROL 次へ]**. また、 **[!UICONTROL アシュランス]** リンク。
