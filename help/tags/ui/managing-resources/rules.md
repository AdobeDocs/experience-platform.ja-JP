...
すべてがイベントを共有し、すべてがデフォルトの優先順位を持つ 5 つのルールのセットについて考えてみましょう。

* 最後に実行するルールがある場合は、その 1 つのルールコンポーネントを編集し、50 より大きい数（例：60）を指定します。
* 最初に実行するルールがある場合は、その 1 つのルールコンポーネントを編集し、50 より小さい数値（例：40）を指定します。

>[!NOTE]
>
>最終的に、アクションを順に実行する責任は、使用しているイベントタイプの拡張開発者が負います。 Adobe 拡張機能の開発者は、拡張機能が意図したとおりに動作することを確認します。Adobeは、サードパーティの拡張機能開発者に対して、これを適切におこなうためのガイダンスを提供しますが、これらのガイドラインに従う方法を保証することはできません。

1 ～ 100 の正の数（デフォルトは 50）でルールを並べ替えることを強くお勧めします。 ルールの順序は手動で管理する必要があるので、ベストプラクティスは、できるだけ簡単に順序を設定することです。 この制限が制限しすぎるエッジケースがある場合、タグは+/- 2,147,483,648 の間のルール順序番号をサポートします。
...
## ルールコンポーネントの優先順位 {#sequencing}
 ...
 ランタイム環境の動作は、プロパティで「**[!UICONTROL ルールコンポーネントを順番に実行]**」がオンとオフのどちらに設定されているかによって異なります。この設定は、ルールのコンポーネントを並行して（非同期で）評価できるか、順番に評価する必要があるかを決定します。

>[!IMPORTANT]
>
>この設定は、各ルール内での条件とアクションの評価方法のみを決定し、プロパティでルール自体が実行される順序には影響しません。 前の節 ( [ルール順序](#rule-ordering) を参照してください。
>
>In [イベント転送](../event-forwarding/overview.md) プロパティやルールのアクションは、常に順番に実行され、この設定は使用できません。 ルールを作成する際は、順序が正しいことを確認してください。

### 有効

この設定が実行時にイベントがトリガーされた場合、ルールの条件とアクションが（定義した順序に基づいて）処理キューに追加され、「first in, first out」(FIFO) 単位で一度に 1 つずつ処理されます。 ルールは、コンポーネントが完了するのを待ってから、次のコンポーネントに移動します。
