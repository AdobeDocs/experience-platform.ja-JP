...
1. 順序のパラメーターを設定し、「 **[!UICONTROL Keep Changes]**」をクリックします。
...
1. 「アクション **[!UICONTROL Add]**」アイコンを選択して、拡張機能と、その拡張機能で使用可能ないずれかのアクションタイプを選択し、アクションの設定を指定してから「**[!UICONTROL Keep Changes]**」を選択します。
   ...
   （詳細）次のアクションの実行を待機：このオプションは、プロパティでルールコンポーネントの優先順位が有効になっている場合に使用できます。オンにすると、完了するまでタグは次のアクションを呼び出しません。オフにすると、直ちに次のアクションの実行が開始されます。デフォルトは **[!UICONTROL Checked]** です。
...
1. ルールを確認して、「**[!UICONTROL Save Rule]**」を選択します。
...
ランタイム環境の動作は、プロパティ **[!UICONTROL Run rule components in sequence]** オンとオフのどちらに設定されているかによって異なります。 この設定はにより、ルールのコンポーネントを並行して（非同期で）評価できるか、順番に評価する必要があるかが決定します。
