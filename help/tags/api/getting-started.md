...
>[!NOTE]
>
>製品プロファイルは、Adobe Admin Consoleを通じて組織で管理され、詳細な機能に関する特定の権限のセットが含まれます。 製品プロファイルとその権限は、組織内の管理者権限を持つユーザーのみが管理できます。 API用に選択する製品プロファイルが不明な場合は、管理者に問い合わせてください。
...
アプリケーションに統合を使用している場合、APIリクエストを通じてプログラムによってアクセストークンを生成できます。 これを実現するには、次の値を取得する必要があります。
