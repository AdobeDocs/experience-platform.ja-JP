---
title: ビルドエンドポイント
description: Reactor APIで/buildsエンドポイントを呼び出す方法を説明します。
...
拡張機能、ルールおよびデータ要素は、Adobe Experience Platformのタグの構成要素です。 アプリケーションで何らかの処理を実行する場合、これらの構築ブロックは[ライブラリ](./libraries.md)に追加されます。 エクスペリエンスアプリケーションにライブラリをデプロイするために、ライブラリはビルドにコンパイルされます。 Reactor APIの`/builds`エンドポイントを使用すると、エクスペリエンスアプリケーション内のビルドをプログラムで管理できます。

ビルドとは、Webおよびモバイルアプリケーション内で読み込まれる実際のファイル（複数可）です。 各ビルドの内容は、次の要因に基づいて異なります。

* ライブラリに含まれるリソース
* ライブラリが構築される[環境](./environments.md)の設定
* ビルドが属する[プロパティ](./properties.md)のプラットフォーム

ビルドは、1つのライブラリに属しています。 ライブラリには多数のビルドを含めることができます。

ビルドと、タグの公開ワークフローに組み込まれる方法に関する一般的な情報については、[公開の概要](../../ui/publishing/overview.md)を参照してください。
...
ライブラリのIDをGETリクエストのパスに含めることで、特定のライブラリのビルドのリストを作成できます。
...
| パラメーター | 説明 |
| --- | --- |
| `LIBRARY_ID` | ビルドのリストを表示するライブラリの`id`。 |

{style=&quot;table-layout:auto&quot;}

>[!NOTE]
>
>クエリパラメーターを使用して、リストされたビルドを次の属性に基づいてフィルタリングできます。<ul><li>`created_at`</li><li>`status`</li><li>`token`</li><li>`updated_at`</li></ul>詳しくは、[応答のフィルタリング](../guides/filtering.md)に関するガイドを参照してください。
...
正常な応答は、指定されたライブラリのビルドのリストを返します。
...
ビルドを検索するには、ビルドリクエストのパスにIDをGETします。
...
ライブラリリクエストのパスにライブラリのIDを含む、ライブラリのビルドをPOSTできます。
...
次のリクエストは、リクエストパスで指定されたライブラリの新しいビルドを作成します。 リクエストペイロードは不要です。
...
ビルドを[公開済みのライブラリ](./libraries.md#publish)から再公開するには、PATCHリクエストのパスにIDを含めます。
...
| プロパティ | 説明 |
| --- | --- |
| `id` | 更新するビルドの`id`。 これは、リクエストパスで指定された`{BUILD_ID}`値と一致する必要があります。 |
| `type` | 更新するリソースのタイプ。 このエンドポイントでは、値を`builds`にする必要があります。 |
| `meta.action` | 実行するPATCHアクションのタイプ。 `republish`に設定する必要があります。 |

{style=&quot;table-layout:auto&quot;}

**応答**

正常な応答は、再公開されたビルドの詳細を返します。
...
## ビルドの関連リソースの取得 {#related}

次の呼び出しは、ビルドの関連リソースを取得する方法を示しています。 [ビルド](#lookup)を検索すると、これらの関係は`relationships`プロパティの下に表示されます。

Reactor APIの関係について詳しくは、[関係ガイド](../guides/relationships.md)を参照してください。

### ビルドの関連データ要素のリスト {#data-elements}

参照リクエストのパスに`/data_elements`を追加すると、ビルドに関連するデータ要素をリストできます。
...
### ビルドの関連する拡張機能のリスト {#extensions}

ルックアップリクエストのパスに`/extensions`を追加すると、ビルドの関連する拡張機能をリストできます。
...
参照リクエストのパスに`/rules`を追加すると、ビルドの関連ルールをリストできます。
...
参照リクエストのパスに`/library`を追加して、ビルドの関連ライブラリを取得できます。
...
### ビルドの関連環境の検索 {#environment}

参照リクエストのパスに`/environment`を追加することで、ビルドの関連環境を取得できます。
