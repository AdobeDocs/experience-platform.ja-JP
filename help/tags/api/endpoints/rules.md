...
データ収集タグのコンテキストでは、ルールはデプロイされたライブラリ内のリソースの動作を制御します。ルールは、1 つ以上の[ルールコンポーネント](./rule-components.md)で構成され、ルールコンポーネントを論理的に結び付けるために存在します。Reactor API の `/rules` エンドポイントを使用すると、タグルールをプログラムで管理できます。

>[!NOTE]
>
>このドキュメントでは、Reactor API でルールを管理する方法について説明します。データ収集 UI でルールを操作する方法については、[UI ガイド](../../ui/managing-resources/rules.md)を参照してください。

ルールは、1 つの[プロパティ](./properties.md)のみに属しています。プロパティには多くのルールがあります。
...
応答が成功すると、指定したプロパティのルールのリストが返されます。
...
GET リクエストのパスで ID を指定することで、ルールを検索できます。

>[!NOTE]
>
>ルールが削除されると、削除済みとマークされますが、実際にはシステムから削除されるわけではありません。 したがって、削除されたルールを取得することができます。 削除されたルールは、`meta.deleted_at` プロパティの存在によって識別できます。
...
POST リクエストをおこなうことで新しいルールを作成できます。
...
| プロパティ | 説明 |
| --- | --- |
| `attributes.name` | **（必須）**&#x200B;人間が判読できるルールの名前。 |
| `attributes.enabled` | ルールが有効かどうかを示すブール値。 |
| `type` | 作成するリソースのタイプ。このエンドポイントでは、値は `rules` である必要があります。 |
...
[ルールを作成したら](#create)、イベント、条件、アクション（総称してルールコンポーネント）を追加して、そのロジックの構築を開始できます。Reactor API でこれをおこなう方法については、`/rule_components` エンドポイントガイドの[ルールコンポーネントの作成](./rule-components.md#create)の節を参照してください。
...
| プロパティ | 説明 |
| --- | --- |
| `attributes` | ルールで、ルールの更新対象の属性を表すオブジェクト。 ルールで更新できる属性は次のとおりです。 <ul><li>`name`</li><li>`enabled`</li></ul> |
| `id` | 更新するルールの `id`。この値は、リクエストパスで指定された `{RULE_ID}` 値と一致する必要があります。 |
...
応答が成功すると、ルールが削除されたことを示す、応答本文のない HTTP ステータス 204（コンテンツなし）が返されます。

## ルールのメモの管理 {#notes}

ルールは「注目すべき」リソースのため、個々のリソースでテキストベースのメモを作成し、取得することができます。ルールや他の互換性のあるリソースのメモを管理する方法について詳しくは、[メモエンドポイントガイド](./notes.md)を参照してください。

## ルールに関連するリソースの取得 {#related}

次の呼び出しは、ルールの関連リソースを取得する方法を示しています。 [ルールを検索](#lookup)すると、これらの関係は `relationships` ルールの下に表示されます。
...
検索リクエストのパスに `/revisions` を追加して、ルールのリビジョンをリストできます。
...
応答が成功すると、指定したルールを使用するリビジョンのリストが返されます。
...
検索リクエストのパスに `/origin` を追加して、ルールのオリジン（以前のバージョン）を検索できます。
...
応答に成功すると、指定したルールの拡張機能の詳細が返されます。
...
### ルールに関連するプロパティの検索 {#property}

検索リクエストのパスに `/property` を追加して、ルールを所有するプロパティを検索できます。
...
応答が成功すると、指定したルールのプロパティの詳細が返されます。
