...
| `relationships` | ルールコンポーネントに必要な関係を確立するオブジェクト。次の 2 つの関係を確立する必要があります。 <ol><li>`extension`：このルールコンポーネントを定義する拡張機能。これは、`delegate_descriptor_id` で示される拡張機能のパッケージと同じにする必要があります。</li><li>`rules`：このコンポーネントが定義されているルール。リクエストパスで指定されているものと同じルール ID にする必要があります。</li></ol>関係に関する一般的な情報について詳しくは、[関係ガイド](../guides/relationships.md)を参照してください。 |
