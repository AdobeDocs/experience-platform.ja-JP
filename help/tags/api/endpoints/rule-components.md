...
データ収集タグでは、[ルール](./rules.md)は、デプロイ済みの[ライブラリ](./libraries.md)内のリソースの動作を制御します。**ルールコンポーネント**&#x200B;は、ルールを構成する個々のパーツです。ルールがレシピだとすると、ルールコンポーネントは食材の 1 つです。Reactor API の `/rule_components` エンドポイントを使用すると、ルールコンポーネントをプログラムで管理できます。
...
ルールコンポーネントには、次の 3 つの基本タイプがあります。

| ルールコンポーネントタイプ | 説明 |
| --- | --- |
| イベント | イベントは、ルールのトリガーです。 ルールは、クライアントデバイスで実行時にイベントが発生すると開始されます。「[!UICONTROL ライブラリ読み込み]」、「[!UICONTROL ページ上部]」、「[!UICONTROL クリック]」などがイベントの例です。 |
| 条件 | 条件とは、アクションが実行される前に、特定の基準を満たしているかどうかを評価するものです。イベントが発生すると、条件が評価されます。ルールのアクションは、すべての条件が満たされた場合にのみ実行されます。 |
| アクション | これらは、Adobe Analytics ビーコンの送信、カスタム訪問者 ID の取得、特定の mbox の実行など、ルールで実際に実行するアクションです。 |

{style=&quot;table-layout:auto&quot;}

ルールコンポーネントは、1 つのルールのみに属しています。ルールには、多くのルールコンポーネントを含めることができます（そして、含める必要があります）。
...
GET リクエストのパスにルールの ID を含めることにより、ルールに属するルールコンポーネントのリストを取得できます。
...
>[!NOTE]
>
>クエリパラメーターを使用し、リストされたルールコンポーネントを次の属性に基づいてフィルタリングできます。<ul><li>`created_at`</li><li>`dirty`</li><li>`enabled`</li><li>`name`</li><li>`negate`</li><li>`origin_id`</li><li>`published`</li><li>`published_at`</li><li>`revision_number`</li><li>`updated_at`</li></ul>詳しくは、[応答のフィルタリング](../guides/filtering.md)に関するガイドを参照してください。
...
応答が成功すると、指定されたルールのルールコンポーネントのリストが返されます。
...
GET リクエストのパスに ID を指定することで、ルールコンポーネントを検索できます。
...
| プロパティ | 説明 |
| --- | --- |
| `attributes.delegate_descriptor_id` | **（必須）**&#x200B;定義できるルールコンポーネントのタイプは、[拡張機能パッケージ](./extension-packages.md)によ って提供されます。新しいルールコンポーネントを作成する際は、このルールコンポーネントが基づいている拡張機能パッケージ、コンポーネントのタイプ（イベント、条件またはアクション）、拡張機能で定義されている特定のコンポーネントの名前（Core 拡張機能の「クリック」イベントなど）を示すデリゲート記述子 ID を指定する必要があります。<br><br>詳しくは、[デリゲート記述子 ID](../guides/delegate-descriptor-ids.md) に関するガイドを参照してください。 |
| `attributes.name` | **（必須）**&#x200B;ルールコンポーネントの人間がはんどくできる名前。 |
| `attributes.delay_next` | 後のアクションを遅らせるかどうかを示すブール値。 |
| `attributes.order` | タイプ別にコンポーネントを読み込む順序を示す整数。 |
| `attributes.rule_order` | 関連付けられたルールの起動に対する優先度を示す整数。 |
| `attributes.settings` | 文字列として表される設定 JSON オブジェクト。 |
| `attributes.timeout` | 順番に実行されるアクションのタイムアウトを示す整数。 |
| `relationships` | ルールコンポーネントに必要な関係を確立するオブジェクト。 次の 2 つの関係を確立する必要があります。 <ol><li>`extension`：このルールコンポーネントを定義する拡張機能。これは、拡張機能パッケージが `delegate_descriptor_id` で示されているのと同じ拡張機能である必要があります。</li><li>`rules`：このコンポーネントを定義するルール。リクエストパスで指定されているのと同じルール ID である必要があります。</li></ol>関係に関する一般的な情報について詳しくは、[関係に関するガイド](../guides/relationships.md)を参照してください。 |
| `type` | 作成するリソースのタイプ。このエンドポイントでは、値は `rule_components` である必要があります。 |
...
| プロパティ | 説明 |
| --- | --- |
| `attributes` | ルールコンポーネントが、ルールコンポーネント用に更新される属性を表すオブジェクト。 ルールコンポーネントで更新できる属性は次のとおりです。 <ul><li>`delay_next`</li><li>`delegate_descriptor_id`</li><li>`name`</li><li>`order`</li><li>`rule_order`</li><li>`settings`</li><li>`timeout`</li></ul> |
...
応答が成功すると、応答本文のない HTTP ステータス 204（コンテンツなし）が返され、ルールコンポーネントが削除されたことを示します。

## ルールコンポーネントのメモの管理 {#notes}

ルールコンポーネントは、「注目すべき」リソースです。つまり、個々のリソースでテキストベースのメモを作成し、取得することができます。 ルールコンポーネントや他の互換性のあるリソースのメモを管理する方法について詳しくは、[メモエンドポイントガイド](./notes.md)を参照してください。

## ルールコンポーネントの関連リソースの取得 {#related}

次の呼び出しは、ルールコンポーネントの関連リソースを取得する方法を示しています。[ルールコンポーネントを検索](#lookup)すると、これらの関係は `relationships` ルールコンポーネントの下に表示されます。

Reactor API の関係について詳しくは、[関係に関するガイド](../guides/relationships.md)を参照してください。

### ルールコンポーネントの関連ルールのリスト {#rules}

検索リクエストのパスに `/rules` を追加すると、特定のルールコンポーネントを利用するルールをリストできます。
...
応答が成功すると、指定されたルールコンポーネントを使用するルールのリストが返されます。
...
| パラメーター | 説明 |
| --- | --- |
| `{RULE_COMPONENT_ID}` | 拡張機能を検索するルールコンポーネントの `id`。 |
...
### ルールコンポーネントに関連するオリジンを検索する {#origin}

検索リクエストのパスに `/origin` を追加することで、ルールコンポーネントのオリジン（以前のリビジョン）を検索できます。
...
| パラメーター | 説明 |
| --- | --- |
| `{RULE_COMPONENT_ID}` | オリジンを検索するルールコンポーネントの `id`。 |
...
応答が成功すると、指定されたルールコンポーネントのオリジンの詳細が返されます。
