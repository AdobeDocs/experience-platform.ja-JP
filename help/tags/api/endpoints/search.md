...
応答が成功すると、クエリに一致するリソースのリストが返されます。API が特定の値の一致を決定する方法について詳しくは、「[一致規則](#conventions)」に関する付録の節を参照してください。
