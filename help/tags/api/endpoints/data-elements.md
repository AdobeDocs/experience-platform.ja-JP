---
title: データ要素エンドポイント
description: Reactor API で /data_elements エンドポイントを呼び出す方法について説明します。
...
データ要素は、アプリケーション内の重要なデータ部分を指す変数として機能します。データ要素は、[ルール](./rules.md)と[拡張機能](./extensions.md)の設定内で使用されます。ブラウザーまたはアプリケーションで実行時にルールがトリガーされると、データ要素の値が解決され、ルール内で使用されます。データ要素は、拡張機能の設定でも同じ機能を果たします。

複数のデータ要素を一緒に使用すると、データディクショナリまたはデータマップになります。このディクショナリは、Adobe Experience Platform が把握し、利用できるデータを表します。

データ要素は、1 つの[プロパティ](./properties.md)にのみ属します。プロパティには、多くのデータ要素を含めることができます。

データ要素とタグでのデータ要素の使用に関する一般的な情報については、UI ドキュメントの[データ要素のガイド](../../ui/managing-resources/data-elements.md)を参照してください。
...
プロパティのデータ要素のリストを取得するには、GET リクエストのパスにプロパティの ID を含めます。
...
| パラメーター | 説明 |
| --- | --- |
| `PROPERTY_ID` | データ要素を持つプロパティの `id`。 |

{style=&quot;table-layout:auto&quot;}

>[!NOTE]
>
>クエリパラメーターを使用し、リストに表示される会社を次の属性に基づいてフィルタリングできます。<ul><li>`created_at`</li><li>`dirty`</li><li>`enabled`</li><li>`name`</li><li>`origin_id`</li><li>`published`</li><li>`published_at`</li><li>`revision_number`</li><li>`updated_at`</li></ul>詳しくは、[応答のフィルタリング](../guides/filtering.md)に関するガイドを参照してください。
...
応答が成功すると、指定されたプロパティのデータ要素のリストが返されます。
...
データ要素を検索するには、GET リクエストのパスでその ID を指定します。

>[!NOTE]
>
>データ要素を削除すると、削除済みとマークされますが、実際にはシステムから削除されていません。このため、削除されたデータ要素を検索することができます。削除されたデータ要素は、`data.meta.deleted_at` 属性の存在によって識別できます。
...
新しいデータ要素を作成するには、POST リクエストを実行します。
...
次のリクエストでは、指定されたプロパティの新しいデータ要素が作成されます。また、この呼び出しでは、データ要素が `relationships` プロパティを通じて既存の拡張機能に関連付けられます。詳しくは、[関係](../guides/relationships.md)に関するガイドを参照してください。
...
| プロパティ | 説明 |
| --- | --- |
| `attributes.name` | **（必須）**&#x200B;人間が判読できるデータ要素の名前。 |
| `attributes.delegate_descriptor_id` | **（必須）**&#x200B;データ要素を拡張機能パッケージに関連付ける、書式設定された文字列。すべてのデータ要素は、最初に作成されるときに拡張パッケージに関連付ける必要があります。各拡張パッケージは、デリゲートデータ要素と互換性のあるタイプと、その意図された動作を定義しているからです。詳しくは、[デリゲート記述子 ID](../guides/delegate-descriptor-ids.md) に関するガイドを参照してください。 |
| `attributes.settings` | 文字列として表される設定 JSON オブジェクト。 |
| `attributes.default_value` | データ要素が `undefined` と評価された場合に返されるデフォルト値。 |
| `attributes.enabled` | データ要素が有効かどうかを示すブール値。 |
| `attributes.force_lower_case` | 保存する前にデータ要素の値を小文字に変換する必要があるかどうかを示すブール値。 |
| `attributes.clean_text` | 保存する前にデータ要素の値の先頭と末尾の空白を削除する必要があるかどうかを示すブール値。 |
...
データ要素を更新するには、PATCH リクエストのパスにその ID を含めます。
...
次のリクエストでは、既存のデータ要素の `name` が更新されます。
...
| プロパティ | 説明 |
| --- | --- |
| `attributes` | 更新するデータ要素の属性を表すプロパティを持つオブジェクト。データ要素のすべての属性を更新できます。属性のリストとその使用例については、[データ要素を作成](#create)するための呼び出し例を参照してください。 |
| `id` | 更新するデータ要素の `id`。この値は、リクエストパスで指定された `{DATA_ELEMENT_ID}` 値と一致する必要があります。 |
| `type` | 更新するリソースのタイプ。このエンドポイントの場合は、値を `data_elements` にする必要があります。 |
...
データ要素を変更すると、現在の（head）リビジョンを使用して新しいリビジョンのデータ要素が作成されます。データ要素の各リビジョンには、独自の ID が割り当てられます。元のデータ要素は、元のリンクを介して検出できます。

データ要素を変更するには、PATCH リクエストの本文で、`meta.action` プロパティに値 `revise` を指定します。
...
| --- | --- |
| `attributes` | 更新するデータ要素の属性を表すプロパティを持つオブジェクト。データ要素のすべての属性を更新できます。属性のリストとその使用例については、[データ要素を作成](#create)するための呼び出し例を参照してください。 |
| `meta.action` | 値 `revise` が含まれている場合、このプロパティは、データ要素の新しいリビジョンを作成する必要があることを示しています。 |
| `id` | 変更するデータ要素の `id`。この値は、リクエストパスで指定された `{DATA_ELEMENT_ID}` 値と一致する必要があります。 |
| `type` | 変更するリソースのタイプ。このエンドポイントの場合は、値を `data_elements` にする必要があります。 |

{style=&quot;table-layout:auto&quot;}

**応答**

応答が成功すると、増分された `meta.latest_revision_number` 属性が示すように、データ要素の新しいリビジョンの詳細が返されます。
...
データ要素を削除するには、DELETE リクエストのパスにその ID を含めます。
...
応答が成功すると、応答本文のない HTTP ステータス 204（コンテンツなし）が返され、データ要素が削除されたことを示します。

## データ要素のメモの管理 {#notes}

データ要素は「注目すべき」リソースのため、個々のリソースでテキストベースのメモを作成したり取得したりすることができます。データ要素およびその他の互換性のあるリソースのメモを管理する方法について詳しくは、[メモエンドポイントガイド](./notes.md)を参照してください。

## データ要素に関連するリソースの取得 {#related}

次の呼び出しは、データ要素に関連するリソースの取得方法を示しています。[データ要素を検索](#lookup)すると、`relationships` プロパティの下にこれらの関係がリストされます。

Reactor API の関係について詳しくは、[関係に関するガイド](../guides/relationships.md)を参照してください。

### データ要素に関連するライブラリのリスト {#libraries}

データ要素を利用するライブラリのリストを取得するには、参照リクエストのパスに `/libraries` を追加します。
...
応答が成功すると、指定されたデータ要素を使用するライブラリのリストが返されます。
...
データ要素の以前のリビジョンのリストを取得するには、参照リクエストのパスに `/revisions` を追加します。
...
### データ要素の関連拡張機能の検索 {#extension}

データ要素を利用する拡張機能を調べるには、GET リクエストのパスに `/extension` を追加します。
...
データ要素のオリジンを検索するには、GET リクエストのパスに `/origin` を追加します。データ要素のオリジンは、現在のリビジョンを作成するために更新された以前のリビジョンです。
...
応答が成功すると、指定したデータ要素のオリジンの詳細が返されます。
...
### データ要素の関連プロパティの検索 {#property}

データ要素を持つプロパティを検索するには、GET リクエストのパスに `/property` を追加します。
...
応答が成功すると、指定されたデータ要素を持つプロパティの詳細が返されます。
