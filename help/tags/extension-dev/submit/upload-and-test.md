...
>[!NOTE]
>
>アップロード時またはパッチ適用時に、拡張機能パッケージは保留状態になりますが、システムによりパッケージが非同期で抽出されデプロイされます。この処理がおこなわれている間に、 `extension_package` API を使用した、UI 内でのステータスの ID。 カタログ内に、「保留」とマークされた拡張機能カードが表示されます。
...
UI にログインし、「 」を選択した後、 **[!UICONTROL タグ]** 左のナビゲーションで、 [!UICONTROL プロパティ] 画面が表示されます。 プロパティは、デプロイするタグのコンテナであり、1 つまたは複数のサイトで使用できます。
...
1. リソースを変更します。拡張機能コンポーネントの設定が変更されている場合は、UI でこれらのリソースを更新する必要があります。
