...
イベントタイプライブラリモジュールは、アクティビティが発生したタイミングを検出し、関数を呼び出して関連ルールを実行するように設計されています。 検出されるイベントはカスタマイズ可能です。 例えば、は、ユーザーが特定のジェスチャーを行ったか、高速スクロールしたか、何かとやり取りしたかを検出できます。
...
タグを使用すると、ユーザーはルールを並べ替えることができます。 例えば、方向変更イベントタイプを使用する場合と、ルールの実行順序をカスタマイズする場合の両方のルールを作成できます。 Adobe Experience Platformユーザーが、ルールAの方向変更イベントに対して順序値`2`を指定し、ルールBの方向変更イベントに対して順序値`1`を指定したとします。これは、モバイルデバイスで向きが変更された場合、ルールAの前にルールBを実行する必要があることを示します（下位の値を持つルールが最初に実行されます）。
