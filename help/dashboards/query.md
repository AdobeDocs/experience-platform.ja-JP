...
# を使用したダッシュボードデータセットの調査、検証、処理 [!DNL Query Service]

Adobe Experience Platformは、Experience PlatformUI 内で使用可能なダッシュボードを通じて、組織のプロファイル、オーディエンス、宛先データに関する重要な情報を提供します。 その後、Adobe Experience Platformを使用できます [!DNL Query Service] データレイクでこれらのダッシュボードを機能させる未加工データセットを調査、検証、処理します。

## [!DNL Query Service] 入門

Adobe Experience Platform [!DNL Query Service] では、標準の SQL を使用してデータレイクのデータをクエリできるようにすることで、マーケターがデータからのインサイトを得るのをサポートしています。 [!DNL Query Service] は、データレイク内のデータセットを結合したり、クエリ結果を新しいデータセットとして取得したりして、レポートや機械学習で使用したり、リアルタイム顧客プロファイルに取り込んだりできる、ユーザーインターフェイスおよび API を提供します。

について詳しくは、 [!DNL Query Service] Experience Platformにおける役割については、まず次を読んでください。 [[!DNL Query Service] の概要](../query-service/home.md).

## 使用可能なデータセットへのアクセス

次を使用できます [!DNL Query Service] プロファイル、オーディエンス、宛先の各ダッシュボードに対して未加工データセットをクエリする場合。 使用可能なデータセットを表示するには、Experience PlatformUI で、 **データセット** 左側のナビゲーションでデータセットダッシュボードを開きます。 ダッシュボードリストは、組織で使用可能なすべてのデータセットを管理します。リストに表示された各データセットに関する詳細（名前、データセットが準拠するスキーマ、最新の取得実行のステータスなど）が表示されます。

![左側のナビゲーションで「データセット」タブがハイライトされたデータセット参照ダッシュボード。](./images/query/browse-datasets.png)
...
システム生成データセットを表示するには、フィルターアイコン（![フィルターアイコン。](./images/query/filter.png)）を選択します。

![フィルターアイコンがハイライト表示されたデータセットの「参照」タブ。](./images/query/filter-datasets.png)

サイドバーが表示され、2 つの切り替えが含まれています。 [!UICONTROL プロファイルに含める] および [!UICONTROL システムデータセットを表示]. の切り替えを選択します [!UICONTROL システムデータセットを表示] システム生成データセットを閲覧可能なデータセットのリストに含めるには、次の手順を実行します。

![システムデータセットを表示切替スイッチがハイライト表示されたデータセット 「参照」タブ。](./images/query/show-system-datasets.png)
...
CDP レポートは、プロファイルデータと、そのオーディエンスおよび宛先との関係に関するインサイトを提供します。 方法について詳しくは、CDP インサイトデータモデルのドキュメントを参照してください。 [cdp インサイトデータモデルを特定の KPI ユースケースに適用する](./data-models/cdp-insights-data-model-b2c.md).

## クエリの例

次のクエリの例には、で使用できる SQL のサンプルが含まれています [!DNL Query Service] ダッシュボードを機能させる未加工データセットを調査、検証、処理します。
...
このガイドを読むことで、以下を利用できるようになりました [!DNL Query Service] いくつかのクエリを実行して、プロファイル、オーディエンス、宛先の各ダッシュボードを機能させる未加工データセットを調査し、処理します。
