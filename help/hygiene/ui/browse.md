...
リストには、一度に 1 つのカテゴリの作業指示のみが表示されます。「**[!UICONTROL 消費者]**」を選択して消費者削除タスクのリストを表示し、「**[!UICONTROL データセット]**」を選択して、スケジュールされたデータセット有効期限のリストを表示します。
...
消費者による削除リクエストには、次のフィルターが適用されます。
...
消費者の削除リクエストの詳細には、現在のステータスとリクエストが行われてから経過した時間が含まれます。 各リクエストには、「**[!UICONTROL サービス別ステータス]**」セクションも含まれ、削除に関連する各ダウンストリームサービスの個々のステータスの詳細が表示されます。右側のパネルでは、コントロールを使用して、作業指示の名前と説明を更新できます。
