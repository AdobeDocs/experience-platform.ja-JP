...
>[!IMPORTANT]
> 
効率を改善し、データセット操作のコストを抑えるための継続的な変更の一環として、デルタ形式に移動された組織は、ID サービス、リアルタイム顧客プロファイル、データレイクからデータを削除できます。 このタイプ of ユーザーは、差分移行済みと呼ばれます。 差分移行された組織のユーザーは、1 つのデータセットまたはすべてのデータセットからレコードを削除することができます。 差分移行されていない組織のユーザーは、以下の図に示すように、1 つのデータセットまたはすべてのデータセットからレコードを削除することは選択できません。 この場合は、 [ID を提供](#provide-identities) 」の節を参照してください。
...
title="プライマリ ID"
abstract="プライマリ ID は、Experience Platform でレコードを消費者のプロファイルに結び付ける属性です。データセットのプライマリ ID フィールドは、そのデータセットが基づいているスキーマによって定義されます。この列では、型を指定する必要があります (or 名前空間 ) を使用して、次のようなレコードのプライマリ ID を取得できます。 `email` （電子メールアドレスの場合） `ecid` (Experience CloudID)。 詳しくは、データライフサイクル UI ガイドを参照してください。"

[!CONTEXTUALHELP]
id="platform_hygiene_identityvalue"
title="ID 値"
abstract="この列では、レコードのプライマリ ID の値（ID タイプに対応している必要があります）を指定する必要があります provided をクリックします。 プライマリ ID タイプの場合 is `email`の場合、値はレコードの電子メールアドレスにする必要があります。 詳しくは、データライフサイクル UI ガイドを参照してください。"
...
Platform のすべての ID フィールドと同様に、プライマリ ID は、**タイプ**（ID 名前空間とも呼ばれる）と&#x200B;**値**&#x200B;の 2 つで構成されます。ID タイプ provides フィールドがレコード（電子メールアドレスなど）を識別する方法に関するコンテキスト。値は、そのタイプに対するレコードの固有の ID を表します (for 例： `jdoe@example.com` （の） `email` id タイプ )。 Common id として使用されるフィールドには、アカウント情報、デバイス ID、cookie ID が含まれます。
...
1 つのデータセットからレコードを削除する場合は、指定する ID のタイプがすべて同じである必要があります。 since 1 つのデータセットに割り当てることができるプライマリ id は 1 つだけです。 すべてのデータセットから削除する場合は、複数の ID タイプを含めることができます since 異なるデータセットは、異なるプライマリ ID を持つ場合があります。
