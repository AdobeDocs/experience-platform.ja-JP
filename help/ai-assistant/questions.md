---
title: AI アシスタントの質問ガイド （レガシー）
description: このドキュメントでは、AI アシスタント（レガシー）をクエリする際に使用できる質問の例について説明します。
...
# AI アシスタントの質問ガイド （レガシー）

AI アシスタント（レガシー）のクエリ時に使用できる一連の質問例については、このドキュメントをに参照してください。

また、このドキュメントを使用して、AI アシスタント（レガシー）から最適な回答を得るための [&#x200B; 質問をフレーズ化する方法 &#x200B;](#phrasing-your-questions) に関するヒントを学習することもできます。

## 目標ベースの質問 {#objectives-questions}

次の質問の例は、AI アシスタント（レガシー）を使用する際に達成できる目標によってグループ化されています。

| 目的 | 説明 | 例 |
| --- | --- | --- |
| 学習の概念と継続的なワークフロー | <ul><li>初心者のユーザーは、AI アシスタント（レガシー）を使用してReal-Time CDPとAdobe Journey Optimizerの概念を学習し、なじみのない製品や機能に自分をオンボーディングできます。</li><li>経験豊富なユーザーは、AI アシスタント（レガシー）を使用して、ワークフローをブロックしている可能性のあるエッジケースを解決できます。 | <ul><li>ジャーニー分析でダッシュボードを設定するにはどうすればよいですか？</li><li>Real-Time CDPのユースケースをいくつか教えてください。</li></ul> |
| トラブルシューティング | ワークフローで発生する可能性のある基本的なエラーをデバッグする方法については、AI アシスタント（レガシー）を使用してください。 | <ul><li>このエラー {ERROR_MESSAGE} はどういう意味ですか？</li><li>「Luma：メールオーディエンス」という名前のオーディエンスを削除できないのはなぜですか？</li></ul> |
| サンドボックスハイジーン | AI アシスタント（レガシー）を使用して重複や未使用のオブジェクトを識別することで、サンドボックスを効率的に維持できます。 | <ul><li>類似のオーディエンスを表示できますか？</li><li>関連するデータセットを持たないスキーマはありますか？</li></ul> |
| 値の分析 | AI アシスタント（レガシー）を使用して、最も使用されているデータオブジェクトを特定し、パフォーマンスインジケーターを評価したり、最も価値のあるデータオブジェクトを見つけたりします。 | <ul><li>「Luma：メールオーディエンス」のセグメント定義にはプロファイルがいくつありますか？</li><li>オーディエンスがExperience Cloud Audiences の宛先に対してアクティブ化されたのはいつですか？</li></ul> |
| 検索 | AI アシスタント（従来）を使用して、オーディエンス、データセット、宛先、スキーマ、ソースなど、サポートされているExperience Platform オブジェクトを検索します。 | <ul><li>前四半期に作成した名前に「Luma」が含まれているオーディエンスをリストします。</li><li>「Luma：カスタムアクション」 XDM スキーマには何の属性がありますか。</li></ul> |
| 影響分析 | AI アシスタント（レガシー）を使用して、特定のワークフローで使用されているデータオブジェクトを識別し、変更の影響を評価できるようにします。 | <ul><li>「Luma:PersonProfiles」スキーマで `homeAddress.city` を使用するのは、どのオーディエンスですか？</li><li>`consents.marketing.push.val` プロファイル属性はどのデータセットに保存されますか。</li></ul> |
...
可能な限り正確に回答するには、質問を明確かつコンテキスト付きで AI アシスタント（レガシー）にフレーズ化する必要があります。 コンテキストを使用して明確な質問をする方法のガイダンスについては、次のヒントのリストを参照してください。

* タスクや質問を簡潔に述べます。
* あいまいな言語や複雑すぎる構文を避けて、理解を容易にします。
* AI アシスタント（レガシー）がより関連性の高い応答を生成できるように、タスクや質問に関する関連性の高いコンテキストを提供します。

AI アシスタント（レガシー）に質問する際に従うべきベストプラクティスについて詳しくは、以下の表を参照してください。

次の表に、AI アシスタント（レガシー）を使用する際に従うことのできるベストプラクティスを示します。

| 推奨 | 例 |
| --- | --- |
| <ul><li>取得または分析するオブジェクトや情報を具体的に指定します。</li><li>データオブジェクト名を引用符で囲んでみてください。 オブジェクト名の一部しか知らない場合は、それを質問で指定することもできます。</li><li>[&#x200B; オブジェクト オートコンプリート &#x200B;](./ui-guide.md#use-auto-complete) を使用すると、AI アシスタント（レガシー）がクエリのコンテキストをより深く理解できます。</li></ul> | <ul><li>「Luma - ロイヤルティ」スキーマを使用するデータセットはどれですか？</li><li>名前に「Luma」が含まれている、アクティブ化されたセグメントを表示します。 プロファイル数別にランク付けします。</li></ul> |
| <ul><li>曖昧さを避け、明確な言語を使用する</li><li>クエリをより明確にするために、正確な用語を使用します。</li><li>Adobe Experience Platformに関する質問をする場合は、回答の関連性を高めるために、Experience Platformに固有の用語を使用するようにしてください。</li></ul> | <ul><li>「ACME オーディエンス」のプロファイル数。</li><li>アクティブ化されたオーディエンスで使用された上位 5 つの XDM 属性を表示します。</li></ul> |
...
AI アシスタント（レガシー）で、ストレージサイズや行数などの特定のデータセット指標に関する質問に回答できるようになりました。
...
以下は、現在 AI アシスタント（レガシー）でサポートされていない質問の例のリストです。
...
このドキュメントでは、AI アシスタント（レガシー）の質問を最適化する方法について説明しました。 ワークフロー中にこの機能を使用する方法については、[AI アシスタント（レガシー） UI ガイド &#x200B;](ui-guide.md) を参照してください。
