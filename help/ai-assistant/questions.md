...
+++選択すると、提案されたプロンプトの追加リストが表示されます

* データの取り込み後にプロファイル対応スキーマを削除できますか？
* ユーザーグループと役割にはどのような関係がありますか？
* データタイプとフィールドグループのどちらを使用するべきか
* ID とプライマリキーまたは外部キーの違いは何ですか？」
* セグメントが評価され、結果がターゲティングに使用できるようになるまで、どれくらいかかりますか？
* プロファイル充実度の計算方法
* Real-Time CDPの一般的なユースケースを教えてください。
* セグメントを削除できないのはなぜですか？
* スキーマがプロファイルに対して有効になった後は、どのような変更を行えますか？
* AEP でダッシュボードにアクセスできないのはなぜですか？
* xdm 属性 {FIELD_PATH} ータを使用するセグメントの数
* 他のセグメントで使用されているセグメントはありますか？
* {SCHEMA_NAME} スキーマ内のどのフィールドに {STRING} が含まれていますか？
* セグメント定義に xdm 属性 {FULL_ATTRIBUTE_PATH} を含むオーディエンスはどれですか？
* xdm 属性 {FULL_ATTRIBUTE_PATH} を含むスキーマはどれですか？
* 使用されていないソースデータフローや、データが取り込まれなくなるソースデータフローはどれですか？
* ライブジャーニーのリストを作成します。
* ライブジャーニーで使用されるオーディエンスをリストします。
* どのソースがアクティブ状態ですか？
...
* このサンドボックス内のプロファイル [ 特定の条件（「カリフォルニアに住む」など）を満たす ] はいくつですか？
