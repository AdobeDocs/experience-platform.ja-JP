...
2. [!DNL Tags]組み込みのデータ要素機能を使用して、web ページの既存のデータレイヤー形式をXDMに再フォーマットします。
3. データ収集にデータ準備を使用して、web ページの既存のデータレイヤー形式を[!DNL Edge Network]経由でXDMに再フォーマットします。

このガイドでは、3つ目のオプションについて説明します。

## データ収集にデータ準備を使用する場合 {#when-to-use-data-prep}

データ収集用のデータ準備は、次の2つの状況で役立ちます。

1. このweb サイトには、適切に形成され、管理され、維持されたデータレイヤーがあり、JavaScript操作を使用してページ上のXDMに変換するのではなく、[!DNL Edge Network]に直接送信することをお勧めします（[!DNL Tags]個のデータ要素またはJavaScriptの手動による操作）。
2. [!DNL Tags]以外のタグ付けシステムがサイトに展開されています。

## Web SDK経由で既存のデータレイヤーをEdge Networkに送信する {#send-datalayer-via-websdk}

既存のデータレイヤーは、`sendEvent` コマンド内の[`data`](/help/collection/js/commands/sendevent/data.md) オブジェクトを使用して送信する必要があります。

[!DNL Tags]を使用している場合は、[**[!UICONTROL Send Event]**](/help/tags/extensions/client/web-sdk/actions/send-event.md) アクションタイプの&#x200B;**[!UICONTROL Data]** フィールドを使用する必要があります。

このガイドの残りの部分では、Web SDKからデータ層を送信した後に、データ層をXDM標準にマッピングする方法について説明します。

>[!NOTE]
>
>計算フィールドの変換関数を含む、すべての[!DNL Data Prep]機能に関する包括的なガイダンスについては、次のドキュメントを参照してください。
>
>* [データ準備の概要](/help/data-prep/home.md)
...
>[!NOTE]
>
>任意のページで使用できるすべてのデータレイヤー要素を表すサンプル JSON オブジェクトを使用します。 例えば、あらゆるページでショッピングカートのデータレイヤー要素が使用されているわけではありません。 ただし、このサンプル JSON オブジェクトにショッピングカートのデータレイヤー要素を含めます。
...
>[!IMPORTANT]
>
>[!DNL Data Prep] マッピングは`identityMap`個のXDM ペイロードを上書きし、[!DNL Real-Time CDP]個のオーディエンスに対するプロファイルのマッチングにさらに影響を与える可能性があります。
...
>[!NOTE]
>
>データストリームは、同じ[サンドボックス](/help/sandboxes/home.md)内でのみ読み込むことができます。 あるサンドボックスから別のサンドボックスにデータストリームをインポートすることはできません。
...
### マッピングの完了 {#complete-mapping}

残りのフィールドのターゲットスキーマへのマッピングを続行します。 利用可能なすべてのソースフィールドをマッピングする必要はありませんが、必要に応じて設定されたターゲットスキーマのフィールドは、この手順を完了するためにマッピングする必要があります。 **[!UICONTROL Required fields]** カウンターは、現在の設定でまだマッピングされていない必須フィールドの数を示します。

必須フィールド数が0に達し、マッピングに満足したら、**[!UICONTROL Save]**&#x200B;を選択して変更を確定します。

![必須フィールド数が0で正常にマッピングされたすべての必須フィールドを示すマッピングインターフェイス。](assets/data-prep/mapping-complete.png)
