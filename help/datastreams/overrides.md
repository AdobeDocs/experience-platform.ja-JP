...
データストリームの上書きの使用方法と使用タイミングをより深く理解できるように、Adobe Experience Platformのお客様がこの機能を使用して解決できる使用例を以下に示します。

**複数地域のデータ収集**

会社は、事業を行う国ごとに異なる Web サイトまたはサブドメインを持っています。 持っている [設定済み](configure.md) 対応する analytics 固有のレポートスイート、国固有のAdobe Targetプロパティトークン、国固有のスキーマ、データセット、Journey Optimizer設定などでデータストリームを分離します。 また、国固有のすべてのデータを集計するグローバルな設定も用意されています。

データストリームオーバーライドを使用すると、1 つのデータストリームにデータを送信するデフォルトの動作ではなく、異なるデータストリームにデータのフローを動的に切り替えることができます。

一般的な使用例としては、国固有のデータストリームにデータを送信したり、グローバルデータストリームにデータを送信したりできます。この場合、顧客は注文やユーザープロファイルの更新など、重要なアクションを実行します。
...
![Adobe Analyticsサービス設定を示すデータストリーム UI のスクリーンショット。レポートスイートのオーバーライドがハイライト表示されています。](assets/overrides/override-analytics.png)
...
![Adobe Experience Platformサービス設定を示すデータストリーム UI のスクリーンショット。イベントデータセットのオーバーライドが強調表示されています。](assets/overrides/override-aep.png)
