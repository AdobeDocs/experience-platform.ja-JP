...
データをAdobe Experience Platformにストリーミングする場合、多数のHTTP呼び出しを作成すると高コストになる場合があります。 例えば、1 KBのペイロードで200個のHTTPリクエストを作成する代わりに、200 KBのペイロードでそれぞれ1 KBのメッセージを200 KBずつ含む1個のHTTPリクエストを作成する方が、はるかに効率的です。 正しく使用すれば、1つのリクエスト内で複数のメッセージをグループ化することは、送信先のデータを最適化する優れた方法で [!DNL Experience Platform]す。

このドキュメントでは、ストリーミング取り込みを使用して1つのHTTPリクエスト内で複数のメッセージを送信す [!DNL Experience Platform] るためのチュートリアルを提供します。

## はじめに

このチュートリアルでは、Adobe Experience Platformに関する十分な理解が必要 [!DNL Data Ingestion]です。 このチュートリアルを開始する前に、次のドキュメントを確認してください。
...
また、APIの呼び出しを正しく行うために、 [Adobe Experience Platformへの認証のチュートリアルを完了している必要があり](../../tutorials/authentication.md)[!DNL Platform] ます。 このチュートリアルでは、すべてのAPI呼び出しに必要な認証ヘッダーの値を認証チュートリアルで説明します。 ヘッダーは、次のようなサンプル呼び出しに表示されます。
...
ストリーミングデータを開始する前に、まずストリーミング接続を作成する必要があり [!DNL Experience Platform]ます。 ストリーミング接続の作成方法については、『ストリーミング接続の [作成](./create-streaming-connection.md) 』ガイドを参照してください。
...
UIまたはAPIのリスト操作を使用して、既存のデータセットのIDを取得でき [!DNL Platform] ます。 データセットIDは、 [Experience Platform](https://platform.adobe.com) で「 **[!UICONTROL Datasets]** 」タブに移動し、IDを設定するデータセットをクリックして、「 **[!UICONTROL Dataset ID]** (データセットID **[!UICONTROL )」フィールド(]** Info)タブ)からコピーすることで確認できます。 APIを使用してデータセットを取得する方法について詳しくは、 [カタログサービスの概要](../../catalog/home.md) を参照してください。
...
- メッセージヘッダーの `imsOrgId` フィールドは、インレット定義と一致する必要があります。 要求ペイロードにフィールドが含まれていない場合は、 `imsOrgId` (DCCS [!DNL Data Collection Core Service] )がフィールドを自動的に追加します。
- メッセージのヘッダーは、 [!DNL Platform] UIで作成された既存のXDMスキーマを参照する必要があります。
- この `datasetId` フィールドでは、の既存のデータセットを参照する必要があり [!DNL Platform]、そのスキーマは、リクエストの本文に含まれる各メッセージ内で、 `header` オブジェクトに指定されたスキーマと一致する必要があります。
...
最初のメッセージは、に正常に送信され [!DNL Platform] 、他のメッセージの結果の影響を受けません。 その結果、失敗したメッセージを再送信する場合は、このメッセージを再度含める必要はありません。
...
3つ目のメッセージは、ヘッダーで無効なIMS組織IDが使用されているために失敗しました。 IMS組織は、投稿先の{CONNECTION_ID}と一致する必要があります。 使用しているストリーミング接続に一致するIMS組織IDを特定するには、を使用して `GET inlet` 要求を実行し [!DNL Data Ingestion API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/ingest-api.yaml)ます。 以前に作成したストリーミング接続の取得方法の例については、 [](./create-streaming-connection.md#get-data-collection-url) ストリーミング接続の取得を参照してください。

4番目のメッセージは、予期されたXDMスキーマに従わなかったため失敗しました。 要求のヘッダーと本文 `xdmSchema` に含まれるIDが、のXDMスキーマと一致しません `{DATASET_ID}`。 メッセージヘッダーと本文のスキーマを修正すると、DCCS検証に合格し、に正常に送信でき [!DNL Platform]ます。 また、メッセージの本文を更新して、のXDMスキーマに一致させ、ストリーミング検証を渡す必要 `{DATASET_ID}` があり [!DNL Platform]ます。 Platformに正常に送信されるメッセージに対して何が起こるかについての詳細は、このチュートリアルの「取り込まれた [メッセージを](#confirm-messages-ingested) 確認する」の節を参照してください。

### 失敗したメッセージの取得元 [!DNL Platform]
...
DCCS検証に合格したメッセージは、にストリーミングされ [!DNL Platform]ます。 オン [!DNL Platform]の場合、バッチメッセージは、に取り込まれる前に、ストリーミング検証によってテストされ [!DNL Data Lake]ます。 バッチのステータス（成功したかどうか）は、で指定したデータセット内に表示され `{DATASET_ID}`ます。

「データセット [!DNL Platform] 」タブに移動し、ストリーミング先のデータセットをクリックし、「データセット [アクティビティ](https://platform.adobe.com)******** 」タブをチェックすると、Experience PlatformUIを使用して、正常にストリーミングされるバッチメッセージのステータスを表示できます。

でのストリーミング検証に合格したバッチメッセージ [!DNL Platform] は、に取り込まれ [!DNL Data Lake]ます。 その後、メッセージを分析または書き出しで使用できます。

## 次の手順

1回のリクエストで複数のメッセージを送信する方法と、メッセージがターゲットデータセットに正しく取り込まれたことを確認できるようになったので、独自のデータを開始ストリーミングに送信でき [!DNL Platform]ます。 から取り込んだデータをクエリおよび取り込む方法の概要につ [!DNL Platform]いては、『 』 [!DNL Data Access](../../data-access/tutorials/dataset-data.md) ガイドを参照してください。
...
| 500 | ペイロードの処理中にエラーが発生しました。 より具体的なエラーメッセージについては、応答本文を参照してください(例えば、Message payloadスキーマが指定されていないか、のXDM定義と一致しませんでした [!DNL Platform])。 |
