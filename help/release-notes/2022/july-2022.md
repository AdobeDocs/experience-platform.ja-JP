...
## CDP B2B(#lead-to-account-matching) でのアカウントマッチングにつながる

B2B 企業では、多くの場合、顧客情報が複数のシステムに保存されており、それぞれのシステムには、同じ実世界のビジネスエンティティに関するデータの一部のみ、または矛盾するデータが含まれています。そのため、顧客を正確に把握することが難しく、B2B マーケティングや営業活動の効率や効果を低下させるという大きな課題を抱えています。リードのアカウント照合のリリースに伴い、CDP B2B は既知の人物プロファイルをアカウントプロファイルに照合し、アカウントやオポチュニティなどの B2B コンテキストデータでセグメント化し、ターゲット化できるようにします。

この機能について詳しくは、次のドキュメントページを参照してください。

- [CDP B2B でのアカウントマッチングにつながる](../../rtcdp/b2b-ai-ml-services/lead-to-account-matching.md)
- [モニタープロファイルエンリッチメントガイドのアカウントマッチングプロファイルエンリッチメントへのリード](../../dataflows/ui/b2b/monitor-profile-enrichment.md)
