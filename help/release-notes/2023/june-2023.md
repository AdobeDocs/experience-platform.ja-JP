...
| [!BADGE ベータ版]の {type=Informative} サポート [!DNL RainFocus] | [!DNL RainFocus] ソース統合を使用して、[!DNL RainFocus] アカウントから Experience Platform にイベント管理および分析データを取り込むことができるようになりました。詳しくは、[[!DNL RainFocus]  ソースの概要](../../sources/connectors/analytics/rainfocus.md)を参照してください。 |
