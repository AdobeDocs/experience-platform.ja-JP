...
カタログサービスは、Adobe Experience Platform 内のデータの場所と系列のレコードのシステムです。Experience Platform に取得されるすべてのデータはファイルとディレクトリとしてデータレイクに保存されますが、カタログには、参照やモニタリングのために、これらのファイルとディレクトリのメタデータと説明が保持されます。
...
| [[!DNL Data Landing Zone]](../../destinations/catalog/cloud-storage/data-landing-zone.md) | 新規 | [!DNL Data Landing Zone]は、Adobe Experience Platformによってプロビジョニングされた[!DNL Azure Blob] ストレージインターフェイスです。これにより、安全なクラウドベースのファイルストレージ機能にアクセスし、Experience Platformからファイルを書き出すことができます。 この新しい宛先は、強化されたファイル書き出し機能を提供し、[!BADGE Beta]{type=Informative} データセットの書き出しをサポートします。 |
...
| [[!DNL Amazon S3] 更新](../../destinations/catalog/cloud-storage/amazon-s3.md#changelog) | 更新済み | このアップデートにより、宛先は強化されたファイル書き出し機能を提供し、[!BADGE Beta]{type=Informative} データセットの書き出しをサポートします。 |
| [[!DNL Azure Blob] 更新](../../destinations/catalog/cloud-storage/azure-blob.md#changelog) | 更新済み | このアップデートにより、宛先は強化されたファイル書き出し機能を提供し、[!BADGE Beta]{type=Informative} データセットの書き出しをサポートします。 |
| [[!DNL SFTP] 更新](../../destinations/catalog/cloud-storage/sftp.md#changelog) | 更新済み | このアップデートにより、宛先は強化されたファイル書き出し機能を提供し、[!BADGE Beta]{type=Informative} データセットの書き出しをサポートします。 |
...
- 既にサポートされているCSV形式に加えて、オーディエンスを[JSONまたはParquet ファイル ](/help/destinations/ui/activate-batch-profile-destinations.md#supported-file-formats-export)として書き出せるようになりました。
...
| 機能 | 説明 |
| ------- | ----------- |
| オーディエンスポータル | オーディエンスポータルは、Adobe Experience Platform のオーディエンスへのアクセス、作成、管理を行うための新しいブラウジングエクスペリエンスを提供します。Audience Portal内では、Experience Platformで生成されたオーディエンスと外部で生成されたオーディエンスの表示、フィルタリング、フォルダー、タグによる作業効率の向上、Experience Platformで生成されたオーディエンスの作成、CSV ファイルを介した外部で生成されたオーディエンスの読み込みができます。 オーディエンスポータルについて詳しくは、[オーディエンスポータルの概要](../../segmentation/ui/audience-portal.md)を参照してください。 |
...
| コンポーネントのタイプ | 名前 | 説明 |
| --- | --- | --- |
| クラス | [[!UICONTROL XDM Individual Prospect Profile]](https://github.com/adobe/xdm/pull/1758/files) | このクラスを使用して、データベンダーの上位顧客獲得の事例から得た見込み客プロファイルを取り込みます。 |
| フィールドグループ | [[!UICONTROL Enriched Event Segment Details]](https://github.com/adobe/xdm/pull/1754/files) | イベント収集時にプロファイルが対象となるオーディエンスのリスト。 |
...
| --- | --- | --- |
| フィールドグループ | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は experimental から `stable` に更新されました。 |
| フィールドグループ | [[!UICONTROL Media Interaction Details]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `stable` から `deprecated` に更新されました。 |
| データタイプ | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `experimental` から `stable` に更新されました。 |
| データタイプ | [[!UICONTROL Qoe Data details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `experimental` から `stable` に更新されました。 |
| データタイプ | [[!UICONTROL Player state data information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `experimental`から `stable` に更新されました。 |
| データタイプ | [[!UICONTROL Media event information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `experimental` から `stable` に更新されました。 |
| データタイプ | [[!UICONTROL Media details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `experimental` から `stable` に更新されました。 |
| データタイプ | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `experimental` から `stable` に更新されました。 |
| データタイプ | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `stable` から `deprecated` に更新されました。 |
| データタイプ | [[!UICONTROL Custom metadata details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `experimental` から `stable` に更新されました。 |
| データタイプ | [[!UICONTROL Chapter details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `experimental` から `stable` に更新されました。 |
| データタイプ | [[!UICONTROL Advertising Pod details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `experimental` から `stable` に更新されました。 |
| データタイプ | [[!UICONTROL Advertising details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` は `experimental` から `stable` に更新されました。 |
| 拡張機能（カスタマージャーニー管理） | [[!UICONTROL Domain]](https://github.com/adobe/xdm/pull/1756/files) | 受信者の電子メールアドレスのドメインを記録するために、`Domain` フィールドが[!UICONTROL Adobe CJM ExperienceEvent - Message Profile Details]に追加されました。 |
| 拡張機能（カスタマージャーニー管理） | [[!UICONTROL Channel's variant Name]](https://github.com/adobe/xdm/pull/1753/files) | このフィールドは、チャネル バリアント名を表すために[!UICONTROL AJO Entity Fields]に追加されました。 |
| 拡張機能（Adobe Analytics） | [[!UICONTROL Context value]](https://github.com/adobe/xdm/pull/1761/files) | `Context value` が [!UICONTROL `Adobe Analytics ExperienceEvent Full Extension`] に追加されました。 |
