...
| 機能 | 説明 |
| ------- | ----------- |
| IAB TCF 2.0 のサポート | [!DNL Real-Time CDP] （IAB）で概要が説明されているように、[!DNL Transparency & Consent Framework] は [!DNL Interactive Advertising Bureau] （TCF）の 2.0 バージョンの登録済みベンダーになりました。 CMP が生成した顧客同意データを受け入れるようにデータ操作とプロファイルスキーマを設定し、ダウンストリーム宛先に対してセグメントをアクティブ化する際に顧客の同意環境設定を適用できます。 |
