...
| レシピ | 新しい PySpark と [!DNL Spark] のレシピは、[!DNL Docker] と R のレシピと同様の [!DNL Python] ワークフローに従うようになりました。 |
...
| Adobe Audience Managerから [!DNL Experience Platform] へのデータ共有 | [!DNL Audience Manager] コネクタのプロビジョニングプロセスが更新されました。 [!DNL Audience Manager] の [!DNL Real-Time Customer Profile] データセットは、デフォルトで無効になりました。 [!DNL Profile] に昇格させるデータセットを手動で選択できます。 新しいデフォルト設定は遡及的ではなく、新しい [!DNL Audience Manager] コネクタのプロビジョニングにのみ影響します。 詳しくは、[ データセットユーザーガイド ](../../catalog/datasets/user-guide.md) を参照してください。 |
