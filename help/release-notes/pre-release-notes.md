...
| 宛先 | 説明 |
| --- | --- |
| [Snowflake ストリーミング ](../destinations/catalog/warehouses/snowflake.md) マルチリージョンのサポート | Snowflake ストリーミングコネクタが、米国 VA7 リージョン以外のお客様も使用できるようになりました。 地域ドロップダウンセレクターを使用して、アカウントを含むSnowflake地域を選択します。 このドキュメントは、Snowflake ストリーミングテーブルに必要なデータ構造を追加して更新されました。 |
| [Snowflake ストリーミング ](../destinations/catalog/warehouses/snowflake.md) および [Snowflake バッチ ](../destinations/catalog/warehouses/snowflake-batch.md) 地域セレクター | 検索とドロップダウンを 1 つのコントロールに組み合わせた新しい検索可能なドロップダウンを使用して、地域をより簡単に見つけられるようになりました。 |
...
| アクティベーションワークフローでのオーディエンスフィルタリング | オーディエンスページと同じエクスペリエンスで、**[!UICONTROL Select audiences]** ステップでオーディエンスを検索およびフィルタリングできるようになりました。例えば、オーディエンスオリジンでフィルタリングして、探しているオーディエンスを簡単に見つけることができます。 |
