...
| 機能 | 説明 |
| ------- | ----------- |
| 編集可能 API オーディエンス | Segmentation Service API またはセグメントビルダーのAudience Agentを使用して作成されたオーディエンスを編集できるようになりました。 **注意：** 時系列データを含むオーディエンス **編集できません** 現時点では、セグメントビルダーを使用して編集できます。 |
| 外部オーディエンスデータの有効期限の更新 | Segmentation Service API を使用して、外部オーディエンスのデータ有効期限を拡張できます。 詳しくは、[ 外部オーディエンス API ガイド ](/help/segmentation/api/external-audiences.md#extend-data-expiration) を参照してください。 |
