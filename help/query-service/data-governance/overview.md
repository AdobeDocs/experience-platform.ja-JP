...
Adobe Experience Platformのアクセス制御では、[Adobe Admin Console](https://adminconsole.adobe.com/)を使用して、ロールベースの権限を使用してクエリサービス機能へのアクセスを管理できます。 同様に、スキーマとデータフィールドのラベル管理を通じて、特定のデータ属性へのアクセスを制御できます。
...
サードパーティクライアントを使用してQuery Serviceにアクセスするには、認証に資格情報が必要です。 これらの資格情報は、互換性のある外部クライアントのいずれかでクエリサービスにアクセスするために必須です。 期限切れの資格情報[または](#expiring-credentials)期限切れでない資格情報[を使用して、外部クライアントに接続できます。](#non-expiring-credentials)
...
期限切れでない資格情報で許可されたテクニカルユーザーアカウントには、役割を割り当てて、その責任とニーズに基づいて読み取りおよび書き込みアクセスの範囲を定義することで、適切なデータガバナンスを確保することができます。 アクセス制御[による役割ベースの権限を使用してクエリサービスへのアクセスを管理する](#access-control)については、前述の節を参照してください。
...
[ SSL パラメーター値を使用して接続する方法など、詳しくは、サードパーティクライアントからQuery Serviceへの接続に使用できる](../clients/ssl-modes.md)SSL オプションに関するガイドを参照してください。`verify-full`
...
クエリログには、[!UICONTROL Logs] ワークスペースの「[!UICONTROL Queries]」タブにあるExperience Platform UIからアクセスできます。
...
スキーマデータフィールドは、Experience Platform UIとクエリサービスを使用してID フィールドとして設定できます。また、[SQL コマンド「ALTER TABLE」 ](../sql/syntax.md#alter-table)を使用してプライマリ IDをマークすることもできます。 `ALTER TABLE` コマンドを使用してIDを設定することは、Experience Platform UIを使用してスキーマから直接データセットを作成するのではなく、SQLを使用してデータセットを作成する場合に特に便利です。 標準スキーマを使用する場合にUI[でID フィールドを](../../xdm/ui/fields/identity.md)定義する方法については、ドキュメントを参照してください。
