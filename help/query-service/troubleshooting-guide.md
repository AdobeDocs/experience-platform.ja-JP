...
### 使用可能な [!DNL Postman] クエリサービス API の

+++回答はい。 [!DNL Postman] （無料のサードパーティ製アプリケーション） 次を監視： [[!DNL Postman] 設定ガイド](https://video.tv.adobe.com/v/28832) Adobe Developer Console でプロジェクトを設定し、で使用するために必要な資格情報をすべて取得する手順については、 [!DNL Postman]. 以下の公式ドキュメントを参照してください。 [開始、実行、共有に関するガイダンス [!DNL Postman] コレクション](https://learning.postman.com/docs/running-collections/intro-to-collection-runs/).
+++
...
+++回答バッチクエリでは、データセット内の行の更新はサポートされません。
+++
...
+++回答出力行の制限を回避するには、クエリに「LIMIT 0」を適用します。 以下に例を示します。
...
### 列名に予約済みのキーワードを使用できますか。

+++回答列名として使用できない特定の予約済みキーワードがあります。例： `ORDER`, `GROUP BY`, `WHERE`, `DISTINCT`. これらのキーワードを使用する場合は、これらの列をエスケープする必要があります。
+++
...
次に、 [!DNL Query Editor]. [!DNL Query Service] API には適用されません：<br/>既に作成、保存、実行されたクエリにのみスケジュールを追加できます。<br/>パラメーター化クエリにスケジュールを追加することは&#x200B;**できません**。<br/>スケジュールされたクエリに匿名ブロックを含めることは&#x200B;**できません**。<br/>スケジュールできるのは **1 つ** UI を使用したクエリテンプレート。 クエリテンプレートにスケジュールを追加する場合は、API を使用する必要があります。 API を使用してスケジュールが既に追加されている場合、UI を使用してスケジュールを追加することはできません。
+++
...
### 一度に実行できる同時クエリの数はいくつですか？

+++回答バッチクエリがバックエンドジョブとして実行されるので、クエリの同時実行制限はありません。 ただし、クエリのタイムアウト制限は 24 時間に設定されています。
+++

### クエリのアクティビティとステータスを確認できるアクティビティダッシュボードはありますか？

+++回答クエリのアクティビティとステータスを確認する監視および警告機能があります。 詳しくは、 [クエリサービス監査ログの統合](./data-governance/audit-log-guide.md) そして [クエリログ](./ui/overview.md#log) ドキュメントを参照してください。
+++

### 更新をロールバックする方法はありますか？ 例えば、データを Platform に書き戻す際に、エラーや一部の計算を再設定する必要がある場合、そのシナリオはどのように処理する必要がありますか？

+++回答現在、その方法でのロールバックや更新はサポートされていません。
+++

### Adobe Experience Platformでクエリを最適化するにはどうすればよいですか？

+++回答システムはデータベースではないのでインデックスを持っていませんが、データストアに関連付けられた他の最適化が行われています。 次のオプションを使用して、クエリを調整できます。

- 時系列データに基づく時間ベースのフィルター。
- 構造体データタイプのプッシュダウンを最適化しました。
- 配列とマップのデータ型のコストとメモリのプッシュダウンを最適化。
- スナップショットを使用した増分処理。
- 永続化されたデータ形式。
+++

### ログインをクエリサービスの特定の側面に制限できますか。それとも、「すべて」または「何も」のソリューションですか。

+++回答クエリサービスは、「すべてまたは何も」のソリューションです。 部分的なアクセスを提供できません。
+++

### Adobe Query Service で使用できるデータを制限できますか。それとも、Adobe Experience Platformデータレイク全体に単にアクセスするだけですか。

+++回答はい。読み取り専用アクセス権を持つデータセットに対するクエリを制限できます。
+++

### クエリサービスがアクセスできるデータを制限するその他のオプションはありますか？

+++回答アクセスを制限する方法は 3 つあります。 次のようになります。

- SELECT のみのステートメントを使用し、データセットに読み取り専用アクセス権を付与します。 また、クエリの管理権限も割り当てます。
- SELECT/INSERT/CREATE 文を使用して、データセットに書き込みアクセス権を付与します。 また、クエリ管理権限を割り当てます。
- 上記の前述の提案に従って統合アカウントを使用し、クエリ統合権限を割り当てます。

+++

### クエリサービスがデータを返した後、保護されたデータが返されていないことを確認するために、Platform が実行できるチェックはありますか？

- クエリサービスは、属性ベースのアクセス制御をサポートします。 データへのアクセスは、列/リーフレベルや構造体レベルで制限できます。 属性ベースのアクセス制御の詳細については、ドキュメントを参照してください。

### サードパーティのクライアントへの接続に SSL モードを指定できますか？ 例えば、「verify-full」をPower BIと共に使用できますか？

+++回答はい、SSL モードがサポートされています。 詳しくは、 [SSL モードのドキュメント](./clients/ssl-modes.md) 使用可能な様々な SSL モードの分類と保護レベルについて説明します。
+++

### TLS 1.2 を使用して、Power BIクライアントからクエリサービスへのすべての接続を実行していますか？

+++答えはい。 送信中のデータは、常に HTTPS に準拠しています。 現在サポートされているバージョンは TLS1.2 です。
+++

### ポート 80 で行われた接続では、まだ https が使用されますか？

+++答えはい、ポート 80 で行われた接続では SSL が使用されます。 また、ポート 5432 を使用することもできます。
+++

### 特定の接続の特定のデータセットや列へのアクセスを制御することはできますか？ これはどのように設定されますか？

+++回答はい。設定に応じて、属性ベースのアクセス制御が適用されます。 詳しくは、 [属性ベースのアクセス制御の概要](../access-control/abac/overview.md) を参照してください。
+++
...
+++回答この問題の一般的な原因は、時間フィルターを使用せずに時系列データを問い合わせることです。 以下に例を示します。
