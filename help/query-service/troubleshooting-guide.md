...
+++回答
考えられる原因の 1 つは、オートコンプリート機能です。 この機能は、特定のメタデータコマンドを処理するので、クエリの編集中にエディターの速度が遅くなることがあります。
+++
...
+++回答
特定のクエリのエラーログを取得するには、まず Query Service API を使用してクエリログの詳細を取得する必要があります。 HTTP 応答には、クエリエラーの調査に必要なクエリ ID が含まれています。
...
+++回答
[`SNAPSHOT`](./sql/syntax.md#snapshot-clause) 句を使用すると、スナップショット ID に基づいて、テーブルのデータを増分的に読み取ることができます。 これは、最後の読み込み実行以降に作成または変更されたデータセット内の情報のみを処理する[増分読み込み](./key-concepts/incremental-load.md)デザインパターンで使用するのに最適です。その結果、処理効率が向上し、ストリーミングデータ処理とバッチデータ処理の両方で使用できます。
+++
...
+++回答
データのセキュリティに関する考慮事項により、UDF のカスタム定義は許可されていません。
+++
...
+++回答
「セッション制限に達しました」は、組織で許可されているクエリサービスセッションの最大数に達したことを意味します。 組織の Adobe Experience Platform 管理者に連絡してください。
+++

### クエリログでは、削除されたデータセットに関連するクエリをどのように処理しますか？

+++回答
クエリサービスはクエリ履歴を削除しません。 つまり、削除されたデータセットを参照するクエリが結果として、「有効なデータセットがありません」を返します。
+++
...
+++回答
Adobe Experience Platformは、データを UTC （協定世界時）タイムスタンプ形式で保持します。 UTC 形式の例は `2021-12-22T19:52:05Z` です
...
この `from_utc_timestamp()` メソッドは、指定されたパラメーターを&#x200B;**ローカルタイムゾーンのタイムスタンプから**&#x200B;解釈し、目的の地域の同等のタイムスタンプを UTC 形式で提供します。 次の例では、時間はユーザーのローカルタイムゾーンの 2:40PM です。 変数として渡されるソウルのタイムゾーンは、ローカルタイムゾーンの 9 時間前です。
...
+++回答
時系列データのクエリを実行する場合は、可能な限りタイムスタンプフィルターを使用して、より正確な分析を行う必要があります。
...
+++回答
`CAST` 演算子を使用してタイムスタンプを変換する場合は、日付 **および時間** 両方を含める必要があります。
...
+++回答
いいえ。これは、クエリサービスを含むすべてのAdobe サービスに適用される、Experience Platform全体にわたる意図的な制限です。 スキーマ名とデータセット名にはアンダースコアを 2 つ入れることができますが、データセットのテーブル名には 1 つしか入れられません。
+++
...
+++回答
はい。読み取り専用アクセス権を持つデータセットのみにクエリ実行対象を制限できます。
+++
...
### ポート 80 で確立された接続では、引き続き HTTPS と TLS 暗号化を使用しますか？

+++回答
はい。ポート 80 上の接続は TLS 暗号化を使用して保護されており、サービスで TLS 強制が必要になります。 プレーン HTTP 接続は受け付けられません。 特定の顧客ネットワークポリシーに対応するために、ポート 80 がサポートされています。 組織がポート 80 をブロックする場合は、代わりにポート 5432 を使用します。 両方のポートに TLS が必要で、同じセキュリティポスチャが提供されます。
+++

### Adobeのクエリサービスでは、暗号化されていない HTTP （ポート 80）経由でデータを公開しますか？

+++回答
いいえ。ポート 80 の接続には TLS が必要で、プレーンテキストの HTTP 要求はサーバーサイドで拒否されます。 ポート 5432 もサポートされており、TLS 暗号化されています。
+++

### Query Service と Data Distillerにポート 80 を使用する従来の設定はありますか？

+++回答
いいえ。必須の TLS を使用するポート 80 は、特定のネットワーク要件を持つお客様向けに設計されたサポートされている設定です。 レガシーモードや安全でないモードではありません。 環境がポート 80 での送信接続を制限する場合は、代わりにポート 5432 を使用します。両方のポートで TLS が強制されています。
+++

### Power BI クライアントからクエリサービスへのすべての接続に対して TLS 1.2 が使用されていますか？

+++回答
はい。送信中のデータは常に HTTPS で保護され、現在サポートされているバージョンは TLS 1.2 です。クエリサービスへのPower BI接続には、すべて暗号化されたトランスポートが必要です。
+++

### データDistillerでポート 80 を使用する場合、暗号化されないのですか？

+++回答
いいえ。データDistillerは、ポート 80 で TLS を適用し、プレーンテキストの HTTP リクエストをすべて拒否します。 ポート 5432 もサポートされており、TLS 暗号化されています。
+++

### Query Service または Data Distillerでポート 80 を使用する場合、リスクや制限はありますか？

+++回答
はい。ポート 80 では TLS が強制されており、暗号化されていない接続はサポートされていません。 一部の組織では、ポリシー制限のためにポート 80 での送信トラフィックをブロックしています。 これがネットワークに当てはまる場合は、代わりにポート 5432 を使用してください。 どの場合も TLS が必要なので、両方のポートで同じレベルのセキュリティが提供されます。
+++
...
+++回答
有効期限のない資格情報の値は、`technicalAccountID` からの連結引数と、設定 JSON ファイルから取得された `credential` です。 パスワードの値は `{{technicalAccountId}:{credential}}` 形式で指定します。
 [資格情報を使用して外部クライアントに接続](./ui/credentials.md#using-credentials-to-connect-to-external-clients)する方法について詳しくは、ドキュメントを参照してください。
+++

### 有効期限のない資格情報のパスワードに対する特殊文字に関する制限はありますか？

+++回答
はい。有効期限のない認証情報のパスワードを設定する場合、少なくとも 1 つの数字、1 つの小文字、1 つの大文字、1 つの特殊文字を含める必要があります。 ドル記号（$）はサポートされていません。 !、@、#、^、&amp;などの特殊文字を使用してください。
+++

### クエリサービスエディターに接続できるサードパーティの SQL エディターの種類を教えてください。

+++回答
PSQL またはクライアントに準拠したサードパーティ製の SQL エディター [!DNL Postgres]、クエリサービスエディターに接続できます。 使用可能な手順のリストについては、[クエリサービスへのクライアントの接続](./clients/overview.md)のドキュメントを参照してください。
+++

### Power BI ツールをクエリサービスに接続できますか？

+++回答
はい、Power BIをクエリサービスに接続できます。 [Power BI デスクトップアプリをクエリサービスに接続する手順](./clients/power-bi.md)については、ドキュメントを参照してください。
+++

### クエリサービスに接続すると、ダッシュボードの読み込みに時間がかかるのはなぜですか？

+++回答
システムがクエリサービスに接続されると、インタラクティブまたはバッチ処理エンジンに接続されます。 その結果、処理されたデータを反映するための読み込み時間が長くなる可能性があります。
