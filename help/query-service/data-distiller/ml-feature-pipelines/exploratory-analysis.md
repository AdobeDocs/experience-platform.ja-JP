...
このドキュメントでは、Data Distillerを使用して Python ノートブックのデータを調査および分析する際の基本的な例とベストプラクティスをいくつか示します。

## はじめに

このガイドに進む前に、Python ノートブックで Data Distillerへの接続が作成されていることを確認してください。 手順については、ドキュメントを参照してください。 [Python ノートブックを Data Distillerに接続する](./establish-connection.md).
...
次に、Python を使用してクエリ結果をプロットします `plotly` ライブラリ：
...
![eventType 電子メールファネルの解説図。](../../images/data-distiller/email-funnel.png)

## イベントの相関関係 {#event-correlations}

もう 1 つの一般的な分析は、イベントタイプとターゲットコンバージョンイベントタイプの間の相関関係を計算することです。 この例では、購読イベントは、 `web.formFilledOut`. この例では、 [!DNL Spark] Data Distillerクエリで使用できる関数を使用して、次の手順を実行できます。
...
The [!DNL Spark] 関数はデータを集計して結果の小さなテーブルを返すので、このタイプのクエリを完全なデータセットに対して実行できます。
...
|   | webFormsFilled_totalUsers | advertisingClicks_totalUsers | productViews_totalUsers | productPurchases_totalUsers | propositionDisces_totaUsers | propositionDisplays_totaUsers | propositionInteract_totalUsers | emailClicks_totalUsers | emailOpens_totalUsers | webLinksClicks_totalUsers | ... | webForms_advertisingClicks | webForms_productViews | webForms_productPurchases | webForms_propositionDisces | webForms_propositionInteracts | webForms_emailClicks | webForms_emailOpens | webForms_emailSends | webForms_webLinkClicks | webForms_webPageViews |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
| 4 | `webForms_propositionDismisses` | 0.060140 | propositionDusses | 0.060140 |
...
![イベントの結果のイベントのピアソン相関の棒グラフ](../../images/data-distiller/pearson-correlations.png)

## 次の手順

このドキュメントでは、Data Distillerを使用して Python ノートブックからデータを調査および分析する方法を学びました。 機械学習環境でExperience Platformからカスタムモデルをフィードするための機能パイプラインを作成する次の手順は、次のとおりです。 [機械学習のエンジニア機能](./feature-engineering.md).
