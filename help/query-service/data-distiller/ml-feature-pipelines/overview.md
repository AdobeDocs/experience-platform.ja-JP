...
Data Distillerを使用すると、データサイエンティストやエンジニアは、Adobe Experience Platformで収集およびキュレーションされた、価値の高いカスタマーエクスペリエンスデータを使用して、機械学習パイプラインを強化できます。 あらゆるExperience Platformの [!DNL Python] ノートブックから、環境の顧客データをインタラクティブに調査し、そのデータから機能を定義して計算し、計算された機能をモデリング用に機械学習に読み込むことができます。
...
- Experience PlatformAPI の [ 認証およびアクセス ](../../../landing/api-authentication.md) 方法。
...
この一連のガイドに含まれるドキュメントでは、Experience Platformから機能パイプラインを作成し、機械学習でカスタムモデルをフィードするための基本的な手順を説明します。 これで、Data Distillerと [!DNL Jupyter Notebook] の間の接続を確立する準備が整いました。
...
- [aepp](https://github.com/adobe/aepp)：コードから Data Distillerやその他のExperience Platformサービスにリクエストを行うための、Adobeが管理するオープンソース [!DNL Python] ライブラリ [!DNL Python] す。
