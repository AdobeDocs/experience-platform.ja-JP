...
2. `Loyal`：一貫性があるが、トップスペンダーではない頻繁な顧客（頻度= 1）。
3. `Whales`：最新性と頻度に関係なく、支出が最も多い（通貨= 1）。
4. `Promising`：頻度は高いが、支出者が少ない（頻度= 1、2、3、通貨= 2、3、4）。
5. `Rookies`：頻度の低い新規顧客（最新性= 1、頻度= 4）。
6. `Slipping`：以前は常連客であった、アクティビティが減少した顧客（最新性= 2、3、4、頻度= 4）。
