...
このドキュメントでは、匿名ブロックおよびスナップショット機能を使用して増分読み込みを実行する方法をより深く理解し、このロジックを独自のクエリに適用する方法について説明します。 クエリの実行に関する一般的なガイダンスについては、[クエリサービスでのクエリのー実行に関するガイド](./writing-queries.md)を参照してください。
