...
>title="OAuth サーバー間資格情報への移行"
>abstract="JWT 資格情報は 2025 年 6 月 30 日（PT）以降に機能しなくなるので、この移行は必須です。 約 30～40 秒かかり、一度開始するとキャンセルすることはできません。 すべての既存のジョブと統合は、移行後も引き続き OAuth で機能します。 この画面を離れて、いつでも戻ってステータスを確認できます。"
