...
クエリをスケジュールするには、「[!UICONTROL Templates]」タブまたは「[!UICONTROL Template]」タブの「[!UICONTROL Scheduled Queries]」列からクエリテンプレートを選択します。 テンプレート名を選択すると、クエリエディターに移動します。
...
スケジュールワークスペースが表示されます。UI に、テンプレートが関連付けられているスケジュールされた実行のリストが表示されます。 「**[!UICONTROL Add Schedule]**」を選択して、スケジュールを作成します。
...
**[!UICONTROL Frequency]** では、次のオプションを選択できます。

- **[!UICONTROL Hourly]**: スケジュールされたクエリは、1 時間ごとに、選択した期間で実行されます。
- **[!UICONTROL Daily]**: スケジュールされたクエリは、X 日ごとに、選択した時間、期間に実行されます。 選択した時間は、ローカルタイムゾーンではなく、**UTC**&#x200B;であることに注意してください。
- **[!UICONTROL Weekly]**：選択したクエリは、選択した曜日、時間、および期間に実行されます。 選択した時間は、ローカルタイムゾーンではなく、**UTC**&#x200B;であることに注意してください。
- **[!UICONTROL Monthly]**：選択したクエリは、毎月、選択した日、時間および期間に実行されます。 選択した時間は、ローカルタイムゾーンではなく、**UTC**&#x200B;であることに注意してください。
- **[!UICONTROL Yearly]**：選択したクエリは、毎年、選択した日、月、時間および期間に実行されます。 選択した時間は、ローカルタイムゾーンではなく、**UTC**&#x200B;であることに注意してください。

### データセットの詳細を入力 {#dataset-details}

既存のデータセットにデータを追加するか、新しいデータセットを作成してデータを追加することで、クエリ結果を管理します。

初めてクエリを実行するときにデータセットを作成する場合は、「**[!UICONTROL Create and append into new dataset]**」を選択します。 その後の実行では、引き続きそのデータセットにデータが挿入されます。 最後に、データセットの名前と説明を入力します。
...
テンプレートの「[!UICONTROL Schedules]」タブから、スケジュール ID を選択して、新しくスケジュールされたクエリのクエリ実行のリストに移動します。
...
| 列タイトル | 説明 |
|---------------------|----------------------------------|
| [!UICONTROL Query Run ID] | クエリの実行ごとに一意の ID を表示します。これにより、スケジュールされたクエリの個々の実行を追跡および参照できます。 |
| [!UICONTROL Query Run Start] | クエリ実行の開始日時を示します。各実行がいつ開始されたかを監視するのに役立ちます。 |
| [!UICONTROL Query Run Complete] | クエリ実行の完了日時を示し、insightに実行期間とステータスを提供します。 |
| [!UICONTROL Status] | クエリ実行の現在のステータスを表示します（結果 `Completed,` 迅速に評価するための `Running,` や `Failed,` など）。 |
| [!UICONTROL Datasets] | クエリ実行で使用されたデータセットをリスト表示し、実行に関与したデータソースを表示します。 |
| [!UICONTROL Compute Hours] | 各クエリの実行に使用される計算時間を時間単位で表示します。 これにより、リソース使用量を追跡し、クエリのパフォーマンスを最適化できます。 |
...
特定のクエリのスケジュールワークスペース、またはスケジュールされたすべてのクエリを一覧表示する [!UICONTROL Scheduled Queries] ワークスペースから、スケジュールを有効、無効、または削除できます。

選択したクエリの「[!UICONTROL Schedules]」タブにアクセスするには、「[!UICONTROL Templates]」タブまたは「[!UICONTROL Scheduled Queries]」タブからクエリテンプレートの名前を選択する必要があります。 これにより、そのクエリのクエリエディターに移動します。 クエリエディターで「**[!UICONTROL Schedules]**」を選択して、スケジュール ワークスペースにアクセスします。
