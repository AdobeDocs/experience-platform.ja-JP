...
`'$'` の序文を使用して、テキストエディターでクエリにクエリパラメーターを入力します。次に、「」の横にある「**[!UICONTROL Query parameters]**」タブを選択 [!UICONTROL Console]、キーの欠落している値を追加します。 必要なキーに値を追加しない場合、クエリは実行できません。 アラートアイコン（![ アラートアイコン。](/help/images/icons/alert.png)）が「クエリパラメーター」セクションの空の [!UICONTROL Value] 入力フィールドの横に表示されます。
...
使用される値は永続的ではないので、テンプレート内にパラメーターを保存することはできません。 ただし、[!UICONTROL Query log details] ページをチェックして、クエリ実行で使用されるパラメーター値を確認することはできます。 この場合、ログはクエリがパラメーター化クエリだったことを示しません。 使用される値を見つける手順については、[ クエリログドキュメント ](./query-logs.md) を参照してください。
