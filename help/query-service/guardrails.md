...
| スケジュール済バッチの同時クエリサービスユーザー | ユーザーの制限なし | なし | スケジュールされたバッチクエリは非同期ジョブなので、ユーザーの制限はありません。 |
