...
次の構文は、`SELECT` でサポートされる [!DNL Query Service] クエリを定義します。
...
>[!NOTE]
>
>`HEAD` 句で `TAIL` または `SNAPSHOT` を使用する場合は、それらを一重引用符で囲む必要があります（「HEAD」、「TAIL」など）。 引用符なしで使用すると、構文エラーが発生します。
...
| `SINCE start_snapshot_id INNER JOIN table_to_be_joined AS OF your_chosen_snapshot_id ON table_to_be_queried.id = table_to_be_joined.id` | 指定されたスナップショット ID から始まるデータを `table_to_be_queried` から読み取り、`table_to_be_joined` の時点の `your_chosen_snapshot_id` からのデータと結合します。 結合は、結合する 2 つのテーブルの ID 列の一致する ID に基づきます。 |
...
デフォルトでは、`WHERE` クエリの `SELECT` 句で生成される一致は、大文字と小文字が区別されます。 大文字と小文字を区別せずに一致させる場合は、`ILIKE` の代わりにキーワード `LIKE` を使用します。
...
変換後の機能を使用してモデルのトレーニングを行う準備が整ったら、[Models のドキュメント ](../advanced-statistics/models.md) を参照して、`CREATE MODEL` 句で `TRANSFORM` を使用する方法を確認してください。
...
- 変換は、テーブルの作成時にのみ適用されます。 `INSERT INTO` を使用してテーブルに挿入された新しいデータは、**自動的には変換されません**。 新しいデータに変換を適用するには、`CREATE TABLE AS SELECT` 句を使用してテーブルを再作成 `TRANSFORM` る必要があります。
...
`TRANSFORM` で `CREATE TABLE` を使用する場合は、次の制限が適用されます。 変換され `CREATE TABLE AS SELECT` データの保存方法、ベクトル出力の処理方法、結果をモデルトレーニングワークフローで直接再利用できない理由について詳しくは、制限事項と動作の節を参照してください。
...
>[!INFO]
> 
>**文** 丸カッコ（） `SELECT` 囲まないでください。 また、`SELECT` ステートメントの結果のスキーマは、`INSERT INTO` ステートメントで定義されたテーブルのスキーマと一致する必要があります。 `SNAPSHOT` 句を指定すると、増分差分をターゲットテーブルに読み込むことができます。
...
`table_exists` SQL コマンドは、テーブルが現在システムに存在するかどうかを確認するために使用します。 このコマンドは、ブール値を返します。テーブル `true` が存在 **する場合は**、テーブル `false` が存在しない **場合は** です。
...
任意の設定の値を返すには、`SET [property key]` を付けずに `property_value` を使用します。
...
`ANALYZE TABLE` コマンドは、指定されたテーブルに対して分布分析および統計計算を実行します。 `ANALYZE TABLE` の使用方法は、データセットが [ 高速化ストア ](#compute-statistics-accelerated-store) と [ データレイク ](#compute-statistics-data-lake) のどちらに保存されているかによって異なります。 使用方法について詳しくは、それぞれの節を参照してください。
...
`ANALYZE TABLE` コマンドを使用した後で使用できる統計計算のリストを以下に示し :- す。
...
[!DNL Azure Data Lake Storage] SQL コマンドを使用して、`COMPUTE STATISTICS` （ADLS）データセットに関する列レベルの統計を計算できるようになりました。 データセット全体、データセットのサブセット、すべての列、列のサブセットのいずれかに関する列の統計を計算。
...
サンプルデータセットは、元のレコードのパーセンテージのみを使用して、既存の [!DNL Azure Data Lake Storage] （ADLS）データセットからの均一なランダムサンプルを使用して作成されます。 データセットサンプル機能は、`ANALYZE TABLE` および `TABLESAMPLE` SQL コマンドで `SAMPLERATE` コマンドを拡張します。
