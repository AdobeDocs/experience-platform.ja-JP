...
# ラムダ関数の例：類似したレコードを取得する

Data Distillerのラムダ関数を使用して、1 つ以上のデータセットから類似したレコードや関連するレコードを識別し、取得することで、いくつかの一般的な使用例を解決します。 このガイドを使用して、特性や属性に大きな類似点がある様々なデータセットの製品を識別できます。 このドキュメントの手法は、データの重複排除、レコードのリンケージ、レコメンデーションシステム、情報の取得、テキスト分析などのソリューションを提供します。

このドキュメントでは、類似性結合を実装するプロセスを説明し、次に Data Distillerのラムダ関数を使用して、データのセット間の類似性を計算し、選択した属性に基づいてフィルタリングします。 SQL コードスニペットと説明は、プロセスの各手順で提供されます。 このワークフローは、Data Distillerのラムダ関数を使用した Jaccard 類似性測定とトークン化を使用して、類似性結合を実装します。 次に、これらのメソッドは、類似性指標に基づいて、類似したレコードや関連するレコードを 1 つ以上のデータセットから識別し、取得するために使用されます。 プロセスの主なセクションは次のとおりです。 [ラムダ関数を使用したトークン化](#data-transformation)、 [一意の要素のクロス結合](#cross-join-unique-elements)、 [Jaccard 類似性計算](#compute-the-jaccard-similarity-measure)、および [しきい値ベースのフィルタリング](#similarity-threshold-filter).
...
- **ラムダ関数** Data Distillerは匿名のインライン関数で、SQL 文内で定義および使用できます。 多くの場合、データとして渡すことができる簡潔で即座に関数を作成できるので、上位関数で使用されます。 ラムダ関数は、多くの場合、 `transform`, `filter`、および `array_sort`. ラムダ関数は、完全な関数を定義する必要がない状況で特に役立ち、短い 1 回限りの関数をインラインで使用できます。

## はじめに

Data Distiller SKU は、Adobe Experience Platformデータに対してラムダ関数を実行するために必要です。 Data Distiller SKU をお持ちでない場合、詳しくは、Adobeのカスタマーサービス担当者にお問い合わせください。
...
## Data Distiller lambda 関数を使用したソリューションの調査 {#lambda-function-solutions}

ラムダ関数は、Data Distillerの構文と同様に「プログラミング」を実装できる強力な構文です。 これらを使用して、配列内の複数の値に対して関数を繰り返し実行できます。

Data Distillerのコンテキストでは、ラムダ関数は n-gram を作成し、文字のシーケンスを繰り返すのに最適です。
...
- 行 3: `x -> reduce(sequence(1, x), 0, (acc, y) -> acc + y)` は、シーケンス内の各要素 x に対して、1 ～ 5 の削減操作を実行します。
   - The `reduce` 関数は、初期アキュムレータ値 0 を取ります。この値は、1 から現在の値の順です。 `x`、およびラムダ関数 `(acc, y) -> acc + y` をクリックして数字を追加します。
   - ラムダ関数 `acc + y` は、現在の値を追加することで合計を累積します `y` 蓄圧器に `acc`.
- 行 8: `AS sum_result` 結果の列の名前を sum_result に変更します。

要約すると、このラムダ関数は 2 つのパラメータ (`acc` および `y`) およびで、実行する操作を定義します（この場合は、を追加します）。 `y` 蓄圧器に `acc`. このラムダ関数は、削減プロセス中に、シーケンス内の各要素に対して実行されます。

この文の出力は、単一の列 (`sum_result`) には、1 ～ 5 の数値の累積合計が含まれます。

### ラムダ関数の値 {#value-of-lambda-functions}

この節では、Data Distillerのラムダ関数の値をより深く理解し、n-gram をより効率的に作成するために、tri-gram SQL 文の縮小版を分析します。
...
- 行 2: `transform` シーケンス内の各整数にラムダ関数を適用します。
...
- 行 4: `i -> substring(lower(replace(ProductName, ' ', '')), i, 3)` は、各整数に対して動作するラムダ関数です `i` 生成されたシーケンス内。
...
このドキュメントを読むと、このロジックを使用して、意味のある関係や異なるデータセット間の重複を強調表示できます。 特性や属性に大きな類似点がある異なるデータセットから製品を識別する機能には、多数の実際のアプリケーションがあります。 このロジックは、製品の照合（顧客に類似した製品をグループ化またはレコメンデーション）、データのクレンジング（データ品質を向上させる）、マーケティングバスケット分析（顧客の行動、好み、潜在的なクロスセルの機会に関するインサイトを提供）などに使用できます。

まだおこなっていない場合は、 [AI/ML 機能パイプラインの概要](../data-distiller/ml-feature-pipelines/overview.md). この概要を使用して、Data Distillerとお客様の好みの機械学習が、Experience Platformデータを使用したマーケティングの使用例をサポートするカスタムデータモデルを構築する方法を学びます。
