...
カスタムイベントを追加するには、 **[!UICONTROL カスタムイベントを追加]**. 次に、カスタムイベント名を入力し、スキーマのイベントフィールドにマッピングします。 影響力のある要因やその他のインサイトを調べると、フィールド値の代わりにカスタムイベント名が表示されます。 つまり、ユーザー ID、予約 ID、デバイス情報およびその他のカスタム値は、イベントの ID/値ではなく、カスタムイベント名で一覧表示されます。 これらの追加のカスタムイベントは、モデルの品質を向上させ、より正確な結果を提供するために顧客 AI で使用されます。
...
>[!NOTE]
>
>カスタムプロファイル属性の追加は、カスタムイベントの追加と同じワークフローに従います。

![カスタムプロファイル属性の追加](../images/user-guide/profile-attributes.png)

### スケジュールの設定&#x200B;*（オプション）* {#configure-a-schedule}

この **[!UICONTROL 詳細]** 手順が表示されます。 このオプションの手順では、予測の実行を自動化するスケジュールを設定したり、予測の除外を定義して特定のイベントをフィルターしたり、選択したりできます **[!UICONTROL 完了]** 何も必要がない場合は。
...
データセットにテストデータとして追加された列が含まれている場合は、「 」を選択して、その列またはイベントを除外リストに追加できます **除外を追加** その後、除外するフィールドを入力します。 これにより、特定の条件を満たすイベントがスコアの生成時に評価されなくなります。 この機能を使用すると、無関係なデータ入力や特定のプロモーションを除外できます。
...
顧客 AI を初めて使用する場合は、モデルの出力結果に満足するまで、この機能をオフにする必要があります。 これにより、モデルを微調整しながら、複数のスコアリングデータセットをリアルタイム顧客プロファイルにアップロードできなくなります。

![プロファイル切り替え](../images/user-guide/advanced-workflow.png)
