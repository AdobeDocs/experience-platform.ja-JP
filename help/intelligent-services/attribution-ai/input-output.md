...
| 推奨される列 | に必要 |
| --- | --- |
| プライマリ ID フィールド | タッチポイント/コンバージョン |
...
| touchpointsDetail （オブジェクト配列） | True | タッチポイント発生またはタイムスタンプで並べ替えられたコンバージョンにつながるタッチポイント詳細のリスト。 |
...
生のスコアへのパスを UI に表示できます。 Experience Platform UI で「**[!UICONTROL Schemas]**」を選択することから開始し、「**[!UICONTROL Browse]**」タブ内からアトリビューション AI スコアスキーマを検索して選択します。

![&#x200B; スキーマを選択 &#x200B;](./images/input-output/schemas_browse.png)

次に、UI の **[!UICONTROL Structure]** ウィンドウ内でフィールドを選択すると、「**[!UICONTROL Field properties]**」タブが開きます。 **[!UICONTROL Field properties]** 内には、生のスコアにマッピングされるパスフィールドがあります。
