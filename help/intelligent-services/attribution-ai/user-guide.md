...
次の **[!UICONTROL スコアリングされた合計コンバージョンイベント数]** 指標は、 **[!UICONTROL モデルを作成]** コンテナ。 この指標では、すべてのサンドボックス環境と削除されたサービスモデルを含む、現在の暦年のAttribution AIでスコアリングされたコンバージョンイベントの合計数を追跡します。
...
- **[!UICONTROL 削除]**:過去の実行を含むサービスモデルを削除できます。 対応する出力データセットが Platform から削除されます。 ただし、リアルタイム顧客プロファイルに同期されたスコアは削除されません。
