...
1 つの名前空間内で複数の ID を使用できる場合は、使用事例に合った正しい ID フィールドを選択するようにしてください。 例えば、電子メール名前空間内では、仕事用電子メールと個人用電子メールの 2 つの電子メール ID を使用できます。 ユースケースによっては、個人の電子メールが入力される可能性が高く、個々の予測でより役に立つようになります。 これは、「 `EMAIL (personalEmail.address)` ID として。
