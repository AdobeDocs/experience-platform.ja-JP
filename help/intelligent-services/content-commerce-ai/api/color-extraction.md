...
| `encoding` | 入力画像のファイル形式。 現在処理できるのは、JPEGと PNG 画像のみです。 このプロパティのデフォルトは `jpeg` です。 | × |
...
以下の最初の例のオブジェクトでは、 `White,0.59,251,251,243` の `feature_value` は、見つかった色が白で、白は画像の 59%で、RGB値は 251,251,243 です。
...
| `feature_value` | 同じプロパティ名を持つキーを含むオブジェクトを持つ配列。 これらのキーには、色名を表す文字列、`content_id` に送信された画像に対するこの色の表示割合、色のRGB値が含まれます。 |
