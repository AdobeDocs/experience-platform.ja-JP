...
フィルターアイコン（）を選択すると、結果を特定のオーディエンスにフィルタリングし、そのセグメント化ジョブを表示できます![フィルターアイコン。](../assets/ui/monitor-audiences/filter-icon.png)）に設定します。 セグメント化ジョブは時系列で並べ替えられ、最新のセグメント化ジョブが最初に表示されます。
...
特定のセグメント化ジョブをさらにフィルタリングして、フィルターアイコン（![フィルターアイコン。](../assets/ui/monitor-audiences/filter-icon.png)）に設定します。 フィルタリングできるセグメント化ジョブには、アクティベーションジョブと評価ジョブの 2 種類があります。
