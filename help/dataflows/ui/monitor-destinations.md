...
- **[!UICONTROL Processing duration]**：データフロー実行が処理されるまでにかかった時間です。
...
## 宛先モニタリングダッシュボード {#monitoring-destinations-dashboard}
...
詳細ページには、データフロー実行エラーとオーディエンスを切り替える切替スイッチもあります。 このオプションは、「[&#x200B; オーディエンスレベルの表示 &#x200B;](#segment-level-view) セクションにリストされている宛先で使用できます。
