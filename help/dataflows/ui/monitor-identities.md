...
## モニタリング ID ダッシュボード {#identity-metrics}
...
メインの **[!UICONTROL Identities]** ダッシュボードでは、**[!UICONTROL Identities]** カードに、受信したレコードの合計数、取り込んだレコードの数、レコード取り込みの成功率に関する情報が表示されます。
...
[!UICONTROL Dataflow run details] ページには、[!DNL Identity] データフロー実行に関する詳細（組織 ID やデータフロー実行 ID など）が表示されます。 また、このページには、取り込みプロセスでエラーが発生した場合に、[!DNL Identity Service] から提供される対応するエラーコードとエラーメッセージも表示されます。
