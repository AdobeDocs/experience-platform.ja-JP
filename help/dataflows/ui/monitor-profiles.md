...
The [!UICONTROL プロファイルの処理] ページには、次の場所に取り込まれたレコードに関する情報が含まれます： [!DNL Profile]（作成したプロファイルフラグメントの数、更新したプロファイルフラグメントの数、プロファイルフラグメントの合計数を含む）。
...
| **[!UICONTROL 合計プロファイルフラグメント]** | に書き込まれたレコードの合計数 [!DNL Profile]（既存のすべてを含む） [!DNL Profile] 更新されたフラグメントと新しいフラグメント [!DNL Profile] フラグメントが作成されました。 |
...
フィルターアイコンを選択します。 ![フィルター](../assets/ui/monitor-profiles/filter.png) データフローの実行開始時間の横に、 [!DNL Profile] データフローの実行。

![フィルターアイコンがハイライト表示されます。 このアイコンを選択すると、選択したデータフローに関する詳細を表示できます。](../assets/ui/monitor-profiles/dataflows-filter.png)

The [!UICONTROL データフローの実行の詳細] ページには、 [!DNL Profile] データフローの実行（組織 ID とデータフローの実行 ID を含む）。 また、このページには、 [!DNL Profile]は、取り込みプロセスでエラーが発生した場合に使用します。
