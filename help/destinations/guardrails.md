...
| 宛先に対して同時にアクティブ化されたオーディエンスの最大数 | 50 | パフォーマンスガードレール | 宛先ごとに1回に50を超えるオーディエンスを一括アクティベートすることはできません。 ベストプラクティスとして、オーディエンスを20個のバッチでアクティベートし、数分待ってから、繰り返します。 50以上のオーディエンスを同時にアクティベートすると、プロセスが行き詰まるか、宛先に予測よりも少ないオーディエンスが届く可能性があります。 |
...
[Destination SDK](/help/destinations/destination-sdk/overview.md)は、任意のデータおよび認証形式に基づいて、エンドポイントにオーディエンスデータとプロファイルデータを配信するようにExperience Platformの宛先統合パターンを設定するための一連の設定APIです。 以下のガードレールは、Destination SDK を使用して設定する宛先に適用されます。

| ガードレール | 上限 | 上限のタイプ | 説明 |
| --- | --- | --- | --- |
| [プライベートカスタム宛先](/help/destinations/destination-sdk/overview.md#productized-custom-integrations)の最大数 | サンドボックスあたり5 | パフォーマンスガードレール | Destination SDKを使用して、サンドボックスごとに最大5つのプライベートカスタムストリーミングまたはバッチ配信先を作成できます。 宛先を 5 つ以上作成する必要がある場合は、カスタムケア担当者にお問い合わせください。 |
