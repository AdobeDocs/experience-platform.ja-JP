...
>[!NOTE]
>
>ファイルの書き出しに `lastQualificationTime` を追加するには、現在、次に示すように、値 `xdm: segmentMembership.ups.seg_id.lastQualificationTime` をソースフィールドに手動で挿入する必要があります。 また、ターゲットフィールドを編集して、`lastQualificationTime` や、この列に名前を付けるその他の値に変更することもできます。 これはベータ版機能なので、`xdm: segmentMembership.ups.seg_id.lastQualificationTime` 値の構文は将来変更される可能性があります。
