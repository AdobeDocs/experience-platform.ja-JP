...
| コントロール | 説明 |
|---------|----------|
| [!UICONTROL Set up] | 宛先への接続を作成します。 |
| [!UICONTROL Activate] | 宛先への接続を確立したら、オーディエンスをアクティブ化するか、データセットをこの宛先に書き出します。 |
| [!UICONTROL Activate audiences] | 宛先への接続を確立したら、この宛先へのオーディエンスをアクティブ化します。 |
| [!UICONTROL Export datasets] | 宛先への接続を確立したら、データセットをこの宛先に書き出します。 |
...
宛先行を選択すると、宛先ID、説明、アクティブ化されたオーディエンスの数など、宛先に関する詳細が右側のパネルに表示されます。

![宛先行をクリック](../assets/ui/workspace/click-destination-row.png)

宛先名を選択すると、この宛先に対してアクティブ化されたオーディエンスに関する情報が表示されます。 **[!UICONTROL Edit destination]**&#x200B;を選択して[宛先設定を変更](/help/destinations/ui/edit-destination.md)または&#x200B;**[!UICONTROL Activate audiences]**&#x200B;し、データフローに新しいオーディエンスを追加します。
...
「**[!UICONTROL Accounts]**」タブには、様々な宛先で確立した接続に関する詳細が表示されます。 このタブから既存のアカウントの詳細を更新または削除できます。 各宛先アカウントで使用できるすべての情報については、次の表を参照してください。
