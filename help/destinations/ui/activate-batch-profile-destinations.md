...
>
>改善されたファイル書き出し機能ベータプログラムに参加している一部のお客様には、新しい **[!UICONTROL マッピング]** アクティベーションワークフローの一部として、 [新しいベータクラウドストレージの宛先](/help/release-notes/2022/october-2022.md#destinations). また、 [既知の制限事項](#known-limitations) をリリースの一部として追加しました。
...
ほとんどの宛先では、デフォルトのファイル名は、宛先名、セグメント ID、日付と時刻のインジケーターで構成されます。 例えば、エクスポートされたファイル名を編集して、異なるキャンペーンを区別したり、データのエクスポート時間をファイルに追加したりできます。一部の宛先開発者は、別のデフォルトのファイル名追加オプションを宛先に表示するように選択する場合があります。

鉛筆アイコンを選択してモーダルウィンドウを開き、ファイル名を編集します。ファイル名は 255 文字までに制限されています。

>[!NOTE]
>
>次の画像は、のファイル名を編集する方法を示しています [!DNL Amazon S3] の宛先の場合、プロセスはすべてのバッチ宛先 (SFTP、 [!DNL Azure Blob Storage]または [!DNL Google Cloud Storage]) をクリックします。
...
| **[!UICONTROL 日時]** | 追加する項目を選択 `MMDDYYYY_HHMMSS` 形式または Unix の 10 桁のタイムスタンプ。 増分エクスポートのたびにファイル名を動的に生成したい場合は、次のオプションのいずれかを選択します。 |
| **[!UICONTROL カスタムテキスト]** | ファイル名に追加するカスタムテキスト。 |
| **[!UICONTROL 宛先 ID]** | セグメントのエクスポートに使用する宛先データフローの ID。 <br> **注意**:このファイル名追加オプションは、改善されたファイルエクスポート機能ベータプログラムに参加しているベータ版のお客様のみが利用できます。 ベータ版プログラムにアクセスする場合は、Adobe担当者またはカスタマーケアにお問い合わせください。 |
| **[!UICONTROL 宛先名]** | セグメントのエクスポートに使用する宛先データフローの名前。 <br> **注意**:このファイル名追加オプションは、改善されたファイルエクスポート機能ベータプログラムに参加しているベータ版のお客様のみが利用できます。 ベータ版プログラムにアクセスする場合は、Adobe担当者またはカスタマーケアにお問い合わせください。 |
| **[!UICONTROL 組織名]** | 組織内のExperience Platform名。 <br> **注意**:このファイル名追加オプションは、改善されたファイルエクスポート機能ベータプログラムに参加しているベータ版のお客様のみが利用できます。 ベータ版プログラムにアクセスする場合は、Adobe担当者またはカスタマーケアにお問い合わせください。 |
| **[!UICONTROL サンドボックス名]** | セグメントの書き出しに使用するサンドボックスの ID。 <br> **注意**:このファイル名追加オプションは、改善されたファイルエクスポート機能ベータプログラムに参加しているベータ版のお客様のみが利用できます。 ベータ版プログラムにアクセスする場合は、Adobe担当者またはカスタマーケアにお問い合わせください。 |
...
>[!IMPORTANT]
> 
>ベータ版のお客様は、 **[!UICONTROL マッピング]** 次に代わる手順 [プロファイル属性を選択](#select-attributes) 上記の手順を参照してください。 この新しい **[!UICONTROL マッピング]** 手順では、必要なカスタム名に対して、書き出されたファイルのヘッダーを編集できます。
> 
> 機能とドキュメントは変更される場合があります。 このベータ版プログラムにアクセスする場合は、Adobe担当者またはカスタマーケアにお問い合わせください。

この手順では、ターゲットの宛先に書き出すファイルに追加するプロファイル属性を選択する必要があります。 エクスポートするプロファイル属性と ID を選択するには：

1. 内 **[!UICONTROL マッピング]** ページ、選択 **[!UICONTROL 新しいフィールドを追加]**.
...
1. 内 **[!UICONTROL ソースフィールドを選択]** ページで、書き出し先のファイルに含めるプロファイル属性と ID を選択し、「 」を選択します **[!UICONTROL 選択]**.

   >[!TIP]
   > 
   >次の画像に示すように、検索フィールドを使用して、選択を絞り込むことができます。

   ![宛先に書き出すことができるプロファイル属性を示すモーダルウィンドウ。](../assets/ui/activate-batch-profile-destinations/select-source-field-modal.png)


1. エクスポート用に選択したフィールドがマッピングビューに表示されます。 必要に応じて、書き出したファイルのヘッダーの名前を編集できます。 それには、「ターゲット」フィールドで「 」アイコンを選択します。

   ![宛先に書き出すことができるプロファイル属性を示すモーダルウィンドウ。](../assets/ui/activate-batch-profile-destinations/mapping-step-select-target-field.png)

1. 内 **[!UICONTROL ターゲットフィールドを選択]** ページで、書き出したファイルのヘッダーの名前を入力し、 **[!UICONTROL 選択]**.

   ![ヘッダーの入力済みのわかりやすい名前を示すモーダルウィンドウ。](../assets/ui/activate-batch-profile-destinations/select-target-field-mapping.png)

1. エクスポート用に選択したフィールドがマッピングビューに表示され、編集したヘッダーがエクスポートされたファイルに表示されます。

   ![宛先に書き出すことができるプロファイル属性を示すモーダルウィンドウ。](../assets/ui/activate-batch-profile-destinations/select-target-field-updated.png)

1. （オプション）エクスポートされたフィールドを選択して、 [必須キー](#mandatory-keys) または [重複排除キー](#deduplication-keys).
...
#### マッピングワークフローでは、セグメントメンバーシップ属性を選択できません
...
#### ID 名前空間は、現在、エクスポート用に選択できません

以下の画像に示すように、書き出し用の ID 名前空間の選択は、現在サポートされていません。 書き出し用の ID 名前空間を選択すると、 **[!UICONTROL レビュー]** 手順

![ID エクスポートを示すサポートされていないマッピング](/help/destinations/assets/ui/activate-batch-profile-destinations/unsupported-identity-mapping.png)

ベータ版の間に書き出したファイルに ID 名前空間を追加する必要がある場合の一時的な回避策として、次のいずれかを実行できます。
* エクスポートに ID 名前空間を含めるデータフローに、従来のクラウドストレージの宛先を使用します
* ID を属性としてExperience Platformにアップロードし、クラウドストレージの宛先に書き出します。
...
例えば、毎日のエクスポート頻度を選択した場合、3 日連続で受け取るファイルは次のようになります。
