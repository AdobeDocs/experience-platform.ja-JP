...
スケジュールされた書き出しには、オーディエンススナップショットデータに加え、スナップショットの作成と書き出し時間の間に発生する増分プロファイルまたは ID の変更が含まれます。 これは、スナップショットデータのみを使用する [&#x200B; オンデマンド書き出し &#x200B;](export-file-now.md) とは異なります。

次の表では、スケジュールされた書き出しが、特にデータの鮮度と使用目的の点で、オンデマンド書き出しとどのように異なるかを示しています。

|  | スケジュールされた書き出し | 今すぐファイルを書き出し |
|--------|-------------------|-----------------|
| **データソース** | スナップショット +増分変更 | スナップショットのみ |
| **プロファイル属性** | エクスポート時の現在の値 | スナップショット時の値 |

オーディエンスの評価後にプロファイルが更新された場合、評価時にオーディエンスメンバーシップが決定されていても、スケジュールされた書き出しには更新された属性値が含まれます。

**例**:「retailID が null のプロファイル」のオーディエンスは、そのフィールドが更新された場合、retailID が入力されたプロファイルを書き出す可能性があります *after* 評価が、スケジュールされた書き出しの *before*。

**レコメンデーション**

* [&#x200B; 重複排除キー &#x200B;](#deduplication-keys) を設定して、レコードの重複を防ぐ
* 正確なスナップショットベースのデータのオンデマンドエクスポートを使用
* バッチ取り込みと評価スケジュールを調整して、不一致を最小限に抑える
...
>[!IMPORTANT]
>
>スケジュールされた書き出しには常に重複排除キーを設定します。 重複排除を使用しない場合、スケジュールされた書き出しはスナップショットと増分データの両方を処理するので、同じプロファイルに対して、行が重複したりセグメントメンバーシップが競合したりすることがあります。

重複排除キーは、プロファイルの重複排除方法を決定するユーザー定義のプライマリキーです。 同じ個人に対して複数のレコードが存在する場合、重複排除は最新のレコードのみが書き出されるようにします。
...
#### マッピングワークフローでは、オーディエンスメンバーシップの属性を選択できません {#audience-membership-attribute-mapping}
