...
* API チュートリアル {#api}
   ...
   * [フローサービス API を使用して、バッチクラウドストレージおよび電子メールマーケティングの宛先に接続し、データをアクティブ化します](./api/connect-activate-batch-destinations.md)
...
* 宛先 SDK {#destination-sdk}
   ...
   * Destination SDK機能 {#functionality}
      ...
      * [（ベータ版）ファイルベースの宛先サーバーとファイル仕様](./destination-sdk/server-and-file-configuration.md)
