...
description: Marketo Munchkin拡張機能は、Adobe Experience Platformのパーソナライズ機能の宛先です。 拡張機能について詳しくは、Adobe Exchange の拡張機能のページを参照してください。
...
リード管理からアカウントベースドマーケティングまで、[!DNL Marketo Engagement Platform]は、エクスペリエンスのあらゆる段階において、顧客や見込み顧客とのエンゲージメントを計画、調整、測定する方法を簡素化します。

[!DNL Marketo’s Munchkin] JavaScriptを使用すると、[!DNL Marketo]のランディングページと外部web ページへのエンドユーザーページの訪問とクリックを追跡できます。

[!DNL Marketo Munchkin]は[!DNL Adobe Experience Platform]のメール拡張機能です。 Marketo Munchkinについて詳しくは、Marketo ドキュメントの[ リードトラッキング ](https://developers.marketo.com/javascript-api/lead-tracking/)を参照してください。
