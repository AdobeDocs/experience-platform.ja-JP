...
* [!DNL SAP Commerce] サービス キーは、Experience Platformを介して[!DNL SAP Subscription Billing] APIへのアクセスを提供します。 「[!DNL SAP Commerce] [ クライアント IDとクライアント秘密鍵](https://help.sap.com/docs/CLOUD_TO_CASH_OD/1216e7b79c984675b0a6f0005e351c74/87c11a0f5dc3494eaf3baa355925c030.html#create-a-service-key-with-client-id-and-client-secret)を使用してサービス キーを作成する」を参照して、サービス キーを作成します。 [!DNL SAP Commerce] には、以下が必要です。
   * クライアント ID
   * クライアントシークレット
...
詳しくは、[!DNL SAP Subscription Billing] [ カスタム参照](https://help.sap.com/docs/CLOUD_TO_CASH_OD/80d121f216af43648e79664efe5595f7/85696a63c8d8453a934e86c9413a25cf.html?version=2023-11-27) ドキュメントを参照してください。
...
これを行うには、各セグメントを選択し、[!DNL SAP Subscription Billing] [!DNL SAP Commerce]宛先コネクタフィールドに&#x200B;**[!UICONTROL Mapping ID]**&#x200B;からのカスタム参照の名前を入力します。 カスタム参照の作成に関するガイダンスについては、「[でカスタム参照を作成する」セクションを参照してください。 [!DNL SAP Subscription Billing]](#prerequisites-custom-reference)
...
[!DNL SAP Subscription Billing] アカウントに移動し、**[!UICONTROL Contacts]** ページに移動して、オーディエンスのステータスを確認します。 リストは、カスタム参照の列を表示したり、対応するオーディエンスのステータスを表示したりするように設定できます。
