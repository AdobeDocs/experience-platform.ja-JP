...
Adobe Campaign Managed Cloud Servicesは、クロスチャネルの顧客体験を設計するためのプラットフォームと、視覚的なキャンペーン編成、リアルタイムのインタラクション管理、クロスチャネルの実行のための環境を提供します。 [Campaign の概要](https://experienceleague.adobe.com/docs/campaign/campaign-v8/start/get-started.html)
...
Campaign がAdobe Experience Platformからデータを取得できるようにするには、Campaign API プロジェクトを作成し、カスタマーケアに依頼して、関連するクライアント ID を許可リストに追加する必要があります。

>[!NOTE]
>
>API プロジェクトの作成方法に関するグローバル情報について詳しくは、 [このドキュメント](https://experienceleague.adobe.com/docs/platform-learn/getting-started-for-data-architects-and-data-engineers/set-up-developer-console-and-postman.html)

1. にログインします。 [Adobe Developer Console](https://console.adobe.io/) 新しいプロジェクトを作成します。

1. 選択 **[!UICONTROL API を追加]** を選択します。 **[!UICONTROL Adobe Campaign]**.

   ![](../../assets/catalog/email-marketing/adobe-campaign-managed-services/create-api.png)

1. キーペアを生成します。

1. を選択します。 `<Instance Name> - admin` 製品プロファイルを選択し、 **[!UICONTROL 設定済み API を保存]**.

1. API プロジェクトが作成されます。 次の点に注意してください： **[!UICONTROL クライアント ID]** プロジェクトに表示されます。 カスタマーケアに問い合わせて、Adobeにクライアント ID の許可リストへの追加を依頼してください。

   ![](../../assets/catalog/email-marketing/adobe-campaign-managed-services/client-id.png)

## サポートされる ID {#supported-identities}

*Adobe Campaign Managed Cloud Services* では、以下の表で説明する id のアクティブ化をサポートしています。 詳細情報： [id](/help/identity-service/namespaces.md).
...
アラートを有効にして、宛先へのデータフローのステータスに関する通知を受け取ることができます。 リストからアラートを選択して、データフローのステータスに関する通知を受け取るよう登録します。アラートについて詳しくは、 [UI を使用した宛先アラートの購読](../../ui/alerts.md).
