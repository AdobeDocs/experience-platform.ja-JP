---
keywords: 電子メール；電子メール；電子メール宛先；adobe campaign;campaign
...
[!DNL Adobe Campaign]は次の接続タイプをサポートしています：
...
* **[!UICONTROL Amazon S3]**&#x200B;接続の場合は、[!UICONTROL Access Key ID]と[!UICONTROL Secret Access Key]を指定する必要があります。
...
* **[!UICONTROL Bucket Name]**: *S3接続の場合*。 [!DNL Experience Platform]が書き出しデータをCSV ファイルとしてデポジットするS3 バケットの場所を入力します。
* **[!UICONTROL Folder Path]**: [!DNL Experience Platform]が書き出しデータをCSV ファイルとしてデポジットする保存場所のパスを指定します。
* **[!UICONTROL Container]**: *BLOB接続*&#x200B;の場合。 フォルダーのパスが含まれるBlobを保持するコンテナ。
