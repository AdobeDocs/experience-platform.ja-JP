...
* **[!UICONTROL 環境]**：接続する RainFocus 環境を指定します（例：`dev`、`prod`）。
* **[!UICONTROL 組織 ID]**:RainFocus のインスタンスに一意の `orgid` を指定します。
...
* **[!UICONTROL イベント ID]**：プロファイルの送信先となる [!DNL RainFocus] ーザーイベントコード識別子。
