...
Merkury エンタープライズ接続の宛先を使用して、オーディエンスを Merkury に安全に配信します。 Merkury は、マーケターがユーザーベースのオーディエンスを簡単にマッチングし、Merkury の 80 を超えるプレミアムなアドレス可能なテレビ/CTV、パブリッシャー、アドテックの接続に配信できるようにします。 Merkury は、2 億 6800 万人以上の包括的な米国の成人消費者 ID グラフを活用しています。

![取り込みやアクティブ化を含め、Merkury とExperience Platformの間の相互接続を示す図](../../assets/catalog/data-partners/merkury-connections/media/image1.png)

このドキュメントページの手順に従って、Adobe Experience Platform ユーザーインターフェイスを使用して Merkury Connections の宛先接続を作成し、オーディエンスをアクティブ化します。
...
* **デジタルメディアの有効化**：オーディエンスプロファイルを簡単に照合して、Merkury の 50 以上のプレミアムなアドレス可能なパブリッシャーとアドテック接続に配信します。
* **効率性の向上**:cookie を使用しないアドレス可能なメディアへのリーチを強化し、ターゲティングの効率と広告費用対効果（ROAS）を向上させます。
