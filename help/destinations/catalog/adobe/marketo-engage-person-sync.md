...
description: Marketo Engageユーザー同期コネクタを使用して、ユーザーオーディエンスからMarketo Engage内の対応するレコードに更新をストリーミングします。
...
# Marketo Engage人物同期接続 {#marketo-engage-person-sync}
...
Marketo Engageユーザー同期コネクタを使用して、ユーザーオーディエンスからMarketo Engageインスタンスの対応するレコードに更新をストリーミングします。
...
Experience Platformの属性を、Marketoでアクセスできる任意の属性にマッピングできます。 Marketoでは、[Describe API](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Leads/operation/describeUsingGET_6) リクエストを使用して、組織がアクセスできる属性フィールドを取得できます。
...
* **[!UICONTROL パーティション]**：ビジネス上の関心事ごとにリードレコードを分離するために使用されるMarketo Engageの概念
...
* 電子メール <test@example.com> と会社名を持つユーザー（Experience Platform内の Example Inc.）は宛先オーディエンスに一致します
...
Experience Platformデータ型とMarketoデータ型は、次の方法でマッピングできます。

| Experience Platformデータタイプ | Marketo データタイプ |
| ----------------------------- | ------------------------------------ |
...
フィールドに値がない場合は、その値を設定できるようにし、既に値が存在するフィールドを統合が更新できないようにすることが望ましい場合もあります。  Marketo Engageコネクタが宛先インスタンス内の既存の値を上書きしないようにする必要がある場合は、Marketo インスタンスの管理者/フィールド管理セクションで更新をブロックするようにフィールドを設定し、Adobe Experience Platform ソースタイプを切り替えます。
