---
keywords: demandbase拡張機能；demandbase;demandbaseの宛先
title: Demandbase 拡張機能
description: Demandbase拡張機能は、Adobe Experience Platformの分析先です。 拡張機能について詳しくは、Adobe Exchange の拡張機能のページを参照してください。
...
業界、収益、ターゲットアカウント別にトラフィックと行動をセグメント化できる[!DNL Demandbase]個のB2B アカウントのインサイトを[!DNL Adobe Analytics]に直接取得できます。 このアカウントベースの表示は、エンゲージメント、コンバージョン、および最も価値の高い訪問者の出所に関する独自の洞察を提供します。

[!DNL Demandbase]は[!DNL Adobe Experience Platform]のAnalytics拡張機能です。 拡張機能について詳しくは、[Adobe Exchange](https://exchange.adobe.com/experiencecloud.details.101605.html) の拡張機能のページを参照してください。
