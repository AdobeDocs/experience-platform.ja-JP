...
title: TMMData 拡張機能
description: TMMData拡張機能は、Adobe Experience Platformの分析先です。 拡張機能について詳しくは、Adobe Exchange の拡張機能のページを参照してください。
...
[!DNL TMMData's] Foundation Platform for Adobe Marketing Cloudは、マーケティング部門に、社内外およびオンライン/オフラインのデータを含むあらゆる重要なデータソースにアクセスしてブレンドするためのツールを提供します。これにより、キャンペーンのセットアップを自動化し、Adobeやその他の分析ツールやBI ツールに直接読み込むことで、信頼性の高い包括的なクロスチャネル分析が可能になります。

[!DNL TMMData]は[!DNL Adobe Experience Platform]のAnalytics拡張機能です。 拡張機能について詳しくは、[Adobe Exchange](https://exchange.adobe.com/experiencecloud.details.100148.tmmdata-foundation-platform.html) の拡張機能のページを参照してください。
