...
どちらの宛先でも、アカウントにデータを物理的にコピーせずに、Snowflake内のデータにアクセスできますが、各コネクタのユースケースに関しては、いくつかの推奨されるベストプラクティスがあります。

次の表に、各データ共有方法が最も適したシナリオを概説することで、使用するコネクタを決定するのに役立ちます。

|  | 必要に応じて [0}Snowflake バッチ } を選択](snowflake-batch.md) | 必要に応じて [0}Snowflake ストリーミング } を選択](snowflake.md) |
|--------|-------------------|----------------------|
...
* Snowflake アカウントのクラウドプロバイダーおよびリージョンを把握している。 宛先に接続する場合、両方を入力する必要があります。
...
>[!IMPORTANT]
>
>この宛先では、ファイアウォールの内側にあるSnowflake アカウントや、[[!DNL Azure Private Link]](https://docs.snowflake.com/en/user-guide/privatelink-azure) を使用する Firewall アカウントはサポートされません。
...
| 書き出し頻度 | **[!UICONTROL Batch]** | この宛先は、Snowflake データ共有を通じて、オーディエンスメンバーシップ全体の定期的なスナップショットを提供します。 各スナップショットで以前のデータが置き換わるので、オーディエンスの完全なビューを常に最新に保つことができます。 |
...
* **[!UICONTROL Snowflake Account ID]**: [Snowflake Data Sharing アカウント識別子 ](https://docs.snowflake.com/en/user-guide/admin-account-identifier#label-account-name-data-sharing)。 アカウントが組織にリンクされているかどうかに応じて、次の形式を使用します。
   * アカウントが組織にリンクされている場合：組織名とアカウント名を **期間** （`.`）で区切って入力します。 例えば、組織名が ACME で、アカウント名が AsiaRegion の場合、「`ACME.AsiaRegion`」と入力します。
   * アカウントが組織にリンクされていない場合：`AccountName`。
* **[!UICONTROL Snowflake Region]**: Snowflake インスタンスがプロビジョニングされている地域を選択します。 サポートされるクラウド地域について詳しくは、Snowflake[ ドキュメント ](https://docs.snowflake.com/en/user-guide/intro-regions) を参照してください。
* **[!UICONTROL Account acknowledgment]**: **[!UICONTROL Snowflake Account ID]** を入力したら、このドロップダウンで **[!UICONTROL Yes]** を選択して、**[!UICONTROL Snowflake Account ID]** が正しいこと、および自分に属していることを確認します。

>[!NOTE]
>
> 宛先を作成した後に、**[!UICONTROL Snowflake Account ID]** 宛先を編集 **[!UICONTROL Snowflake Region]** ワークフローを使用して [ と ](../../ui/edit-destination.md) を編集することはできません。 異なるアカウント値または地域値を使用するには、[ 新しい宛先接続を作成する ](../../ui/connect-destination.md) を参照してください。
...
* **TS**：共有テーブルの各行が最後に更新された日時を示すタイムスタンプ列
* **結合ポリシー ID**：アクティブ化されるオーディエンスが属する [ 結合ポリシー ](../../../profile/merge-policies/overview.md) の ID
...
![ 動的テーブルデータを含むSnowflake インターフェイスを示すスクリーンショット ](../../assets/catalog/cloud-storage/snowflake-batch/data-validation.png) {align="center" zoomable="yes"}
