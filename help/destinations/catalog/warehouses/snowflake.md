...
Snowflakeのプライベートリスト承認画面を示す![&#x200B; スクリーンショット &#x200B;](../../assets/catalog/cloud-storage/snowflake/snowflake-accept-listing.png)
...
データフローがオーディエンスに対して初めて実行されると、バックフィルが実行され、現在選定されているすべてのプロファイルが共有されます。 この最初のバックフィルの後、増分更新のみが共有テーブルに反映されます。 これは、オーディエンスに追加またはオーディエンスから削除されるプロファイルを意味します。 このアプローチにより、効率的に更新し、共有テーブルを最新の状態に保つことができます。
...
ストリーミングデータ共有は、セグメントの変更、ID マップの変更、属性の変更などにもとづいて継続的に更新されるため、遅延が重要で、すぐに更新する必要があるシナリオに適しています。
...
宛先への認証方法を示す![&#x200B; サンプルのスクリーンショット &#x200B;](../../assets/catalog/cloud-storage/snowflake/authenticate-destination.png)
...
宛先の詳細を入力する方法を示す![&#x200B; サンプルのスクリーンショット &#x200B;](../../assets/catalog/cloud-storage/snowflake/configure-destination-details.png)
...
![Experience Platformの宛先のマッピング画面を示すSnowflake ユーザーインターフェイスの画像。](../../assets/catalog/cloud-storage/snowflake/mapping.png)

ターゲット属性は、**[!UICONTROL Attribute name]** フィールドで指定した属性名を使用して、Snowflakeで自動的に作成されます。

## 書き出されたデータ／データ書き出しの検証 {#exported-data}

Snowflake アカウントを確認して、データが正しく書き出されたことを確認します。

## 既知の制限事項 {#known-limitations}

### デフォルトの結合ポリシー制限 {#default-merge-policy-restriction}

現在、書き出すことができるのは、デフォルトの結合ポリシーにマッピングされたオーディエンスのみです。

### 地域の可用性 {#regional-availability}

現在、[!DNL Snowflake] ストリーミング宛先は、Experience Platform VA7 リージョンでプロビジョニングされている[!DNL Real-Time CDP]のお客様のみが利用できます。
