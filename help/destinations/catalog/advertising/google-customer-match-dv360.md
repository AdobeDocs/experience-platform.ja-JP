...
|---------|----------|---------|
| PII ベースのオーディエンス書き出し、 [!DNL Google Customer Match] で利用可能な広告枠でリーチします。 | [!DNL Google Display & Video 360]、Google が所有および運営するプロパティ(YouTube や [!DNL Search] など)など、さまざまな広告枠で利用可能な広告枠全体でcookieベースのオーディエンスにリーチします。 | [!DNL Google Customer Match] で PII ベースのオーディエンス作成、[!DNL Google Display & Video 360] で利用可能な広告枠(Google が所有および運営するプロパティのみ)でユーザーにリーチします。 |
...
著名なテクノロジー会社が新しい電話を発売しました。 この新しい電話モデルを宣伝するために、彼らは電話の以前のモデルを所有している顧客に電話の新機能認知（ビジネスを）推進することを目指しています。

リリースを宣伝するために、電子メールアドレスを識別子として使用して、CRM データベースから Experience Platform に電子メールアドレスアップロードしました。 Audiencesは、古い電話モデルを所有している顧客に基づいて作成されます。 その後、オーディエンスは[!DNL Google Customer Match]に送信されるため、会社は、現在の顧客、古い電話モデルを所有している顧客、および[!DNL Google Display & Video 360]、[!DNL Search]、[!DNL YouTube]、[!DNL Gmail]などの[!DNL Google Display Network]プロパティの同様の顧客をターゲットできます。
...
次に、[!DNL Google] アカウントが [!DNL Standard] 以上の権限レベルに設定されていることを確認してください。 詳しくは [Google 広告のドキュメント](https://support.google.com/google-ads/answer/9978556?visit_id=637611563637058259-4176462731&rd=1) をご覧ください。

### アカウントのリンク要件 {#linking}

この宛先コネクタを設定する前に、お使いのGoogle アカウント ID をAdobeのGoogle アカウント ID `4641108541` にリンクする必要があります。
...
準拠アカウントをお持ちのお客様は、Google によって自動的に許可リストに登録されます。

## ID の一致要件 {#id-matching-requirements}

[!DNL Google] では、個人を特定できる情報(PII)を明確に送信しないことを要求します。 したがって、 [!DNL Google Customer Match] にアクティブ化されたオーディエンスは、ハッシュ化された電子メールアドレスや電話番号などの *ハッシュ化された* 識別子をキーオフする必要があります。
...
[!DNL Google Customer Match + Display & Video 360] 接続の監視情報には、各データフローおよびデータフローの実行でアクティブ化された ID、除外された ID、および失敗した ID に関連する オーディエンス レベルの情報が含まれます。[詳細](/help/dataflows/ui/monitor-destinations.md#segment-level-view) 機能についてお読みください。

## オーディエンスアクティベーションが成功したことを確認する {#verify-activation}

アクティベーション フローを完了したら、 **[!UICONTROL Google Ads]** アカウントに切り替えます。 アクティブ化されたオーディエンスは、Google アカウントに顧客リストとして表示されます。 オーディエンスサイズによっては、提供するアクティブユーザーが 1000 人以上でない限り、一部のオーディエンスが入力されない場合があります。 詳しくは、[Google オーディエンスパートナーのドキュメント ](https://developers.google.com/audience-partner/api/docs/customer-match/get-started#verify-list) を参照してください。 リンク内のドキュメントにアクセスするには、Googleに問い合わせる必要があります。
