...
この宛先を使用して、Experience Cloud ID （ECID）とAdformのID Fusionに基づいて、Adobe [!DNL Real-Time CDP] オーディエンスをAdformに送信してアクティベーションします。 AdformのID Fusionは、Experience Cloud ID （ECID）に基づいて1st パーティオーディエンスをアクティブ化できるAdformのID解決サービスです。
