...
| [!DNL Tradedesk] | [!DNL TDID] プラットフォームの[!DNL The Trade Desk] | プロファイルにECIDがあり、ECIDとTrade Desk ID マッピングがExperience Platformに存在する場合にアクティブ化されます。 |
...
>abstract="これら 4 つのマッピングセットは事前設定されています。The Trade Deskにデータをアクティベートする場合、アクティベートされたオーディエンスに適格なプロファイルには、プロファイルに4つのIDがすべて存在する必要はありません。この宛先は、ここに示されているターゲット IDのいずれかで動作するからです。 <br> The Trade Desk IDに基づくCookie ベースのターゲティングの場合、プロファイルにECIDが存在し、The Trade Desk IDとECIDの間にID同期マッピングが必要です。"
>additional-url="https://experienceleague.adobe.com/ja/docs/experience-platform/destinations/catalog/advertising/tradedesk#preconfigured-mappings" text="詳しくは、事前設定されたマッピングを参照してください。"
