...
[!DNL Microsoft Ads Customer Match]宛先を使用する方法とタイミングをより深く理解するために、[!DNL Adobe Experience Platform]のお客様がこの機能を使用して解決できるユースケースの例を次に示します。
...
あるハイテク企業が新製品を発売した。 新製品のプロモーションでは、以前に関連製品を購入したことがある顧客の認知度を高めることを目的としています。 メールアドレスを識別子としてCRM データベースからExperience Platformにアップロードします。 オーディエンスは、関連商品を所有する顧客に基づいて作成されます。 これらのオーディエンスは[!DNL Microsoft Ads Customer Match]に送信されるので、会社は[!DNL Microsoft Advertising Network]の現在の顧客や類似の顧客をターゲットにできます。
...
| ターゲット ID | 説明 | 注意点 |
|---|---|---|
| `email` | プレーンテキストのメールアドレス | プレーンテキストの電子メールアドレスのみが[!DNL Microsoft Ads Customer Match]接続でサポートされています。 Experience Platformは、Microsoftの要件に一致するように、書き出し時にメールアドレスを自動的にハッシュします。 |
...
>[!IMPORTANT]
>
>ハッシュ化されていない（プレーンテキスト）ソースフィールドを使用する必要があります。 `Emails (SHA256, lowercased)`などの事前にハッシュ化されたソース IDは使用しないでください。 Experience Platformは、Microsoftの要件に一致するように、書き出し時にメールアドレスを自動的にハッシュ化します。
