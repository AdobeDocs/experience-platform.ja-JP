...
* DSP アカウントまたは広告主のソースキー。DSP[[!DNL Real-Time CDP] で](https://experienceleague.adobe.com/docs/advertising-cloud/dsp/audiences/sources/source-create.html) ソースが作成されたときに生成されます。 DSP アカウントチームがこのキーを共有します。 Advertising DSPの宛先への宛先接続を作成するには、Experience Platform内で使用します。以下の[の説明に従ってください](#authenticate)。
