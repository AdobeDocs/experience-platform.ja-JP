...
へのライブアウトバウンド接続を作成します [!DNL Amazon S3] Adobe Experience Platformから独自の S3 バケットにデータファイルを定期的に書き出すストレージ。
...
の場合 [!DNL Amazon S3] 宛先、 [!DNL Platform] 指定したストレージの場所にデータファイルが作成されます。 ファイルの詳細については、 [プロファイルの一括書き出し先に対するオーディエンスデータのアクティブ化](../../ui/activate-batch-profile-destinations.md) （セグメントのアクティベーションに関するチュートリアル）。
