...
[!DNL Azure Event Hubs] ストレージ へのリアルタイムのアウトバウンド接続を作成して、Adobe Experience Platform からデータをストリーミングできます。
...
* プログラムで [!DNL Azure Event Hubs] に接続するには、 [ストリーミング宛先 API チュートリアル](../../api/streaming-destinations.md) を参照してください。
* Experience Platform ユーザー インターフェイスを使用して [!DNL Azure Event Hubs] に接続するには、以下のセクションを参照してください。
...
例えば、ある見込み客がダウンロードしたホワイトペーパーによって、「コンバージョンする傾向の高い」セグメントに見込み客を選定するとします。 見込み客が含まれるオーディエンスを [!DNL Azure Event Hubs] の宛先にマッピングすると、[!DNL Azure Event Hubs] でこのイベントを受け取ります。 そこでは、日曜大工のアプローチを採用し、企業のITシステムで最適と思われるイベントの上にビジネスロジックを説明することができます。
...
顧客のセキュリティとコンプライアンスの要件を満たすために、Experience Platform では、 [!DNL Azure Event Hubs] 宛先を許可リストに登録できる静的 IP のリストを提供しています。 許可リストに使用できる IP のリストについて詳しくは、[ストリーミング宛先用 IP アドレスの許可リスト](/help/destinations/catalog/streaming/ip-address-allow-list.md)を参照してください。
...
* **[!UICONTROL SAS Key]**: Event Hubs 名前空間の主キー。 `sasPolicy`が対応する`sasKey`には、Event Hubs リストを設定するために **管理** 権限が構成されている必要があります。SAS キーを使用した [!DNL Azure Event Hubs] への認証については、 [マイクロソフトのドキュメント](https://docs.microsoft.com/en-us/azure/event-hubs/authenticate-shared-access-signature)を参照してください。
...
* **[!UICONTROL eventHubName]**: [!DNL Azure Event Hubs] 宛先へのストリームの名前を指定します。
* **[!UICONTROL Include Segment Names]**:データのエクスポートに、エクスポートするオーディエンスの名前を含めるかどうかが切り替わります。 このオプションを選択したデータの書き出しの例については、[書き出されたデータ](#exported-data)の節を参照してください。
...
既存の宛先に新しいオーディエンスを追加するか、新しい宛先を作成してオーディエンスをその宛先にマッピングすると、Experience Platform は履歴オーディエンス認定データを宛先にエクスポートします。 オーディエンス *以前* に適合し、オーディエンスが宛先に追加されたプロファイルは、約 1 時間以内に宛先に書き出されます。
...
+++ 以下のデータ エクスポート サンプルでは、 `segmentMembership` セクションにオーディエンス名前が含まれています
