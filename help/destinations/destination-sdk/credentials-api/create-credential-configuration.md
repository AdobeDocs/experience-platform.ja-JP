...
このドキュメントを読むと、資格情報エンドポイントを使用するタイミングと、`/authoring/credentials` API エンドポイントを使用して資格情報設定を設定する方法が分かりました。[Destination SDKを使用して宛先を設定する方法](../guides/configure-destination-instructions.md)を参照して、この手順が宛先を設定するプロセスにどの程度適合するかを理解します。
