...
| `hasSegments` | この関数は、名前空間オーディエンス ID のマップをパラメーターとして取ります。 この関数は、を返します。 `true` （ステータスに関係なく）マップに少なくとも 1 つのオーディエンスが存在する場合、 `false` それ以外の場合は この関数を使用して、オーディエンスのマップを繰り返し処理するかどうかを決定できます。 | `hasSegments(input.profile.segmentMembership)` |
| `destination.namespaceSegmentAliases` | 特定のAdobe Experience Platform名前空間のオーディエンス ID からパートナーのシステム内のオーディエンスエイリアスにマッピングします。 | `destination.namespaceSegmentAliases["ups"]["seg-id-1"]` |
