...
動的ドロップダウンセレクターは、と同じように見えます。 [標準ドロップダウンセレクター](#dropdown-selectors) UI で上書きできます。 唯一の違いは、値が API から動的に取得される点です。

動的ドロップダウンセレクターを作成するには、次の 2 つのコンポーネントを設定する必要があります。

**手順 1.** [宛先サーバーの作成](../../authoring-api/destination-server/create-destination-server.md#dynamic-dropdown-servers) （を使用） `responseFields` 以下に示すように、動的 API 呼び出し用のテンプレート。
...
**手順 2.** の使用 `dynamicEnum` 次に示すように、オブジェクトです。 以下の例では、 `User` ドロップダウンは、動的サーバーを使用して取得されます。
...
を `destinationServerId` 手順 1 で作成した宛先サーバーの ID に対するパラメーター。 の応答で宛先サーバー ID を確認できます [宛先サーバー設定の取得](../../authoring-api/destination-server/retrieve-destination-server.md) API 呼び出し。
...
ネストされたフィールドを追加するには、 `properties` パラメーターで次のように指定します。 以下の設定例では、内に 3 つの異なるネストされたフィールドを確認できます **宛先 – 統合固有の設定** 顧客データフィールド。
...
より広いコンテキストでは、次の項目を表示できます `conditional` 以下の宛先設定で、と共に使用されているフィールド `fileType` 文字列と `csvOptions` が定義されているオブジェクト。 条件付きフィールドは、 `properties` パラメーター。
