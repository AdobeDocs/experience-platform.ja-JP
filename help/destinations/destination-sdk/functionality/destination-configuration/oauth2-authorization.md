...
## 宛先設定にOAuth 2認証の詳細を追加する方法 {#how-to-setup}

### システムの前提条件 {#prerequisites}

最初の手順として、[!DNL Adobe Experience Platform]のシステムでアプリを作成するか、またはシステムにExperience Platformを登録する必要があります。 目標は、宛先に対するExperience Platformの認証に必要なクライアント IDとクライアントシークレットを生成することです。

システムのこの設定の一部として、以下のリストから取得できる[!DNL Adobe Experience Platform] OAuth 2 リダイレクト/コールバック URLが必要です。
...
>[!IMPORTANT]
>
>システムに[!DNL Adobe Experience Platform]のリダイレクト/コールバック URLを登録する手順は、認証コード [付与タイプの](#authorization-code)OAuth 2にのみ必要です。 その他の 2 つのサポートされる付与タイプ（パスワードとクライアント資格情報）の場合、この手順をスキップできます。
...
>[!IMPORTANT]
>
>* 以下の節で指示されているように、入力パラメーターを指定します。Adobeの内部システムは、プラットフォームの認証システムに接続し、出力パラメーターを取得します。このパラメーターは、ユーザーの認証と宛先への認証の維持に使用されます。
>* テーブルで太字で強調表示されている入力パラメーターは、OAuth 2認証フローで必要なパラメーターです。 その他のパラメーターはオプションです。ここに示されていないその他のカスタム入力パラメーターについては、[OAuth 2 設定のカスタマイズ](#customize-configuration)および[アクセストークンの更新](#access-token-refresh)の節で詳しく説明しています。
...
上記の表には、標準的な OAuth 2 フローで使用されるフィールドが示されています。これらの標準的なフィールドに加えて、様々なパートナー統合では、追加の入力および出力が必要になる可能性があります。Adobeは、Destination SDK用に柔軟なOAuth 2認証フレームワークを設計し、上記の標準フィールドパターンに対するバリエーションを処理できるだけでなく、期限切れのアクセストークンなどの無効な出力を自動的に再生成するメカニズムをサポートしています。

あらゆる場合の出力にはアクセストークンが含まれており、Experience Platformが宛先の認証と認証を維持するために使用します。

AdobeがOAuth 2認証用に設計したシステム：

* 3 つの OAuth 2 付与をすべてサポートすると同時に、それらのバリエーション（追加のデータフィールド、非標準的な API 呼び出しなど）を考慮します。
* 様々なライフタイム値を持つアクセストークンをサポートします。 Adobeでは、トークンの有効期間を最低24時間に設定することをお勧めします。
...
宛先に対してこの認証メソッドを設定するには、[宛先設定を作成](../../authoring-api/destination-configuration/create-destination-configuration.md)するときに、次の行を設定に追加します。
...
| `options.useBasicAuth` | ブール | *オプション*。アクセストークンの認証コードを交換する際に、クライアントの資格情報（クライアント IDとクライアント秘密鍵）をOAuth プロバイダーのトークンエンドポイントに送信する方法を制御するブール値。 <ul><li>`false`または未定義に設定した場合、認証情報はPOST リクエスト本文の`client_id`および`client_secret` パラメーターとして送信されます（デフォルトの動作）。</li><li>このパラメーターが`true`に設定されている場合、基本認証形式`Authorization`を使用して、HTTP `Authorization: Basic base64(clientID:clientSecret)` ヘッダーに資格情報が送信されます。</li></ul> OAuth プロバイダーがリクエスト本文ではなく`useBasicAuth` ヘッダーでクライアント資格情報を送信する必要がある場合は、`true`を`Authorization`に設定します。 |

{style="table-layout:auto"}

## パスワード付与を使用した OAuth 2 {#oauth2-password-grant}

OAuth 2 パスワード付与（[RFC 標準仕様](https://tools.ietf.org/html/rfc6749#section-4.3)を参照）の場合、Experience Platform には、ユーザーのユーザー名とパスワードが必要です。認証フローでは、Experience Platformはこれらの資格情報をアクセストークンと必要に応じて更新トークンと交換します。
...
### 例1: `authenticationDataFields`を使用して、承認応答から取得された情報を取得する {#example-1}
...
| `authenticationDataFields.isRequired` | ブール | 認証フローでカスタムデータフィールドが必要かどうかを指定します。 |
| `authenticationDataFields.format` | 文字列 | `"format":"password"`を選択すると、Adobeは認証データフィールドの値を暗号化します。 `"fieldType": "CUSTOMER"` と共に使用された場合、これも、ユーザーがフィールドに入力する際に UI で入力を非表示にします。 |
...
認証のカスタマイズに応じて、前の節に示すように、認証応答のデータフィールドにアクセスする必要がある場合があります。 これを行うために、アドビが使用する [Pebble テンプレート言語](https://pebbletemplates.io/)についてよく理解し、OAuth 2 実装をカスタマイズするための以下のテンプレート規則を参照してください。
...
| userContext | 現在の承認試行に関する情報へのアクセス | <ul><li>`{{ userContext.sandboxName }}`</li><li>`{{ userContext.sandboxId }}`</li><li>`{{ userContext.imsOrgId }}`</li><li>`{{ userContext.client }} // the client executing the authorization attempt`</li></ul> |
