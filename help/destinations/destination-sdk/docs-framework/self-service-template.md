...
>[!IMPORTANT]
>
>この宛先コネクタとドキュメントページは、*YourDestination* チームが作成および管理します。 お問い合わせや更新のリクエストについては、*リンクまたはメールアドレスを挿入してください。このアドレスやメールアドレスなどに連絡して、更新を受けることができます `support@YourDestination.com`。*
...
*宛先でサポートされているオーディエンスに関する情報を、この節で追加します。 テーブルには、いくつかの標準値があらかじめ入力されています。 `Yes` と `No` を使用して、オーディエンスタイプがこの宛先でサポートされているかどうかをマークします。*
...
| 項目 | タイプ | メモ |
|---------|----------|---------|
| 書き出しタイプ | **[!UICONTROL Audience export]** | *YourDestination* 宛先で使用される識別子（名前、電話番号など）を使用して、オーディエンスのすべてのメンバーを書き出します。 |
...
| 書き出しタイプ | **[!UICONTROL Dataset export]** | オーディエンスの関心や選定別にグループ化または構造化されていない未加工のデータセットを書き出しています。 |
