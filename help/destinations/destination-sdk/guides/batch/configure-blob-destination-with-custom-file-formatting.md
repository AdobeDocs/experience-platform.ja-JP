---
description: Destination SDKを使用して、カスタムファイル形式オプションとカスタムファイル名設定を使用してAzure Blob Storageの宛先を設定する方法を説明します。
title: カスタムファイル形式オプションとカスタムファイル名設定を使用して、Azure Blob Storageの宛先を設定します。
...
# カスタムファイル形式オプションとカスタムファイル名設定を使用して[!DNL Azure Blob Storage]宛先を設定します

## 概要 {#overview}

ここでは、Destination SDKを使用して、カスタム [!DNL Azure Blob Storage] ファイル形式オプション [とカスタム ](configure-file-formatting-options.md) ファイル名設定[を使用して](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration)宛先を設定する方法について説明します。

このページでは、Azure Blob Storageの宛先で使用できるすべての設定オプションを示します。 必要に応じて、以下の手順に示す設定を編集したり、設定の特定の部分を削除したりできます。
...
以下のペイロードには、一般的なAzure Blob設定が含まれており、ユーザーがExperience Platform UIで定義できるカスタム [CSV ファイル形式](../../functionality/destination-server/file-formatting.md)の設定パラメーターが含まれています。
...
![選択した宛先カードを含む宛先カタログページを表示する画面の記録。](../../assets/guides/batch/blob-destination-card.gif)
...
Destination SDKを使用してカスタム [!DNL Azure Blob Storage]宛先をオーサリングする方法を理解しました。 次に、チームはファイルベースの宛先[の](../../../ui/activate-batch-profile-destinations.md) アクティベーションワークフローを使用して、宛先にデータを書き出すことができます。
