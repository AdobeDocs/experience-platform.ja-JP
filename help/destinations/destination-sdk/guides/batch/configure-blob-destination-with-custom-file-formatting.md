---
description: Destination SDKを使用して、カスタムのファイル形式オプションとカスタムのファイル名設定で Azure Blob ストレージの宛先を設定する方法を説明します。
title: カスタムのファイル形式オプションとカスタムのファイル名設定を使用して、Azure Blob ストレージの宛先を設定します。
...
# カスタムのファイル形式オプションとカスタムのファイル名設定を使用した、[!DNL Azure Blob Storage] の宛先の設定

## 概要 {#overview}

ここでは、Destination SDKを使用して、カスタムの [ ファイル形式オプション ](configure-file-formatting-options.md) およびカスタムの [ ファイル名設定 ](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration) で [!DNL Azure Blob Storage] しい宛先を設定する方法について説明します。

このページには、Azure Blob ストレージ宛先に使用できるすべての設定オプションが表示されます。 必要に応じて、以下の手順に示す設定を編集したり、設定の特定の部分を削除したりできます。
...
以下のペイロードには、汎用の Azure Blob 設定と、ユーザーがExperience PlatformUI で定義できるカスタムの [CSV ファイル形式 ](../../functionality/destination-server/file-formatting.md) 設定パラメーターが含まれています。
...
![ 選択した宛先カードを含む宛先カタログページを示す画面録画。](../../assets/guides/batch/blob-destination-card.gif)
...
この記事を読むことで、Destination SDKを使用したカスタム [!DNL Azure Blob Storage] 宛先の作成方法を理解しました。 次に、チームは [ ファイルベース宛先のアクティベーションワークフロー ](../../../ui/activate-batch-profile-destinations.md) を使用して、宛先にデータを書き出すことができます。
