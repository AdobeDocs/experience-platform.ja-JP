...
title: カスタムのファイル形式オプションとカスタムのファイル名設定を使用した、Azure Blob ストレージの宛先の設定.
...
このページでは、Destination SDKを使用して [!DNL Azure Blob Storage] カスタムの宛先 [ファイル形式オプション](../../server-and-file-configuration.md#file-configuration) そして習慣 [ファイル名設定](../../file-based-destination-configuration.md#file-name-configuration).

このページには、Azure BLOB ストレージの宛先で使用できるすべての構成オプションが表示されます。 以下の手順で示す設定を編集したり、必要に応じて設定の特定の部分を削除したりできます。
...
まず、 `/destination-server` エンドポイント：サーバーとファイルの設定を作成します。 HTTP リクエストのパラメーターについて詳しくは、 [ファイルベースの宛先のサーバーおよびファイル構成の仕様](../../server-and-file-configuration.md#blob-example) そして関連する [ファイル形式設定](../../server-and-file-configuration.md#file-configuration).
...
次のリクエストは、ペイロード内のパラメーター設定に基づいて、新しい宛先サーバー設定を作成します。以下のペイロードには、カスタムの汎用 Azure BLOB 設定が含まれています [CSV ファイル形式](../../server-and-file-configuration.md#file-configuration) ユーザーが設定 UI で定義できるExperience Platformパラメーター。
...
* [認証設定](../../authentication-configuration.md#blob)
...
![選択した宛先カードを含む宛先カタログページを示す画面記録。](../../assets/blob-destination-card.gif)
...
この記事を読むと、カスタムの作成方法がわかります [!DNL Azure Blob Storage] 宛先を指定します。Destination SDKを使用します。 次に、チームが [ファイルベースの宛先のアクティベーションワークフロー](../../../ui/activate-batch-profile-destinations.md) をクリックして、宛先にデータを書き出します。
