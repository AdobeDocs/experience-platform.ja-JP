...
| セグメント評価ジョブの遅延、失敗、エラー | セグメントジョブの遅延 | このアラートは、セグメントジョブの完了に 150 分以上かかった場合にトリガーします。<br> 次のいずれかのステータスが表示されます。 <br>- FIRING — 失敗または遅延の条件が満たされています（ACTIVE 状態であると考えてください）。 <br>- INACTIVE — 条件が満たされていないか、解決されていません（RESOLVED 状態であると見なします）。 |
