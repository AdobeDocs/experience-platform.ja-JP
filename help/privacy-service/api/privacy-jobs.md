...
description: プライバシーサービス API を使用して、経験のあるクラウドアプリケーションのプライバシージョブを管理する方法について説明します。
...
このドキュメントでは、API 呼び出しを使用して、プライバシージョブを操作する方法について説明します。 具体的には、API でのエンドポイントの使用について説明して `/job` [!DNL Privacy Service] います。 このガイドを読む前に、「入門ガイド」を参照してください [ ](./getting-started.md) 。 api の呼び出しについては、必要なヘッダーやコード例を参照してください。

>[!NOTE]
>
>お客様からの同意またはオプトアウト要求を管理している場合は、「 [ 同意エンドポイントガイド」を参照してください ](./consent.md) 。

## すべてのジョッブをリスト {#list}

エンドポイントに GET 要求を作成することによって、組織内で利用可能なすべてのプライバシージョブの一覧を表示することができ `/jobs` ます。

**API 形式**

この要求フォーマットは、 `regulation` 次に示すように、エンドポイントでクエリパラメーターを使用し `/jobs` ます。そのため、次に示すように、疑問符 () で始まり `?` ます。 応答はページ分割され、他のクエリパラメーター（`page` および `size`）を使用して応答をフィルターできます。アンパサンド（`&`）を使用して、複数のパラメーターを区切ることができます。
...
| パラメーター | 説明 |
| --- | --- |
| `{REGULATION}` | クエリする規制の種類。指定できる値は次のとおりです。 <ul><li>`gdpr` (欧州連合)</li><li>`ccpa` California</li><li>`lgpd_bra` (ブラジル)</li><li>`nzpa_nzl` (ニュージーランド)</li><li>`pdpa_tha` (タイ)</li></ul> |
...
新しいジョブリクエストを作成する前に、まず、データにアクセス、削除、またはオプトアウトするデータ主体の識別情報を収集する必要があります。必要なデータを取得したら、そのデータをエンドポイントへの POST 要求のペイロードで指定する必要があり `/jobs` ます。

>[!NOTE]
>
> 互換性のある Adobe Experience Cloud アプリケーションは、データの主題を識別するために異なる値を使用します。アプリに必要な識別子について詳しくは、[Privacy Service および Experience Cloud アプリケーションの](../experience-cloud-apps.md)に関するガイドを参照してください。どの Id に送信するかを決定する方法の詳細については [!DNL Privacy Service] 、 [ プライバシー要求に含まれる id データに関するドキュメントを参照してください ](../identity-data.md) 。

この [!DNL Privacy Service] API は、個人データに対する2種類のジョブ要求をサポートします。
...
| `expandIDs` | 「」に設定されている場合は、 `true` アプリケーションの id を処理するための最適化を表す、(現在はサポートされて [!DNL Analytics] います) オプションのプロパティです。 省略した場合、この値はデフォルトで `false` になります。 |
...
| `regulation` **(必須)** | プライバシージョブの規制。 次の値を使用できます。 <ul><li>`gdpr` (欧州連合)</li><li>`ccpa` California</li><li>`lgpd_bra` (ブラジル)</li><li>`nzpa_nzl` (ニュージーランド)</li><li>`pdpa_tha` (タイ)</li></ul> |
...
このようなジョブは、そのジョブの現在の状態などの情報を取得するために、 `jobId` GET 要求のパスに含めることができ `/jobs` ます。
...
| パラメーター | 説明 |
| --- | --- |
| `{JOB_ID}` | 検索対象のジョブの ID。 この ID は、によって、 `jobId` [ ジョブを作成し、すべてのジョブを一覧表示するために、正常に API 応答として返され ](#create-job) [ ](#list) ます。 |
...
| プロパティ | 説明 |
| --- | --- |
| `productStatusResponse` | 配列内の各オブジェクトには、 `productResponses` 特定のアプリケーションに関するジョブの現在の状態に関する情報が含まれてい [!DNL Experience Cloud] ます。 |
| `productStatusResponse.status` | ジョブの「現在の状態」カテゴリー。 [使用可能なステータスカテゴリ ](#status-categories) とそれに対応する意味の一覧については、次の表を参照してください。 |
| `productStatusResponse.message` | 「状態」カテゴリに対応する、ジョブの具体的な状態です。 |
| `productStatusResponse.responseMsgCode` | で受信した製品応答メッセージの標準コード [!DNL Privacy Service] です。 メッセージの詳細については、で説明されてい `responseMsgDetail` ます。 |
| `productStatusResponse.responseMsgDetail` | ジョブの状態について詳しく説明します。 類似した状態のメッセージは製品によって異なる場合があります。 |
| `productStatusResponse.results` | 一部の製品では、 `results` によって扱われていない追加情報を提供するオブジェクトが返されることがあり `responseMsgDetail` ます。 |
| `downloadURL` | ジョブのステータスが `complete` の場合、この属性はジョブの結果を ZIP ファイルとしてダウンロードする URL を指定します。このファイルは、ジョブの完了後 60 日間ダウンロードできます。 |

{style = &quot;テーブル-layout: auto&quot;}

### ジョブ状態カテゴリ {#status-categories}

以下の表に、使用可能な各ジョブ状態カテゴリと、それに対応する意味を示します。
...
>[!NOTE]
>
>送信されたジョブは、 `processing` まだ処理中の依存している子ジョブがある場合、状態のままになります。

## 次の手順

この API を使用して、プライバシージョブを作成および監視する方法を学習しました [!DNL Privacy Service] 。 ユーザーインターフェイスを使用して同じタスクを実行する方法について詳しくは、「[Privacy Service UI の概要](../ui/overview.md)」を参照してください。
