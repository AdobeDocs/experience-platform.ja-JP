...
[!DNL Experience Data Model] (XDM) は標準化されたフレームワークで、 [!DNL Platform] は顧客体験データを整理します。 に取り込まれるすべてのデータ [!DNL Platform] で保持するには、事前定義済みの XDM スキーマに準拠する必要があります。 [!DNL Data Lake] データセットとして。
