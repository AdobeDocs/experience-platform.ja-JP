...
この呼び出しは、`extensions.adobe_lakeHouse.rowExpiration` セクションに現在の `ttlValue` が設定されている場合、その値を返します。
...
+++回答
時系列スキーマを使用する任意のデータセットに、TTL ベースの保持ポリシーを適用できます。 これには、標準の XDM ExperienceEvent クラスに基づくデータセットと、XDM Time Series クラスを拡張するカスタムスキーマが含まれます。

行レベルの有効期限には、次の技術的条件が必要です。

- スキーマは、XDM 時系列の基本クラスを拡張する必要があります。
