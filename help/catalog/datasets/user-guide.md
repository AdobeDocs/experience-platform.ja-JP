...
[!UICONTROL  データセット ] ワークスペースの [!UICONTROL  参照 ] タブから、データセットレベルでのデータセットの有効期限および保持ポリシーの管理。 この機能を使用すると、データレイクおよびプロファイルサービスに既に取り込まれているデータの保持ポリシーを設定できます。 有効期限は、データが Platform に取り込まれた日時と、保持ルールに基づいています。

[!UICONTROL  データ保持の設定 ] ダイアログを開くには、データセットの横にある省略記号を選択し、ドロップダウンメニューから **[!UICONTROL データ保持ポリシーを設定]** を選択します。
...
[!UICONTROL  データ保持の設定 ] ダイアログが表示されます。 このダイアログには、サンドボックスレベルのライセンス使用指標、データセットレベルの詳細およびデータレイクの設定が表示されます。 これらの指標は、使用権限と比較した使用状況を示します。 データセットの詳細には、データセット名、タイプ、プロファイル有効化ステータスおよび現在のデータレイクストレージの使用状況が含まれます。

>[!NOTE]
>
>サンドボックスレベルでライセンス認証されたデータレイクのストレージ指標は、まだ開発中で、使用できません。

![ データ保持の設定ダイアログ ](../images/datasets/user-guide/set-data-retention-dialog.png)

データセット保持ポリシーを設定する前に、ダイアログに推奨される保持設定が表示されます。 デフォルトの推奨保存期間は 1 か月です。 標準の保持ポリシーを調整するには、数値を選択して更新してから、目的の期間（日、月、年）を選択します。 データレイクとプロファイルサービスの保持設定は、それぞれ独立して設定できます。

>[!NOTE]
> 
>データレイクの最小データ保持期間は 30 日です。 プロファイルサービスの最小データ保持期間は 1 日です。

![ 期間のドロップダウンと「保存」がハイライト表示されたデータ保持を設定ダイアログ ](../images/datasets/user-guide/time-unit-dropdown.png)

データセットの有効期限の日付範囲を定義するルールと、データ保持ポリシーを設定するためのベストプラクティスについて詳しくは、[ よくある質問 ](../catalog-faq.md) ページを参照してください。

#### （Beta）保存期間とストレージ指標の可視性の向上 {#retention-and-storage-metrics}

ベータ版ユーザーは、**[!UICONTROL Data Lake Storage]**、**[!UICONTROL Data Lake Retention]**、**[!UICONTROL Profile Storage]**、**[!UICONTROL Profile Retention]** の 4 つの新しい列を使用して、データ管理をより明確に把握できます。 これらの指標は、データレイクとプロファイルサービスの両方で、データが消費するストレージとその保持期間を示します。 これらの詳細は、保持ポリシーの最適化、使用権限に対する使用状況の追跡、組織や規制の標準へのコンプライアンスの確保に役立ちます。 この可視性の向上により、十分な情報に基づいた意思決定、コスト管理、ガバナンスの合理化、データ状況の明確な理解が可能になります。

![4 つの新しいストレージ列と保持列がハイライト表示されたデータセットワークスペースの「参照」タブ。](../images/datasets/user-guide/storage-and-retention-columns.png)

次の表に、ベータ版リリースで使用できる新しい保持およびストレージ指標の概要を示します。 各列の目的と、Platform UI 内でのデータ保持とストレージの管理にどのように役立つかについて詳しく説明します。

| 列タイトル | 説明 |
|---|---|
| [!UICONTROL  データレイクの保持 ] | 各データセットの現在の保持期間を表示します。 この値は、各データセットの保持設定で変更できます。Data Lake の保持ポリシーでは、データを保存する期間と、異なるサービスでいつ削除するかのルールを設定します。 |
| [!UICONTROL Data Lake ストレージ ] | データレイク内の各データセットの現在のストレージ使用状況を表示します。 この指標は、各データセットが占有するスペースを追跡するのに役立ち、ストレージ制限の管理と使用の最適化に役立ちます。 |
| [!UICONTROL  プロファイルストレージ ] | プロファイルサービス内の各データセットの現在のストレージ使用状況を表示します。 この情報を使用してストレージ消費を監視し、データ管理目標に適合していることを確認します。 |
| [!UICONTROL  プロファイル保持 ] | 各プロファイルデータセットの保持期間を示します。 この値は、データセットの保持設定で調整でき、削除前にプロファイルデータを保存する期間を制御するのに役立ちます。 |
