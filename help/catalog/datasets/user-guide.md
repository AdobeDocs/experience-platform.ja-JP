...
* [[!DNL Experience Data Model (XDM) System]](../../xdm/home.md)：[!DNL Experience Platform] が、カスタマーエクスペリエンスデータを整理する際に使用する、標準化されたフレームワーク。
   ...
   * [スキーマ エディター](../../xdm/tutorials/create-schema-ui.md): [!DNL Schema Editor] ユーザー インターフェイス内の [!DNL Experience Platform] を使用して、独自のカスタム XDM スキーマビルド方法を学びます。
...
データセット UI で、使用可能な各データセットに対してインラインアクションのコレクションが提供されるようになりました。 管理するデータセットの省略記号 (...) を選択して、使用可能なオプションをポップアップメニューに表示します。 使用可能なアクションは次のとおりです。
...
これらの使用可能なアクション詳細、それぞれの節を参照してください。 多数のデータセットを同時に管理方法については、「 [一括操作](#bulk-actions) のセクションを参照してください。
...
* データタイプの各列名の横に、データセットの構造へのインサイトインジケーターが表示されます。
* ウィンドウの上部に SQL クエリが表示され、データセットの生成に使用されたクエリが表示されます。
...
組織が Data Distiller ライセンスを持っている場合は、データセットプレビューウィンドウから直接 [!UICONTROL Advanced Query Editor] にアクセスできます。 このショートカットを使用すると、サンプル データのプレビューから Query サービス でのクエリの実行と調整にシームレスに移行できます。

>[!AVAILABILITY]
>
>[!UICONTROL Advanced Query Editor]へのアクセスは、データDistiller SKUライセンス版を持つ組織に制限されています。組織に必要なライセンス版がない場合、このオプションは [データセットプレビュー] ウィンドウに表示されません。

プレビュー ウィンドウの右上にある [ [!UICONTROL Advanced Query Editor] ] を選択して、現在の SQL クエリが事前に読み込まれて実行されたクエリ サービス を開きます。 クエリを再入力せずに SQL の分析または変更を続行できます。

![右上に詳細クエリエディターボタンが表示されているデータセットプレビューウィンドウ。](../images/datasets/user-guide/dataset-preview-advanced-query-editor.png)

追加の分析には、 [!DNL Query Service] や [!DNL JupyterLab] などのダウンストリーム サービスを使用します。 詳しくは、次のドキュメントを参照してください。
...
[!UICONTROL Manage tags]ダイアログでは、データセットから既存のタグを削除することもできます。削除するタグの横にある「x」を選択し、 **[!UICONTROL Save]**&#x200B;を選択するだけです。

タグがデータセットに追加されると、対応するタグに基づいてデータセットをフィルタリングできます。 詳細については、 [データセットをタグでフィルタリング](#enable-profile) 方法に関するセクションを参照してください。

検出と分類を容易にするためにビジネス オブジェクトを分類する方法の詳細については、 [メタデータ分類の管理](../../administrative-tags/ui/managing-tags.md)に関するガイドを参照してください。 このガイドでは、適切なアクセス許可を持つユーザーが定義済みのタグを作成し、それらをカテゴリに割り当て、関連するすべての CRUD 操作を Experience Platform UIで管理する方法について説明します。
...
>[!NOTE]
> 
>データ レイクの最小保持期間は 30 日です。 プロフィールサービスの最小保持期間は 1 日です。
>
>さらに、プロフィールサービスの保持期間は 30 日ごとにのみ更新できます。
...
グラフには、現在選択されているデータセットのさまざまな保持期間にわたるエクスペリエンスイベントの分布が表示されます。 各バーにカーソルを合わせると、選択した保持期間が適用された場合に削除されるレコードの正確な数が表示されます。

ビジュアル予測を使用して、さまざまな保持期間の影響を評価し、情報に基づいたビジネス上の意思決定を行うことができます。 たとえば、30 日間の保持期間を選択し、グラフにデータの 60% が削除されることが示されている場合は、保持期間を延長して、より多くのデータを分析保持することを選択できます。

>[!NOTE]
>
>エクスペリエンスイベントの分布グラフは、選択したデータセットに固有であり、そのデータのみを反映します。 これは、データ レイクに格納されているデータにのみ適用されます。

![エクスペリエンスイベントの配分グラフが表示された設定データ保持ダイアログ。](../images/datasets/user-guide/visual-forecast.png)
...
データセット [!UICONTROL Move] フォルダーに追加ダイアログが表示されます。 オーディエンスの移動先のフォルダーを選択し、[ **[!UICONTROL Move]**] を選択します。 データセット移動が正常に行われたことを通知するポップアップ通知が表示されます。

![[!UICONTROL Move]データセットダイアログで[!UICONTROL Move]ハイライト表示されます。](../images/datasets/user-guide/move-dialog.png)

>[!TIP]
>
>移動データセットダイアログから直接フォルダを作成することもできます。 フォルダーを作成するには、フォルダーの作成アイコン (![フォルダーの作成アイコン) を選択します。](/help/images/icons/folder-add.png))をクリックして、ツールの右上に移動します。
>
...
データセットプロフィールが有効になっている場合、UIを使用してそのデータセットを削除すると、データ レイク、IDサービスサービス、およびプロフィール ストア内のそのデータセットに関連付けられているプロファイル データから削除されます。

Real-時間 Customer プロフィール API を使用して、データセットに関連付けられているプロファイルデータを [!DNL Profile] ストアから削除できます (データはデータ レイクに残ります)。 詳しくは、 [プロファイルシステムジョブ API エンドポイントのガイド](../../profile/api/profile-system-jobs.md) を参照してください。
...
適用されたフィルターのリストは、フィルターされた結果の上に表示されます。
...
### プロフィールが有効なデータセットフィルター {#filter-profile-enabled-datasets}
...
[!UICONTROL Browse] タブ 内のデータセットは、昇順または降順日付で並べ替えることができます。[!UICONTROL Created]または[!UICONTROL Last updated]の列見出しを選択して、昇順と降順を入れ替えます。選択すると、列ヘッダーの横への上向き矢印または下向き矢印でこのことが示されます。

![データセットの参照タブワークスペース、[作成日] 列と [最終更新] 列が強調表示されます。](../images/datasets/user-guide/ascending-descending-columns.png)

## データセットの作成 {#create}

新しいデータセットを作成するには、**[!UICONTROL Create dataset]**&#x200B;ダッシュボードで&#x200B;**[!UICONTROL Datasets]**&#x200B;を選択して開始します。
...
[!DNL Experience Platform] UI で、左側のナビゲーションの「**[!UICONTROL Monitoring]**」を選択します。 **[!UICONTROL Monitoring]** ダッシュボードでは、バッチ取り込みまたはストリーミング取り込みからのインバウンドデータのステータスを表示できます。 個人ロットのステータス表示には、[ **[!UICONTROL Batch end-to-end]** ]または[ **[!UICONTROL Streaming end-to-end]**]を選択します。 ダッシュボードには、成功、失敗、またはまだ進行中のすべてのバッチまたはストリーミング取得の実行リストされます。 各リストには、バッチ ID、ターゲットデータセットの名前、取得したレコード数など、バッチの詳細が表示されます。ターゲット データセットが [!DNL Profile] に対して有効になっている場合は、取り込まれた ID レコードとプロファイルレコードの数も表示されます。
