...
>title="調整されたキャンペーン"
>abstract="この切替スイッチを有効にすると、選択したデータセットを Adobe Journey Optimizer の調整されたキャンペーンで使用できるようになります。データセットは 1 つのリレーショナルスキーマを使用する必要があり、スキーマごとにデータセットを 1 つだけ作成できます。"
...
「[!UICONTROL  参照 ]」タブのインラインオプションと「[!UICONTROL  データセットアクティビティ ] ビューの両方で、データセットサンプルデータをプレビューできます。 ナビゲーションとコンテキストが追加された新しいデータセットプレビューウィンドウが使用できます。

「[!UICONTROL  参照 ]」タブから、プレビューするデータセット名の横にある省略記号（...）を選択します。 オプションのリストが表示されます。 次に、使用可能なオプションから [!UICONTROL  データセットをプレビュー ] を選択します。 データセットが空の場合、プレビューリンクは非アクティブになり、プレビューが使用できないことを示します。
...
データセットプレビューウィンドウは、データセットの調査と検証を行うための合理化されたインターフェイスを提供します。
...
* データセットの構造をすばやくinsightするために、各列名の横に表示されるデータタイプインジケーター。
* ウィンドウの上部に SQL クエリが表示され、データセットの生成に使用したクエリが表示されます。
* 右下の領域に最大 100 行の書式設定されたテーブルビューがあり、効率的なデータレビューを実現します。
* Data Distiller ユーザーはクエリエディターに直接移動し、さらに調査や変更を行うために SQL クエリが事前入力された状態になります。

これらの機能は、迅速なナビゲーション、スキーマの把握、透過的なデータセット検証をサポートします。

インラインアクションまたは **[!UICONTROL データセットアクティビティ]** 画面から [!UICONTROL  データセットをプレビュー ] を選択して、プレビューウィンドウを開きます。

>[!NOTE]
>
>プレビューウィンドウには、最大 100 行のサンプルが表示されます。 データのないフィールドは、ビューから除外されます。

#### 高度なクエリエディターのショートカット {#query-editor-shortcut}

組織が Data Distiller ライセンスを持っている場合は、データセットプレビューウィンドウから詳細クエリエディターに直接アクセスできます。

>[!AVAILABILITY]
>
>この機能にアクセスできるのは、必要な Data Distiller ライセンスを持つユーザーのみです。 組織にデータDistillerがない場合、「[!UICONTROL  詳細クエリエディター ]」オプションは表示されません。

プレビューウィンドウの右上にある「**[!UICONTROL 詳細クエリエディター]**」を選択して、クエリエディターを開きます。 現在のプレビュークエリがプリロードされ、実行または追加の分析の準備が整います。

![ 右上に「詳細クエリエディター」ボタンが表示されているデータセットプレビューウィンドウ ](../images/datasets/user-guide/dataset-preview-advanced-query-editor.png)

このショートカットを使用すると、SQL やコンテキストを再入力することなく、サンプルデータのプレビューからクエリサービスでのクエリの実行と絞り込みにシームレスに移動できます。

追加のデータアクセスおよび分析には、[!DNL Query Service] や [!DNL JupyterLab] などのダウンストリームサービスを使用します。 詳しくは、次のドキュメントを参照してください。
