...
| パラメーター | 説明 |
| --- | --- |
| `{OBJECT_TYPE}` | 次のタイプの [!DNL Catalog] リストするオブジェクト。 有効なオブジェクトは次のとおりです。 <ul><li>`batches`</li><li>`dataSets`</li><li>`dataSetFiles`</li><li>`dataSetViews`</li></ul> |
...
正常な応答は、 [!DNL Catalog] オブジェクト。キーと値のペアの形式で指定し、リクエストで指定されたクエリパラメーターでフィルタリングします。 キーと値のペアごとに、キーは [!DNL Catalog] 問題のオブジェクト。これは、別の [特定のオブジェクトを表示](look-up-object.md) を参照してください。
