...
description: クエリパラメーターを使用して Catalog Service API の応答データをフィルタリングし、必要な情報のみを取得します。 API 呼び出しにフィルターを適用して、負荷を軽減しパフォーマンスを向上させ、より速く、より効率的なデータ取得を確保します。
...
`limit` クエリパラメーターでは、応答で返されるオブジェクトの数を制限します。 応答 [!DNL Catalog] 事前定義の制限に従います。
...
| パラメーター | 説明 |
| --- | --- |
| `{OBJECT_TYPE}` | 取得するオブジェ [!DNL Catalog] トのタイプ。 有効なオブジェクト： <ul><li>`batches`</li><li>`dataSets`</li><li>`dataSetFiles`</li></ul> |
| `{LIMIT}` | 返されるオブジェクトの数を指定する整数（範囲：1 ～ 100）。 |
...
| パラメーター | 説明 |
| --- | --- |
| `{OBJECT_TYPE}` | 取得するカタログオブジェクトのタイプ。有効なオブジェクトは次のとおりです。 <ul><li>`batches`</li><li>`dataSets`</li><li>`dataSetFiles`</li></ul> |
...
>[!NOTE]
>
>複数の `property` パラメーターを 1 つのクエリで組み合わせる場合は、少なくとも 1 つが `id` または `created` フィールドに適用される必要があります。 次のクエリは、`id` が `abc123` である **AND**`name` が `test` でないオブジェクトを返します。
>
>```http
...
| * | ワイルドカードは任意の文字列プロパティに適用され、任意の文字列のシーケンスに一致します。 リテラルのアスタリスクをエスケープするには、`**` を使用します。 | `property=name==te*st` |
| &amp; | 複数の `property` パラメーターを、それらの間の `AND` の関係で組み合わせます。 | `property=id==abc&property=name!=test` |
...
次のクエリ例では、`val1` と `val2` の両方を含む配列から結果のみを返します。
