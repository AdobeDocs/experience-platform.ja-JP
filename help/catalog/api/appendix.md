...
一部の [!DNL Catalog] オブジェクトは、他の [!DNL Catalog] オブジェクトと相互に関連付けることができます。 応答ペイロードで先頭に付くフィールド `@` は、関連オブジェクトを示します。 これらのフィールドの値はURIの形式をとります。URIは、それらが表す関連オブジェクトを取得するために別のGET要求で使用できます。
...
APIのルートエンドポイントは、1回の呼び出しで複数のリクエストを行うことができます。 [!DNL Catalog] 要求ペイロードには、通常は個々の要求となるものを表すオブジェクトの配列が含まれ、その配列が順に実行されます。

これらのリクエストがに対する変更または追加であり、いずれかの変更が失敗した場合 [!DNL Catalog] 、すべての変更が元に戻されます。
...
| プロパティ | 説明 |
| --- | --- |
| `id` | リクエストを応答に一致させるために、応答オブジェクトに添付されるユーザー指定ID。 [!DNL Catalog] はこの値を格納せず、単に参照用に応答として返します。 |
| `resource` | APIのルートを基準とする相対的なリソースパス [!DNL Catalog] 。 プロトコルとドメインは、この値の一部にすることはできません。また、「/」というプリフィックスを付ける必要があります。 <br/><br/> PATCHまたはDELETEをサブリクエストとして使用する場合は、オブジェクトIDをリソースパス `method`に含めます。 ユーザーが指定したIDと混同しないように、リソースパス `id`は [!DNL Catalog] オブジェクト自体のID(例えば `resource: "/dataSets/1234567890"`)を使用します。 |
...
成功した応答は、各要求に割り当てたオブジェクト `id` の配列、個々の要求のHTTPステータスコード、および応答を返し `body`ます。 3つのサンプルリクエストはすべて新しいオブジェクトを作成するためのものなので、各オブジェクト `body` の配列は、で最も成功したPOST応答を持つ標準のものと同様に、新しく作成されたオブジェクトのIDのみを含み [!DNL Catalog]ます。
...
圧縮とは、小さなファイルのデータを、データを変更せずに大きなファイルにマージする [!DNL Experience Platform] サービスです。 パフォーマンス上の理由から、小さなファイルのセットを大きなファイルに組み合わせて、クエリーを実行する際のデータへのアクセスを高速化すると便利な場合があります。
