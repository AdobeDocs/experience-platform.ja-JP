...
| パラメーター | 必須／オプション | 説明 |
|---|---|---|
| `xdm._experience.decisioning.propositions` | 必須 | この節では、手動でレンダリングした提案を定義します。 提案 `ID`、`scope`、`scopeDetails` を含める必要があります。 詳しくは、[ 表示イベントの管理 ](display-events.md) を参照してください。 手動でレンダリングしたパーソナライゼーションコンテンツを、ページヒットの下部に含める必要があります。 |
