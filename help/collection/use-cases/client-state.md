...
複数のタグ付けでは、同じプロパティ内の複数の SDK インスタンスが異なる組織を参照する可能性があります。すべての状態エントリには、複数のタグ付けをサポートするために、`kndctr_` と URL で使用できる組織 ID の接頭辞が自動的に付きます。
...
`Secure` フラグは、cookie のメタデータ表示枠内には存在しません。 `SameSite` 属性のみが含まれます。この場合、`SameSite` 属性が存在するときは常に `Secure` フラグを正しく設定するのはクライアントの責任です。`SameSite=None` の cookie は、セキュアなコンテキスト（HTTPS）を必要とするため、`Secure` 属性も設定する必要があります。
