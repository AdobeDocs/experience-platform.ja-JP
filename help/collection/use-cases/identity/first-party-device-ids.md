...
>[!IMPORTANT]
>
>ファーストパーティデバイス ID は Web SDKの [&#x200B; サードパーティ cookie](/help/tags/extensions/client/web-sdk/configure/identity.md) 機能と互換性がありません。 ファーストパーティデバイス ID またはサードパーティ Cookie のいずれかを使用できますが、両方を同時に使用することはできません。

## 前提条件 {#prerequisites}

開始する前に、ECID や `identityMap` など、web SDKでの ID データの仕組みに精通していることを確認してください。 詳しくは、[Web SDKの ID データ &#x200B;](id-overview.md) に関する概要を参照してください。
...
`HTTPOnly` フラグを使用して設定された Cookie には、クライアントサイドスクリプトを使用してアクセスできません。 つまり、`HTTPOnly` を設定する際に [!DNL FPID] フラグを設定する場合、`identityMap` に含める cookie の値を読み取るために、サーバーサイドのスクリプト言語を使用する必要があります。
...
1. [!DNL ECID] に含まれる `identityMap`
1. cookie に保存された [!DNL ECID]
1. [!DNL FPID] に含まれる `identityMap`
...
![FPID への移行後、訪問間に顧客の ID 値が更新される方法を示す図 &#x200B;](/help/collection/js/assets/identity/tracking/visits.png)
...
| 訪問 | 説明 |
| --- | --- |
| 初回訪問 | [!DNL FPID] cookie の設定をまだ開始していないとします。 [!DNL ECID]AMCV cookie[&#x200B; に含まれる &#x200B;](https://experienceleague.adobe.com/docs/id-service/using/intro/cookies.html?lang=ja#section-c55af54828dc4cce89f6118655d694c8) は、訪問者の識別に使用される識別子になります。 |
...
ファーストパーティデバイス ID （[!DNL FPIDs]）は、ファーストパーティ cookie を使用して訪問者を追跡します。 ファーストパーティ cookie は、DNS [&#x200B; または &#x200B;](https://datatracker.ietf.org/doc/html/rfc1035) コードとは異なり、DNS [A レコード &#x200B;](https://datatracker.ietf.org/doc/html/rfc3596) （IPv4 の場合）または [!DNL CNAME]AAAA レコード [!DNL JavaScript] （IPv6 の場合）を使用するサーバーを使用して設定する場合に最も効果的です。
...
Web SDKを Web サイト `example.com` ージに実装する場合を考えてみましょう。 Web SDKは、データをEdge Networkに `edge.adobedc.net` ドメインに送信します。

| [!DNL CNAME] なし | （[!DNL CNAME] を使用） |
|---------|----------|
| <ul><li>Web サイト `example.com` は、Web SDK ドメイン `edge.adobedc.net` を使用してEdge Networkにデータを送信します。</li><li>`edge.adobedc.net` で設定された Cookie は、`example.com` ドメインから取得されたものではないので、サードパーティ Cookie と見なされます。 お使いのブラウザーによっては、サードパーティ Cookie がブロックされ、お客様のデータがEdge Networkに到達しない場合があります。</li></ul> | <ul><li>`metrics.example.com` など、web SDKをデプロイするサブドメインを作成します。</li><li>[!DNL CNAME] が `metrics.example.com` を指すように、DNS システムで `edge.adobedc.net` レコードを設定します。</li><li>Web サイトで `metrics.example.com` を通じて cookie が設定されると、ブラウザーには、`example.com` （サードパーティ）ではなく `edge.adobedc.net` （ファーストパーティ）から取得されたように見えます。 これにより、ファーストパーティ ID cookie がブロックされにくくなり、より正確なデータ収集が可能になります。</li></ul> |
...
**手順 2. データストリームの&#x200B;**&#x200B;[!UICONTROL First Party ID Cookie]&#x200B;**機能を有効にする**

CNAME を設定したら、データストリームのオプション **[!UICONTROL First Party ID Cookie]** 有効にする必要があります。 この設定は、ファーストパーティデバイス ID を参照する際に、この値を [ID マップ &#x200B;](#identityMap) で参照するのではなく、指定された Cookie を参照するようにEdge Networkに指示します。

データストリームの設定方法については、[&#x200B; データストリーム設定ドキュメント &#x200B;](/help/datastreams/configure.md#advanced-options) を参照してください。
...
![&#x200B; ファーストパーティ ID の Cookie 設定を強調表示したデータストリーム設定を示す Platform UI 画像 &#x200B;](/help/collection/js/assets/first-party-id-datastreams.png)

この設定を有効にする場合、[!DNL FPID] が保存されていると期待される Cookie の名前を指定する必要があります。

>[!NOTE]
>
>ファーストパーティ ID を使用する場合、サードパーティ ID の同期を実行することはできません。 サードパーティの ID 同期は、[!DNL Visitor ID] サービスと、そのサービスによって生成される `UUID` に依存します。 ファーストパーティ ID 機能を使用する場合、[!DNL ECID] サービスを使用せずに [!DNL Visitor ID] が生成されるので、サードパーティ ID の同期が不可能になります。
><br> ファーストパーティ ID を使用する場合、Audience Managerのパートナー ID 同期が主に [&#x200B; または &#x200B;](https://experienceleague.adobe.com/ja/docs/audience-manager) に基づいていることを考慮すると、`UUIDs`Audience Manager`DIDs` の、パートナープラットフォームでのアクティベーション向けの機能はサポートされません。 ファーストパーティ ID から派生する [!DNL ECID] は、`UUID` にリンクされていないので、アドレス指定できません。

## 方法 2:`identityMap` での FPID の使用 {#identityMap}

[!DNL FPID] を独自の cookie に保存する代わりに、ID マップを通じてEdge Networkに [!DNL FPID] を送信することもできます。

[!DNL FPID] で `identityMap` を設定する方法の例を以下に示します。
...
他の ID タイプと同様に、[!DNL FPID] 内の他の ID と共に `identityMap` を含めることができます。 次に、認証済み [!DNL FPID] に含まれる [!DNL CRM ID] の例を示します。
...
`identityMap` の `primary` インジケーターがないため、次の [!DNL FPID] ではEdge Networkからエラー応答が返されます。 最後に、`identityMap` に存在する ID のうち 1 つを `primary` としてマークする必要があります。
