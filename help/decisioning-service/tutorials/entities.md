...
- **`limit`**:特定のリクエストに対して返す必要のある最大アイテム数に関するヒントとして正の整数を指定します。 実際の応答サイズは、応答パラメータの信頼性の高い動作を提供する必要性によって制約される、より小さいか大きい開始です
