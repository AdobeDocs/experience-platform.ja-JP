...
description: サンドボックスツール API の/tools エンドポイントを使用すると、Adobe Experience Platformでジョブの JSON データを取得できます。
...
サンドボックスツールを使用すると、様々なアーティファクトを選択し、それらをパッケージに書き出すことができます。 パッケージは、1 つのオブジェクトまたは複数のオブジェクトで構成できます。パッケージに含まれるオブジェクトは、同じサンドボックスからのものである必要があります。

サンドボックスツール API の `/tools` エンドポイントを使用すると、ジョブ JSON データのリストと取得が可能になります。

## ジョブの詳細 {#details}

ジョブの JSON データを個別に取得するには、`/tools` エンドポイントに対してGETリクエストを実行し、ジョブの ID を指定します。
...
応答が成功すると、クエリされたジョブ ID の詳細が返され、`completedTasks` に応じてリアルタイムのステータス更新が提供さ `failedTasks`、ジョブの進行に応じてステータスが更新されます。
