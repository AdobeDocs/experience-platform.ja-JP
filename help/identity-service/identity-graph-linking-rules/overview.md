...
* **共有デバイス**：共有デバイスとは、複数の個人で使用されるデバイスを指します。 共有デバイスの例としては、タブレット、図書館のコンピューター、キオスクなどがあります。
* **無効な E メールおよび電話番号**：メールアドレスや電話番号が正しくない場合、「テスト」など、無効な連絡先情報をエンドユーザーが登録することを示します<span>電子メールの場合は「@test.com」、電話番号の場合は「+1-111-111-1111」となります。
* **ID 値がエラーまたは正しくない**：エラーまたは不正な ID 値は、CRM ID を結合する可能性のある一意でない ID 値を参照しています。 例えば、IDFA には 36 文字（英数字 32 文字、ハイフン 4 つ）が必要ですが、ID 値が「user_null」の IDFA が取り込まれる場合があります。 同様に、電話番号は数字のみをサポートしますが、ID 値が「指定なし」の電話名前空間が取り込まれる場合があります。

ID グラフリンクルールのユースケースシナリオについて詳しくは、次のドキュメントを参照してください [サンプルシナリオ](./example-scenarios.md).
...
* Scott はタブレットを使用し、Google Chrome ブラウザーを開いてナイキに行きます<span>.com、彼がサインインして新しいバスケットボールの靴を閲覧します。
   * このシナリオでは、バックグラウンドで次の ID がログに記録されます。
      * ブラウザーの使用を表す ECID 名前空間および値
      * 認証済みユーザー（Scott がユーザー名とパスワードの組み合わせでサインイン）を表す CRM ID 名前空間と値。
* その後、息子のピーターも同じタブレットを使用し、Googleクロームも使用してナイキに行きます<span>.com、ここで彼はサッカー用具を参照するために自分のアカウントでサインイン。
   * このシナリオでは、バックグラウンドで次の ID がログに記録されます。
...
* **名前空間の優先度は数値です** 相対的な重要度を示す名前空間に割り当てられます。 これは、名前空間のプロパティです。
...
詳しくは、のガイドを参照してください。 [名前空間の優先度](./namespace-priority.md).
