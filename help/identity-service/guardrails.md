...
| （今後の行動）グラフの ID の数 [!BADGE ベータ版]{type=Informative} | 50 | 50 個の ID がリンクされたグラフが更新されると、ID サービスは、「先入れ先出し」メカニズムを適用し、最も古い ID を削除して、最新の ID 用の領域を確保します。削除は、ID タイプとタイムスタンプに基づいて行われます。上限は、サンドボックスレベルで適用されます。詳しくは、 [削除ロジックについて](#deletion-logic). |
