...
* [Experience Platform Web SDKの ID データ](/help/collection/use-cases/identity/id-overview.md)
...
この例では、ECID:32110 が取り込まれて `timestamp=51` で大きなグラフにリンクされるので、50 個の ID の制限を超えています。
...
ECID:35577 を削除した結果、削除された ECID を持つ CRMID:60013 および CRMID:25212 をリンクしていたエッジも削除され :35577 す。 この削除プロセスにより、グラフが 2 つの小さなグラフに分割されます。
...
以下の例では、ECID:21011 が取り込まれて `timestamp=51` でグラフにリンクされるので、50 個の ID の制限を超えています。

![](./images/guardrails/hub-and-spoke-start.png)

>[!TAB  削除プロセス ]

その結果、ID サービスは、ID グラフ（この場合は ECID:35577）からのみ最も古い ID を削除します。 ECID を削除すると :35577 次も削除されます。

* CRMID:60013 と削除された ECID:35577 の間のリンクなので、グラフ分割のシナリオが発生します。
