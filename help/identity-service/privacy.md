...
API でジョブリクエストを作成する際は、userID 内で指定するいずれの ID に対しても特定の名前空間およびタイプを使用する必要があります。 ID サービスによって認識される有効な ID 名前空間を名前空間値に指定する必要があります。 標準名前空間には `standard` を使用し、カスタム名前空間には `custom` を使用します。
...
UI でジョブリクエストを作成する場合は、**[!UICONTROL Identity]** に保存されたデータのジョブを処理するために、必ず **[!UICONTROL Products]** の下にある [!DNL Identity Service] を選択します。
...
ID サービスのプライバシーリクエスト（`ProfileService`）に製品としてリアルタイム顧客プロファイル（`aepDataLake`）とデータレイク（`identity`）も含めたかどうかに応じて、ID に関連する様々なデータセットが、場合によっては異なる時間にシステムから削除されます。
