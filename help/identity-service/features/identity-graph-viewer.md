...
Platform UI で、「 」を選択します。 **[!UICONTROL ID]** 左側のナビゲーションで「 」を選択し、 **[!UICONTROL ID グラフ]** を選択します。
...
>[!IMPORTANT]
>
>ID グラフでは、少なくとも 2 つのリンクされた ID を生成し、有効な ID 名前空間と値の組み合わせが必要です。 グラフビューアに表示できる ID の最大数は 50 です。 詳しくは、 [付録](#appendix) 詳しくは、以下の節を参照してください。
...
The [!UICONTROL ID] この表は、id データの様々な表示を示し、id 名前空間と id 値の組み合わせを表形式で示しています。 グラフ内のノードを選択すると、 [!UICONTROL ID] 表。
...
また、 *[!UICONTROL データソース]* id に影響を与えるデータソースのリストを確認するには、以下を実行します。 選択 [!UICONTROL データソース] ：データセットとバッチ ID の表形式の表示。
...
![選択されているグラフビューアのタイムスタンプスライダー。](../images/graph-viewer/slider-one.png)
...
スライダを調整して、グラフの最も古い反復を確認することもできます。 次の例では、ID グラフビューアに、グラフが最初に作成された日付が表示されています **[!UICONTROL 08/19/2020, 4:11:午後 49 時]**（最初のリンクが ECID、電子メールおよび電話）。
...
- ID グラフビューアは、最大 50 個の ID を超えることはできません。
