...
スキーマライブラリ内のフィールドグループを参照し、スキーマの設計とデータの取り込みに影響するスキーマの使用状況、フィルタリング、およびメタデータを理解するには、[UIのスキーマリソースの探索](../explore.md)を参照してください。
...
このガイドでは、XDM システムに関する実用的な理解が必要です。 Experience Platform エコシステム内でのXDMの役割の概要については、[XDMの概要](../../home.md)を参照し、XDM スキーマにフィールドグループがどのように貢献しているかについてスキーマ構成](../../schema/composition.md)の[基本を参照してください。
...
[!DNL Schema Editor]でスキーマを開いたら、左側のパネルの[!UICONTROL Field groups] セクションの横にある&#x200B;**[!UICONTROL Add]**&#x200B;を選択します。
...
[!DNL Schema Editor]が再び表示され、新しいフィールドグループが左側のパネルに表示されます。 これは新しいフィールドグループであるため、現在、スキーマにフィールドは提供されないため、キャンバスは変更されません。 フィールドグループ ](#add-fields)にフィールドを[追加できるようになりました。
...
![[!UICONTROL Standard]と[!UICONTROL Custom]がハイライト表示された[!UICONTROL Schemas] ワークスペースの[!UICONTROL Field groups] タブ。](../../images/ui/resources/field-groups/standard-and-custom-field-groups.png)

## 既存のフィールドグループの編集 {#edit}

>[!NOTE]
>
>組織で定義されたカスタムフィールドグループのみが完全に編集およびカスタマイズできます。 Adobeで定義されたコアフィールドグループの場合、個々のスキーマのコンテキスト内で編集できるのは、フィールドの表示名のみです。 スキーマエディターには、南京錠アイコン（![南京錠アイコン。](/help/images/icons/lock-closed.png)）で示されます。 詳しくは、[ スキーマフィールドの表示名の編集](./schemas.md#display-names)の節を参照してください。
>
