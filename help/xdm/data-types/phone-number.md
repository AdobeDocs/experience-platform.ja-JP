...
| `primary` | これが個人の主な電話番号であるかどうかを示す Boolean 値です。 アドレスや E メールアドレスとは異なり、複数の主電話番号（通信チャネルごとに 1 つ）が存在する場合があります。 通信チャネルは、タイプ（親プロパティの名前で示される）で定義されます。 `textMessaging`, `mobile`, `phone`, `home`, `work`, `unknown`、および `fax`. |
