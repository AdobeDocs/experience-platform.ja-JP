---
keywords: Experience Platform；ホーム；人気のあるトピック；スキーマ；スキーマ；XDM；フィールド；スキーマ；スキーマ；支払項目；データ型；データ型；
...
[!UICONTROL 支払] 項目は、支払のタイプ、金額および関連通貨を定義する注文に関連する支払を記述する、標準のExperience Data Model(XDM)データ型です。
