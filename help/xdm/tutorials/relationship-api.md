...
リレーションシップ・スキーマは、SQLテーブルの外部キーと同様に、リレーションシップ・レジストリ内で機能します。ソーススキーマ内のフィールドは、ターゲットスキーマのフィールドへの参照として機能します。 関係を定義する場合、各スキーマは、他の関係への参照として使用する専用のフィールドを持つ必要があります。スキーマ
