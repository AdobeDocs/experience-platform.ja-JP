...
>[!IMPORTANT]
>
>スキーマを [リアルタイム顧客プロファイルでの使用に対して有効にする場合](../../profile/home.md)、宛先スキーマの参照フィールドは **主IDである必要があります**。 これについては、このチュートリアルで後述します。
...
>[!NOTE]
>
>この手順は、 [リアルタイム顧客プロファイルでの使用を有効にするスキーマに対してのみ必要です](../../profile/home.md)。 スキーマを和集合に参加させたくない場合、またはスキーマにプライマリIDが既に定義されている場合は、次の手順に進んで、宛先スキーマの参照ID記述子 [を](#create-descriptor) 作成します。
