...
スキーマレジストリは、Adobe Experience Platform内のスキーマライブラリにアクセスするために使用されます。 スキーマライブラリには、アドビ、エクスペリエンスプラットフォームパートナー、および使用するアプリケーションのベンダーが提供するリソースが含まれています。 レジストリは、使用可能なすべてのライブラリリソースにアクセスできるユーザーインターフェイスとRESTful APIを提供します。

このチュートリアルでは、スキーマレジストリAPIを使用して、標準クラスを使用してスキーマを作成する手順を説明します。 Experience Platformでスキーマインターフェイスを使用したい場合は、スキーマエディターで同様の操作を実行するための手順を、 [エディター](create-schema-ui.md) 「チュートリアル」で順を追って説明します。
...
* [Experience Data Model(XDM)System](../home.md):エクスペリエンスプラットフォームが顧客エクスペリエンスデータを整理するための標準化されたフレームワーク。
   * [スキーマ構成の基本](../schema/composition.md):XDMスキーマの基本的な構成要素について説明します。この中には、主な原則や構成のベストプラクティスが含まれています。スキーマ構成の基本要素です。
...
このチュートリアルでは、忠誠度メンバーのスキーマの構成手順を説明し、小売忠誠度メンバーのメンバーに関連するデータについて説明します。プログラム 開始する前に、付録の完全な忠誠度メンバー [スキーマをプレビューする](#complete-schema) ことができます。

## 標準クラスでスキーマを構成する

スキーマは、エクスペリエンスプラットフォームに取り込むデータの青写真と見なすことができます。 各スキーマは、クラスと0個以上のミックスインで構成されます。 つまり、スキーマを定義するためにミックスインを追加する必要はありませんが、ほとんどの場合は少なくとも1つのミックスインが使用されます。

### クラスの割り当て

スキーマ構成プロセスは、クラスの選択から始まります。 このクラスは、データの主要な動作面（レコードと時系列）と、取り込まれるデータを記述するために必要な最小フィールドを定義します。

このチュートリアルで作成するスキーマでは、XDM Individualプロファイルクラスを使用します。 XDM個々のプロファイルは、レコードの動作を定義するためにアドビが提供する標準クラスです。 動作に関する詳細は、組版の基本を参 [照してください](../schema/composition.md)。

クラスを割り当てるために、API呼び出しが行われ、テナントコンテナ内に新しいスキーマが作成(POST)されます。 この呼び出しには、スキーマが実装するクラスが含まれます。 各スキーマは、1つのクラスのみを実装できます。
...
リクエストには、クラスのを参 `allOf` 照する属性を含める `$id` 必要があります。 この属性は、スキーマが実装する「基本クラス」を定義します。 この例では、基本クラスはXDM Individualプロファイルクラスです。 XDM個 `$id` 別プロファイルクラスの値は、以下の配列内のフ `$ref` ィールドの値とし `allOf` て使用されます。
...
リクエストが成功すると、HTTP Response Status 201(Created)が返され、応答本文に、、およびを含む新しく作成されたスキーマの詳細が `$id`含ま `meta:altIt`れていま `version`す。 これらの値は読み取り専用で、割り当てはスキーマレジストリです。
...
新しく作成したスキーマを表示するには、またはエンコードされたURL `meta:altId``$id` URIを使用して、参照(GET)リクエストをスキーマに対して実行します。
...
応答の形式は、リクエストで送信されるAcceptヘッダーによって異なります。 異なるAcceptヘッダーを試してみて、どのヘッダーがニーズに最も適しているかを確認してください。
...
ロイヤルティメンバースキーマが作成され、確認されたので、ミックスインを追加できます。

選択したミックスのクラスに応じて、使用できる標準ミックスインが異なります。スキーマの種類によって異なります。 各ミックスインには、そ `intendedToExtend` のミックスインと互換性があるクラスを定義するフィールドが含まれています。

ミックスインは、「名前」や「住所」など、同じ情報を取り込む必要のあるスキーマで再利用できる概念を定義します。
...
この要求は、忠誠度メンバースキーマを更新(PATCH)し、「プロファイル — 人 — 詳細」ミックスイン内のフィールドを含めます。

「プロファイル — 個人 — 詳細」ミックスインを追加することで、忠誠度メンバースキーマは、姓、名、誕生日などの忠誠度プログラムメンバーに関する情報を取り込むようになりました。
...
応答には、新しく追加されたmixinが配列に `meta:extends` 表示され、mixinに対 `$ref` するが属性に含まれ `allOf` ます。
...
>[!TIP] 使用可能なすべてのミックスインを確認して、各ミックスインに含まれるフィールドに慣れておくことをお勧めします。 「グローバル」コンテナと「テナント」要素に対してリクエストを実行し、「meta:intentedToExtend」フィールドが使用しているクラスと一致するミックスインのみを返すことで、特定のクラスで使用できるすべてのミックスインをリスト(GET)できます。 この場合、XDM Individualプロファイルクラスなので、XDM Individualプロファイルが使用さ `$id` れます。
...
この要求は、忠誠度メンバースキーマを更新(PATCH)し、「プロファイル — 個人 — 詳細」ミックスイン内のフィールドを含め、「ホームアドレス」、「電子メールアドレス」、「ホーム電話」の各フィールドをスキーマに追加します。
...
[忠誠度メンバ]スキーマには、次の3つの値が配 `$ref` 列に含まれている必要が `allOf` あります。「プロファイル」、「プロファイル — 個人 — 詳細」および「プロファイル — 個人 — 詳細」。
...
忠誠度メンバースキーマは、忠誠度メンバーに固有の情報を取り込む必要があります。プログラム。 この情報は、標準ミックスインには含まれません。

スキーマレジストリは、テナントレジストリ内で独自のミックスインを定義できるので、これを考慮してコンテナします。 これらのミックスインは組織に固有のもので、IMS組織外のユーザーは表示したり編集したりすることはできません。

新しいミックスインを作成(POST)するには、リクエストに、ミックスインに互換性がある基本クラスのを含むフィールドと、ミックスインに含めるプロパティを含める必要があります。 `meta:intendedToExtend``$id`

他のミックスインやフィールドとの衝突を避けるために、す `TENANT_ID` べてのカスタムプロパティをユーザの下にネストする必要があります。
...
このリクエストは、次の4つの忠誠度プログラム固有のフィールドを含む「忠誠度」オブジェクトを持つ新しいミックスインを作成します。&quot;loyaltyId&quot;、&quot;loyaltyLevel&quot;、&quot;loyaltyPoints&quot;および&quot;memberSince&quot;。
...
リクエストが成功すると、HTTP応答ステータス201（作成済み）が返され、応答本文に、、およびを含む新しく作成されたミックスインの詳細が `$id`含ま `meta:altIt`れていま `version`す。 これらの値は読み取り専用で、割り当てはスキーマレジストリです。
...
同じ手順で標準のミックスインを追加し [て、新しく作成したミックスインを](#add-a-mixin) スキーマに追加できます。
...
この要求は、忠誠度メンバースキーマを更新(PATCH)し、新しい「忠誠度メンバーの詳細」ミックスイン内のフィールドを含めます。
...
応答に新しく追加されたmixinが配列に表示され、mixin属性にmixinが含まれているので、mixinが正常に追加され `meta:extends` たこ `$ref` とを確認でき `allOf` ます。
...
Acceptヘッダーを使用す `application/vnd.adobe.xed-full+json; version=1` ると、すべてのプロパティを表示する完全なスキーマを表示できます。 これらのプロパティは、クラスおよびミックスインがスキーマの構成に使用したフィールドです。 この応答例では、個々のプロパティ属性が空き領域に対して最小化されています。 このスキーマの最後にある付録で、すべてのプロパティとその属性を含む完全な [表示](#appendix) （全体）をドキュメントできます。

の下に、 `"properties"`カスタムMixinを追加したときに `_{TENANT_ID}` 作成された名前空間が表示されます。 この名前空間内には、「忠誠度」オブジェクトと、Mixinの作成時に定義されたフィールドが含まれます。
...
作成した忠誠度ミックスインには、他のユーザーに役立つ特定の忠誠度プロパティが含まれています。スキーマ 例えば、データをエクスペリエンスイベントの一部として取り込んだり、別のクラスを実装するスキーマが使用したりできます。 この場合、オブジェクト階層をデータ型として保存し、別の場所で簡単に定義を再利用できるようにすると効果的です。

データ型を使用すると、1つのオブジェクト階層を1回定義し、他のスカラ型と同様にフィールド内で参照できます。

つまり、データ型を「型」として追加することでスキーマ内のどこにも含められるので、ミックスインよりも柔軟に、複数フィールド構造を一貫して使用できます。
...
リクエストが成功すると、HTTP Response Status 201(Created)が返され、応答本文に、、およびを含む新しく作成されたデータ型の詳細が `$id`含ま `meta:altIt`れま `version`す。 これらの値は読み取り専用で、割り当てはスキーマレジストリです。
...
URLエンコードされたURIを使用して参照(GET)要求を実行し、新しいデータ型を直接 `$id` 表示することができます。 参照リクエストのAcceptヘッ `version` ダーにを必ず含めてください。

### 「Use data type in」スキーマ

忠誠度の詳細データ型が作成されたので、作成したミックスインの「忠誠度」フィールドを更新(PATCH)して、以前に存在していたフィールドの代わりにデータ型を参照できます。
...
応答に、以前に定義したフィールドの代わりに、「`$ref`忠誠度」オブジェクトのデータタイプへの参照()が含まれるようになりました。
...
スキーマを参照するGET要求を実行すると、次に示すように、「properties/_{TENANT_ID}」の下にデータ型への参照が表示されるようになりました。
...
スキーマは、データをエクスペリエンスプラットフォームに取り込むために使用されます。 このデータは、最終的に複数のサービスで使用され、個人の単一の統合表示を作成します。 この処理を支援するために、主要フィールドを「ID」としてマークし、データ取り込み時に、これらのフィールド内のデータをその個人の「IDグラフ」に挿入できます。 その後、リアルタイム顧客プロファイルおよび他の [Experience Platformサービスによって](../../profile/home.md) 、グラフデータにアクセスして、各顧客の表示をつなぎ合わせることができます。

一般に「ID」とマークされるフィールドには、次のものが含まれます。電子メールアドレス、電話番号、 [Experience Cloud ID(ECID)](https://marketing.adobe.com/resources/help/ja_JP/mcvid/)、CRM IDまたはその他の一意のIDフィールド。

組織に固有の識別子は、適切なIDフィールドである場合もあるので、それらを考慮します。

ID記述子は、「sourceSchema」の「sourceProperty」が「Identity」と見なす必要のある一意の識別子であることを示します。

記述子の操作について詳しくは、『 [スキーマレジストリ開発者ガイド](../api/getting-started.md)』を参照。
...
次のリクエストは、「loyaltyId」フィールドにID記述子を定義します。 これにより、Experience Platformは、一意の忠誠度プログラムのメンバー識別子（この場合は、メンバーの電子メールアドレス）を使用して、個人に関する情報を組み合わせるのに役立つようになります。
...
>[!NOTE] 使用可能な「xdm: [名前空間」値をリストしたり、](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/id-service-api.yaml)Identity Service APIを使用して新しい値を作成したりできます。 「xdm:property」の値は、使用する「xdm:名前空間」に応じて、「xdm:code」または「xdm:id」にすることができます。

**応答**

成功した応答は、新しく作成された記述子の詳細（記述子を含む）を含む応答本文と共に、HTTPステータス201（作成済み）を返しま `@id`す。 は読み取 `@id` り専用のフィールドで、スキーマレジストリによって割り当てられ、APIの記述子を参照するために使用されます。
...
属性に「和集合」タグを追加することで、 `meta:immutableTags` リアルタイム顧客スキーマが使用する忠誠度メンバープロファイルを有効にできます。

和集合表示の使用に関する詳細は、『スキーマレジストリ開発者ガイド [](../api/unions.md) 』の和集合に関する節を参照してください。
...
応答には、操作が正常に実行され、スキーマには最上位の属性(「和集合」という値を含む `meta:immutableTags`配列)が含まれています。
...
これで、XDM個人スキーマのプロファイル和集合 同じ和集合に属するすべてのスキーマのリストを表示するには、クエリパラメータを使用してGETリクエストを実行し、応答をフィルタリングします。

クエリパ `property` ラメーターを使用して、XDM個々のスキーマクラスと等しいフ `meta:immutableTags` ィールドを含むプロファイル `meta:class` のみが返されるよ `$id` うに指定できます。
...
次の例のリクエストは、XDM個々のスキーマの一部であるすべてのプロファイル和集合を返します。
...
応答はフィルタされたリストのスキーマで、両方の要件を満たすもののみが含まれます。 複数のパラメーターを使用する場合は、ANDクエリが想定されることに注意してください。 応答の形式は、リストで送信されるAcceptヘッダーによって異なります。
...
このチュートリアルに従うと、標準のミックスインと定義したスキーマの両方を使用して、ミックスインを正しく構成することができます。 これで、このスキーマを使用して、データセットを作成し、レコードデータをAdobe Experience Platformに取り込むことができます。

このチュートリアル全体で作成した[忠誠度メンバ]スキーマは、次の付録で完全に利用できます。 スキーマを見ると、ミックスインが全体的な構造にどのように貢献しているか、およびデータ取り込みに使用できるフィールドを確認できます。

複数のリレーションを作成したら、スキーマ記述子を使用して、リレーションシップを定義できます。 詳しくは、2つの関係を定 [義するためのチュートリアルを参照してください](relationship-api.md) 。スキーマを参照してください。 レジストリ内のすべての操作(GET、POST、PUT、PATCH、DELETE)を実行する方法の詳細な例については、APIを使用する際に、 [スキーマレジストリ開発者ガイド](../api/getting-started.md) を参照してください。

## 付録 {#appendix}

次の情報は、APIチュートリアルを補完するものです。

## 完全な忠誠度メンバーのスキーマ {#complete-schema}

このチュートリアル全体で、スキーマは小売忠誠度のメンバーを説明するために構成されています。プログラム

このスキーマはXDM Individualプロファイルクラスを実装し、複数のミックスインを組み合わせます。標準の「個人の詳細」と「個人の詳細」ミックスインを使用して忠誠度メンバーに関する情報を取り込む。また、このチュートリアルで定義された「忠誠度の詳細」ミックスインを使用して、忠誠度メンバーに関する情報を取り込みます。
