...
このリクエストは、「ロイヤルティメンバー」スキーマを更新 (PATCH) し、「profile-person-details」フィールドグループ内のフィールドを含めます。

「プロファイル — 個人 — 詳細」フィールドグループを追加すると、「ロイヤルティメンバー」スキーマは、姓、名、誕生日など、ロイヤルティプログラムメンバーに関する情報を取り込むようになります。
...
このリクエストは、「ロイヤルティメンバー」スキーマを更新 (PATCH) し、「profile-personal-details」フィールドグループ内のフィールドを含め、「自宅住所」、「電子メールアドレス」、「自宅電話」の各フィールドをスキーマに追加します。
...
この [!DNL Schema Registry] を使用するには、テナントコンテナ内で独自のフィールドグループを定義します。 これらのフィールドグループは組織に固有のもので、IMS 組織外のユーザーは表示したり編集したりすることはできません。
...
の下 `"properties"`に設定されている場合、 `_{TENANT_ID}` カスタムフィールドグループを追加したときに作成された名前空間。 その名前空間内には、「loyalty」オブジェクトと、フィールドグループの作成時に定義されたフィールドが含まれます。
...
作成した「Loyalty」フィールドグループには、他のスキーマで役立つ特定のロイヤルティプロパティが含まれています。 例えば、データはエクスペリエンスイベントの一部として取得されたり、別のクラスを実装するスキーマによって使用されたりします。この場合、オブジェクト階層をデータ型として保存し、別の場所で簡単に定義を再利用できるようにすると効果的です。
...
次のリクエストは、「loyaltyId」フィールドに ID 記述子を定義します。これは [!DNL Experience Platform] 一意のロイヤルティプログラムメンバー識別子（この場合はメンバーの電子メールアドレス）を使用して、個人に関する情報を組み合わせるのに役立ちます。
...
スキーマがを実装します。 [!DNL XDM Individual Profile] 複数のフィールドグループを組み合わせ、クラスを設定します。標準の「人物の詳細」フィールドグループと「個人の詳細」フィールドグループを使用して、およびチュートリアル中に定義された「ロイヤルティの詳細」フィールドグループを使用して、ロイヤルティメンバーに関する情報を取り込みます。
