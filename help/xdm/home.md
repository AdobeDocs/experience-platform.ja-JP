...
![の人気列 [!UICONTROL フィールドグループを追加] ダイアログ。](./images/overview/popularity.png)
...
また、を使用して、組織に固有の新しいスキーマとリソースを作成および管理することもできます [!DNL Schema Registry API]、または [!UICONTROL スキーマ] platform UI のワークスペース。
...
>title="データの動作"
>abstract="Experience Platform での使用を意図したデータは、レコード、時系列およびアドホックの 3 つの動作タイプに分けられます。レコードスキーマは、サブジェクトの属性に関する情報を提供し、時系列スキーマは、アクションが実行された時点でのシステムのスナップショットをキャプチャします。アドホックスキーマは、単一のデータセットでのみ使用するために名前空間が設定されているフィールドをキャプチャします。 Platform でのデータの動作について詳しくは、ドキュメントを参照してください。"
...
内で独自のクラスを定義できますが、 [!DNL Schema Registry]の場合は、標準クラスを使用することをお勧めします **[!UICONTROL XDM 個人プロファイル]** および **[!UICONTROL XDM ExperienceEvent]** （レコードの場合）と（時系列データの場合）。 これらのクラスの詳細については、以下で説明します。

>[!NOTE]
>
>アドホック動作に基づく標準クラスはありません。アドホックスキーマは、それを使用する Platform プロセスによって自動的に生成されますが、次のこともあります [スキーマレジストリ API を使用して手動で作成](./tutorials/ad-hoc.md).
...
![時間の経過に伴うエクスペリエンスイベントで視覚化されたカスタマージャーニーのインフォグラフィック。](images/overview/experience-event-journey.png)
...
カタログデータは、接触チャネルやファイル形式に関係なく、Platform で管理されるすべてのデータを含む、非常にきめ細かいデータストアであるデータレイクに保存されます。
...
を参照してください。 [Catalog Service の概要](../catalog/home.md) を参照してください。 を参照してください。 [データ取り込みの概要](../ingestion/home.md) Adobe Experience Platformのデータ取得について詳しくは、こちらを参照してください。

### クエリサービス {#query-service}

標準の SQL を使用してExperience Platformデータに対してクエリを実行し、Adobe Experience Platform クエリサービスの様々なユースケースをサポートできます。
...
リアルタイム顧客プロファイルは、に基づいてスキーマ形式のデータを使用します [!UICONTROL XDM 個人プロファイル] および [!UICONTROL XDM ExperienceEvent] そのデータに基づいてクエリをクラス分けし、応答します。
...
As [!UICONTROL XDM 個人プロファイル] および [!UICONTROL XDM ExperienceEvent] データがデータレイクに取り込まれ、リアルタイム顧客プロファイルは、使用できるようになっているすべてのデータを取り込みます。 取り込まれるインタラクションや詳細が多いほど、個人プロファイルは正確になります。
...
Adobe Experience Platform Data Science Workspace は、機械学習と人工知能を使用して、Experience Platform 内に保存されたデータからインサイトを得ます。データサイエンティストは、Data Science Workspace を使用すると、に基づいてレシピを作成できます [!UICONTROL XDM 個人プロファイル] および [!UICONTROL XDM ExperienceEvent] 顧客とそのアクティビティに関するデータ。 これらのレシピは、購入傾向や、個人が評価して使用する可能性が高い推奨オファーなどの予測を容易にします。
