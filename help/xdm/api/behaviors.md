...
`/behaviors`エンドポイントへのGET要求のパスにIDを指定すると、特定のビヘイビアーを検索できます。
...
| パラメーター | 説明 |
| --- | --- |
| `{BEHAVIOR_ID}` | 調べるビヘイビアーの`meta:altId`またはURLエンコードされた`$id`。 |

**リクエスト**

次の要求は、要求パスに`meta:altId`を指定して、レコードビヘイビアーの詳細を取得します。
...
応答が成功すると、ビヘイビアーのバージョン、説明、ビヘイビアーを使用するクラスにビヘイビアーが提供する属性など、ビヘイビアーの詳細が返されます。
...
このガイドでは、[!DNL Schema Registry] APIでの`/behaviors`エンドポイントの使用について説明します。 APIを使用してクラスにビヘイビアーを割り当てる方法については、[クラスエンドポイントガイド](./classes.md)を参照してください。