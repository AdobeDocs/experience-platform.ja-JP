...
次のスクリーンショットは、データタイプがプラットフォームUIでどのように表されるかを示しています。 [!UICONTROL 人口統計詳細]ミックスインで提供されるフィールドの1つは、「[!UICONTROL 人名]」データ型を使用します。これは、フィールド名の横のパイプ文字(`|`)に続くテキストで示されます。 この特定のデータ型は、個々の人の名前に関連するいくつかのサブフィールドを提供します。この構成体は、人の名前を取り込む必要がある他のフィールドで再利用できます。
