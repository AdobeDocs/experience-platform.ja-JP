...
このプロセスに役立つように、スキーマ内のキーフィールドを ID としてマークできます。 データの取り込み時に、これらのフィールドのデータは、その個人の「[!UICONTROL Identity Graph]」に挿入されます。 その後、[[!DNL Real-Time Customer Profile]](../../profile/home.md) やその他のExperience Platform サービスからグラフデータにアクセスし、個々の顧客の関連付けられたビューを提供できます。
...
>title="使用中のスキーマを編集"
>abstract="このスキーマは、データセットまたは取り込みワークフローで既に使用されています。 データフローが実行中またはスケジュール済みの場合、既存のフィールドまたは ID を変更すると、取り込みエラーが発生する可能性があります。 取り込み開始後は、追加のスキーマ変更のみがサポートされます。"
...
>[!NOTE]
>
>スキーマに重大な変更を加えることができるのは、スキーマがExperience Platformへのデータの取り込みにまだ使用されておらず、リアルタイム顧客プロファイルでの使用が有効になっていない場合のみです。 ただし、スキーマがExperience Platformで使用されたら、追加バージョン管理ポリシーに従う必要があります。 スキーマのデータセットを作成すると、スキーマ進化のルールが、スキーマ内のすべてのフィールドに対してシステムによって厳密に適用されます。 フィールドが参照されているすべての XDM スキーマで編集できなくなります。
...
例えば、「[!UICONTROL First Name]」スキーマの「[!UICONTROL Home Address]」や「[!UICONTROL Loyalty Members]」などの詳細を取り込むには、これらの一般的な概念を定義する標準のフィールドグループを使用できます。 ただし、標準のフィールドグループではカバーされない可能性のある、組織に固有の概念（カスタムロイヤルティプログラムの詳細や製品属性など）です。 この場合、この情報を取得するには、独自のフィールドグループを定義する必要があります。
...
Web サイトのトラフィックを追跡する別のスキーマは、「[!UICONTROL Web Visits]」と呼ばれる場合があります。 また、[!DNL XDM ExperienceEvent] クラスも実装していますが、今回は標準の [!UICONTROL Web] フィールドグループを組み合わせたものです。

次の図に、これらのスキーマと、各フィールドグループによって提供されるフィールドを示します。 また、このガイドで前述した「[!DNL XDM Individual Profile]」スキーマなど、[!UICONTROL Loyalty Members] クラスに基づく 2 つのスキーマも含まれています。
