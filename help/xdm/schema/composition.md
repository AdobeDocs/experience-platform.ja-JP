...
>title="業種"
>abstract="自社のビジネスに関連する業界を選択した場合、機械学習モデルは、業界標準に合致する標準フィールドグループでソースフィールドをより正確にマッピングすることで、より優れたデータ編成を提供できます。 これにより、業界固有のニーズに合わせてデータ統合がカスタマイズされるので、より正確で関連性の高いデータインサイトが得られます。"
...
>title="フィールドグループ"
>abstract="フィールドグループは、追加の属性でスキーマを拡張できる、再利用可能なコンポーネントです。 ほとんどのフィールドグループは、特定のクラスにしか対応していません。 Adobeが定義した標準フィールドグループを使用するか、独自のカスタムフィールドグループを手動で定義できます。 フィールドグループがスキーマ構成にどのように関わっているかについて詳しくは、ドキュメントを参照してください。"

>[!CONTEXTUALHELP]
>id="platform_schemas_fieldgroup_requiredFieldgroup"
>title="必須のフィールドグループ"
>abstract="このフィールドグループは、使用しているソースに必須です。 このため、スキーマから削除することはできません。"
