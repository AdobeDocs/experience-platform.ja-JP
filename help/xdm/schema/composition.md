...
次のスクリーンショットは、プラットフォームUIでクラスがどのように表されるかを示しています。 表示される例のスキーマにはミックスインが含まれていないので、表示されるすべてのフィールドはスキーマのクラス([!UICONTROL XDM Individualプロファイル])によって提供されます。
...
次のスクリーンショットは、プラットフォームUIでのミックスインの表現方法を示しています。 この例のスキーマには、1つのミックスイン（[!UICONTROL 人口統計の詳細]）が追加され、スキーマの構造に対してフィールドのグループを提供します。

![](../images/schema-composition/mixin.png)

利用可能な標準XDMミックスインの最新リストについては、[正式なXDMリポジトリ](https://github.com/adobe/xdm/tree/master/components/mixins)を参照してください。 また、UIで表示リソースを使用したい場合は、[XDMコンポーネントの詳細](../ui/explore.md)のガイドを参照してください。
...
次のスクリーンショットは、データタイプがプラットフォームUIでどのように表されるかを示しています。 （[!UICONTROL 人口統計詳細]）ミックスインで提供されるフィールドの1つは、「[!UICONTROL 人名]」というデータ型を使用します。これは、フィールド名の横のパイプ文字(`|`)に続くテキストで示されます。 この特定のデータ型は、個々の人の名前に関連するいくつかのサブフィールドを提供します。この構成体は、人の名前を取り込む必要がある他のフィールドで再利用できます。
...
* [[!UICONTROL セグメントのメンバーシップ] の詳細Mixin](../mixins/profile/segmentation.md):顧客プロファイルを特定のセグメントに追加関連付けるために、  XDM個々のプロファイルスキーマにこのミックスインを行います。
