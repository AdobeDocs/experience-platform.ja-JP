...
[!UICONTROL セグメントのメンバーシップ] の詳細： [[!DNL XDM Individual Profile] クラスの標準スキーマフィールドグループ](../../classes/individual-profile.md)。フィールドグループには、個々の属するセグメント、最後のクオリフィケーション時間、およびメンバーシップが有効になるまでの期間など、セグメントのメンバーシップに関する情報を取り込む単一のマップフィールドが提供されます。

>[!WARNING]
>
>`segmentMembership`フィールドは、このフィールドグループを使用してプロファイルスキーマに手動で追加する必要がありますが、手動でこのフィールドに値を入力したり、更新したりしないでください。 セグメント化ジョブの実行時に、各プロファイルの`segmentMembership`マップが自動的に更新されます。
