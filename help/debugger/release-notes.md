...
* ユーザーが技術的および個人的なデータ収集をオプトイン/オプトアウトできるようにします。

### 修正点および改善点

* Firefox スクリプトの挿入とプライバシーポリシーのリンクを修正しました。
* 不足している Analytics リクエストをキャプチャします。
* 多数の複雑なコンソールメッセージを含むページでのクラッシュ修正。
* Adobe Experience Platform Debuggerを Manifest v3 拡張機能に更新します。
...
* 「デバッガーを開いたときにアクティブなタブにロック」設定を追加しました。

### 修正点および改善点

* プライベートドメインで Analytics リクエストが欠落していた問題を修正しました。
* Analytics リクエストテーブルからActivity Mapデータが欠落する問題を修正しました。
* Target トレースを表示するとクラッシュが発生する問題を修正しました。
* Firefox でデバッガーがオンページインフラストラクチャのセットアップに失敗した場合の警告を追加しました。
...
* Analytics イベントが無視または重複される問題を修正しました。
* 状態のストレージサイズの上限を超える問題を修正しました。
...
* Assurance セッションで CORS エラーが表示される問題を修正しました
* Target Trace が表示されない問題を修正しました。
* 「フィードバックを送信」ボタンを修正しました。
* バージョン ≥2.18.0 の Web SDK 概要に見つからない「データストリーム ID」を修正しました。
